...
archivePathSeparators = (u".zip" + os.sep, u".tar.gz" + os.sep, u".eis" + os.sep, u".xml" + os.sep, u".xfd" + os.sep, u".frm" + os.sep, u'.taxonomyPackage.xml' + os.sep) + \
                        ((u".zip/", u".tar.gz/", u".eis/", u".xml/", u".xfd/", u".frm/", u'.taxonomyPackage.xml/') if os.sep != u"/" else ()) #acomodate windows and http styles

POST_UPLOADED_ZIP = os.sep + u"POSTupload.zip"
SERVER_WEB_CACHE = os.sep + u"_HTTP_CACHE"

XMLdeclaration = re.compile(ur"<\?xml[^><\?]*\?>", re.DOTALL)

TAXONOMY_PACKAGE_FILE_NAMES = (u'.taxonomyPackage.xml', u'catalog.xml')
...
def archiveFilenameParts(filename, checkIfXmlIsEis=False):
    ...
    for archiveSep in archivePathSeparators:
        if (filename and 
            ...
            (not archiveSep.startswith(u".xml") or checkIfXmlIsEis)):
...
class ArchiveFileIOError(IOError):
    ...
    def __str__(self):
        return _(u"Archive does not contain file: {0}, archive: {1}").format(self.fileName, self.url)
            
class FileSource(object):
    def __init__(self, url, cntlr=None, checkIfXmlIsEis=False):
        self.url = unicode(url)  # allow either string or FileNamedStringIO
        self.baseIsHttp = isHttpUrl(self.url)
        self.cntlr = cntlr
        self.type = self.url.lower()[-7:]
        self.isTarGz = self.type == u".tar.gz"
        if not self.isTarGz:
            self.type = self.type[3:]
        self.isZip = self.type == u".zip"
        self.isEis = self.type == u".eis"
        self.isXfd = (self.type == u".xfd" or self.type == u".frm")
        self.isRss = (self.type == u".rss" or self.url.endswith(u".rss.xml"))
        ...
        if (not (self.isZip or self.isEis or self.isXfd or self.isRss) and
            ...
            if os.path.split(self.url)[-1] in TAXONOMY_PACKAGE_FILE_NAMES:
                ...
                try:
                    file = open(self.cntlr.webCache.getfilename(self.url), u'r', errors=u'replace')
                    l = file.read(128)
                    file.close()
                    if re.match(ur"\s*(<[?]xml[^?]+[?]>)?\s*<cor[a-z]*:edgarSubmission", l):
                        self.isEis = True
                except EnvironmentError, err:
                    if self.cntlr:
                        self.cntlr.addToLog(_(u"[{0}] {1}").format(type(err).__name__, err))
                    pass
    ...
    def open(self):
        if not self.isOpen:
            ...
            if self.isZip:
                try:
                    self.fs = zipfile.ZipFile(openFileStream(self.cntlr, self.basefile, u'rb'), mode=u"r")
                    self.isOpen = True
                except EnvironmentError, err:
                    self.logError(err)
                    pass
                ...
                try:
                    self.fs = tarfile.open(self.basefile, u"r:gz")
                    self.isOpen = True
                except EnvironmentError, err:
                    self.logError(err)
                    pass
                ...
                buf = ''
                try:
                    file = open(self.basefile, u'rb')
                    more = True
                    while more:
                        l = file.read(8)
                        if len(l) < 8:
                            break
                        if len(buf) == 0 and l.startswith("<?xml "): # not compressed
                ...
                if firstline.startswith("application/x-xfdl;content-encoding=\"asc-gzip\""):
                    ...
                    while i < len(fb):
                        ...
                        gzchunk = (str((31,139,8,0)) + fb[i:i+lenCompr])
                        try:
                    ...
                    file = io.StringIO(initial_value=ungzippedBytes.decode(u"utf-8"))
                else:
    ...
    def fileSourceContainingFilepath(self, filepath):
        if self.isOpen:
            ...
            u''' change to return file source if archive would be in there (vs actually is in archive)
    ...
    def file(self, filepath, binary=False, stripDeclaration=False, encoding=None):
        ...
        if archiveFileSource is not None:
            ...
            if archiveFileSource.isZip:
                try:
                    b = archiveFileSource.fs.read(archiveFileName.replace(u"\\",u"/"))
                    if binary:
                        return (io.BytesIO(b), )
                    if encoding is None:
                ...
                for docElt in self.eisDocument.iter(tag=u"{http://www.sec.gov/edgar/common}document"):
                    ...
                    if outfn == archiveFileName:
                        b64data = docElt.findtext(u"{http://www.sec.gov/edgar/common}contents")
                        if b64data:
                            b = base64.b64decode(b64data.encode(u"latin-1"))
                            ...
                            if binary:
                                return (io.BytesIO(b), )
                            if encoding is None:
                                encoding = XmlUtil.encoding(b, default=u"latin-1")
                            return (io.TextIOWrapper(io.BytesIO(b), encoding=encoding), 
                                    encoding)
                raise ArchiveFileIOError(self, archiveFileName)
            elif archiveFileSource.isXfd:
                for data in archiveFileSource.xfdDocument.iter(tag=u"data"):
                    outfn = data.findtext(u"filename")
                    if outfn == archiveFileName:
                        b64data = data.findtext(u"mimedata")
                        if b64data:
                            b = base64.b64decode(b64data.encode(u"latin-1"))
                            # remove BOM codes if present
                            if len(b) > 3 and b[0] == 239 and b[1] == 187 and b[2] == 191:
                ...
                if filepath.startswith(archiveFileSource.basefile):
                    ...
                    for f in TAXONOMY_PACKAGE_FILE_NAMES:
                        if filepath[l - len(f):l] == f:
                            ...
                            break
        if binary:
            return (openFileStream(self.cntlr, filepath, u'rb'), )
        else:
        ...
        if not self.isOpen:
            ...
            def packageDirsFiles(dir):
                for file in os.listdir(dir):
                    path = dir + u"/" + file   # must have / and not \\ even on windows
    ...
    def select(self, selection):
        ...
        if isHttpUrl(selection) or os.path.isabs(selection):
            self.url = selection
        elif self.baseIsHttp or os.sep == u'/':
            self.url = self.baseurl + u"/" + selection
        else: # MSFT os.sep == '\\'
            self.url = self.baseurl + os.sep + selection.replace(u"/", os.sep)
            
def openFileStream(cntlr, filepath, mode=u'r', encoding=None):
    ...
    if archiveFilenameParts(filepath): # file is in an archive
        return openFileSource(filepath, cntlr).file(filepath, binary=u'b' in mode, encoding=encoding)[0]
    if isHttpUrl(filepath) and cntlr:
        _cacheFilepath = cntlr.webCache.getfilename(filepath)
        if _cacheFilepath is None:
            raise IOError(_(u"Unable to open file: {0}.").format(filepath))
    ...
    if filepath.startswith(SERVER_WEB_CACHE) and cntlr:
        filestream = None
        cacheKey = filepath[len(SERVER_WEB_CACHE) + 1:].replace(u"\\",u"/")
        ...
        if mode.endswith(u't') or encoding:
            contents = filestream.getvalue()
            filestream.close()
            filestream = FileNamedStringIO(filepath, contents.decode(encoding or u'utf-8'))
        return filestream
    # local file system
    elif encoding is None and u'b' not in mode:
        openedFileStream = io.open(filepath, mode=u'rb')
...
def openXmlFileStream(cntlr, filepath, stripDeclaration=False):
    ...
    openedFileStream = openFileStream(cntlr, filepath, u'rb')
    # check encoding
    hdrBytes = openedFileStream.read(512)
    encoding = XmlUtil.encoding(hdrBytes)
    if encoding.lower() in (u'utf-8',u'utf8',u'utf-8-sig') and (cntlr is None or not cntlr.isGAE) and not stripDeclaration:
        text = None
        openedFileStream.close()
    else:
    ...
    if text is None:  # ok to read as utf-8
        return io.open(filepath, u'rt', encoding=u'utf-8'), encoding
    else:
        if stripDeclaration:
...
def saveFile(cntlr, filepath, contents, encoding=None):
    ...
    if filepath.startswith(SERVER_WEB_CACHE):
        cacheKey = filepath[len(SERVER_WEB_CACHE) + 1:].replace(u"\\",u"/")
        if cntlr.isGAE: # check if in memcache
            gaeSet(cacheKey, contents.encode(encoding or u'utf-8'))
    else:
        with io.open(filepath, u'wt', encoding=(encoding or u'utf-8')) as f:
            f.write(contents)
...
def gaeGet(key):
    ...
    if isinstance(chunk_keys, _STR_BASE):
        ...
        for chunk_key in chunk_keys:
            ...
            chunks.append(chunk)
    try:
        return zlib.decompress(''.join(chunks)) # must be bytes join, not unicode
    except zlib.error:
        return None
...
def gaeSet(key, bytesValue): # stores bytes, not string valye
    ...
    chunkKeys = []
    for pos in xrange(0, valueSize, GAE_MEMCACHE_MAX_ITEM_SIZE):
        ...
        # under the same key.
        chunkKey = u'%s%d%d' % (key, pos, random.getrandbits(31))
