...
class xfiFunctionNotAvailable(Exception):
    def __init__(self):
        self.args =  (_(u"xfi function not available"),)
    def __repr__(self):
        return self.args[0]
...
def instance(xc, p, args, i=0):
    ...
    if len(args[i]) != 1: # a sequence of instances isn't acceptable to these classes of functions
        raise XPathContext.FunctionArgType(i+1,u"xbrl:xbrl")
    xbrliXbrl = anytypeArg(xc, args, i, u"xbrli:xbrl")
    if isinstance(xbrliXbrl, ModelObject) and xbrliXbrl.elementQname == XbrlConst.qnXbrliXbrl:
        ...
        return xbrliXbrl
    raise XPathContext.FunctionArgType(i,u"xbrl:xbrl")

def item(xc, args, i=0):
    if len(args[i]) != 1: raise XPathContext.FunctionArgType(i+1,u"xbrl:item")
    modelItem = xc.modelItem(args[i][0])
    if modelItem is not None: 
        return modelItem
    raise XPathContext.FunctionArgType(i,u"xbrl:item")

def xbrlTuple(xc, args, i=0):
    # can't name this just tuple because then it hides tuple() constructor of Python
    if len(args[i]) != 1: raise XPathContext.FunctionArgType(i+1,u"xbrl:tuple")
    modelTuple = args[i][0]
    if isinstance(modelTuple, (ModelFact, ModelInlineFact)) and modelTuple.isTuple:
        return modelTuple
    raise XPathContext.FunctionArgType(i,u"xbrl:tuple")
...
def item_context_element(xc, args, name):
    ...
    raise XPathContext.FunctionArgType(1,u"xbrl:item")
...
def unit(xc, p, args):
    if len(args) != 1: raise XPathContext.FunctionNumArgs()
    if len(args[0]) != 1: raise XPathContext.FunctionArgType(1,u"xbrl:item")
...
def unit_numerator(xc, p, args):
    if len(args) != 1: raise XPathContext.FunctionNumArgs()
    if len(args[0]) != 1: raise XPathContext.FunctionArgType(1,u"xbrl:unit")
    unit = args[0][0]
    if isinstance(unit,ModelObject) and \
       unit.localName == u"unit" and unit.namespaceURI == XbrlConst.xbrli: 
        measuresParent = XmlUtil.descendant(unit, XbrlConst.xbrli, u"unitNumerator")
        if measuresParent is None: measuresParent = unit
        return XmlUtil.descendants(measuresParent, XbrlConst.xbrli, u"measure")
    raise XPathContext.FunctionArgType(1,u"xbrl:unit")
...
def unit_denominator(xc, p, args):
    ...
    if isinstance(unit,ModelObject) and \
       unit.localName == u"unit" and unit.namespaceURI == XbrlConst.xbrli: 
        measuresParent = XmlUtil.descendant(unit, XbrlConst.xbrli, u"unitDenominator")
...
def measure_name(xc, p, args):
    if len(args) != 1: raise XPathContext.FunctionNumArgs()
    if len(args[0]) != 1: raise XPathContext.FunctionArgType(1,u"xbrl:measure")
    unit = args[0][0]
    if isinstance(unit,ModelObject) and \
       unit.localName == u"measure" and unit.namespaceURI == XbrlConst.xbrli:
        return qname(unit, XmlUtil.text(unit)) 
    raise XPathContext.FunctionArgType(1,u"xbrl:unit")

def period(xc, p, args):
    return item_context_element(xc, args, u"period")

def context_period(xc, p, args):
    return parent_child(args, u"context", u"period")

def parent_child(args, parentName, childName, findDescendant=False):
    if len(args) != 1: raise XPathContext.FunctionNumArgs()
    if len(args[0]) != 1: raise XPathContext.FunctionArgType(1,u"xbrl:" + parentName)
    parent = args[0][0]
    if isinstance(parent,ModelObject) and \
       parent.localName == parentName and parent.namespaceURI == XbrlConst.xbrli:
        if childName.startswith(u'@'):
            return parent.get(childName[1:])
        elif childName == u'text()':
            return XmlUtil.textNotStripped(parent)
        elif childName == u'strip-text()':
    ...
    raise XPathContext.FunctionArgType(1,u"xbrl:" + parentName)

def is_start_end_period(xc, p, args):
    return is_period_type(args, u"startDate")

def is_forever_period(xc, p, args):
    return is_period_type(args, u"forever")

def is_duration_period(xc, p, args):
    return is_period_type(args, (u"forever",u"startDate") )

def is_instant_period(xc, p, args):
    return is_period_type(args, u"instant")

def is_period_type(args, periodElement):
    if len(args) != 1: raise XPathContext.FunctionNumArgs()
    if len(args[0]) != 1: raise XPathContext.FunctionArgType(1,u"xbrl:period")
    period = args[0][0]
    if isinstance(period,ModelObject) and \
       period.localName == u"period" and period.namespaceURI == XbrlConst.xbrli:
        return XmlUtil.hasChild(period, XbrlConst.xbrli, periodElement)
    raise XPathContext.FunctionArgType(1,u"xbrl:period")

def period_start(xc, p, args):
    return period_datetime(p, args, (u"startDate",u"instant"))

def period_end(xc, p, args):
    return period_datetime(p, args, (u"endDate",u"instant"))

def period_instant(xc, p, args):
    return period_datetime(p, args, u"instant")
...
def period_datetime(p, args, periodElement):
    ...
    if (isinstance(period,ModelObject) == 1 and 
        ...
        if child is not None:
            addOneDay = child.localName != u"startDate"
            return dateTime( child, addOneDay=addOneDay, type=DATETIME)
        elif periodElement == u"instant":
            raise XPathContext.XPathException(p, u'xfie:PeriodIsNotInstant', _(u'Period is not instant'))
        else:
            raise XPathContext.XPathException(p, u'xfie:PeriodIsForever', _(u'Period is forever'))
    raise XPathContext.FunctionArgType(1,u"xbrl:period")

def entity(xc, p, args):
    return item_context_element(xc, args, u"entity")

def context_entity(xc, p, args):
    return parent_child(args, u"context", u"entity")

def identifier(xc, p, args):
    return item_context_element(xc, args, u"identifier")

def context_identifier(xc, p, args):
    return parent_child(args, u"context", u"identifier", True)

def entity_identifier(xc, p, args):
    return parent_child(args, u"entity", u"identifier")

def identifier_value(xc, p, args):
    return parent_child(args, u"identifier", u"strip-text()")

def identifier_scheme(xc, p, args):
    scheme = parent_child(args, u"identifier", u"@scheme")
    if scheme is None:
        return None
    return anyURI(scheme)

def fact_identifier_value(xc, p, args):
    return XmlUtil.text(item_context_element(xc, args, u"identifier")).strip()

def fact_identifier_scheme(xc, p, args):
    scheme = item_context_element(xc, args, u"identifier").get(u"scheme")
    if scheme is None:
        return None
    return anyURI(scheme)

def segment(xc, p, args):
    seg = item_context_element(xc, args, u"segment")
    if seg is None:
        return () # no segment
    return seg

def entity_segment(xc, p, args):
    seg = parent_child(args, u"entity", u"segment")
    if seg is None:
        return () # no segment
    return seg

def context_segment(xc, p, args):
    seg = parent_child(args, u"context", u"segment", True)
    if seg is None:
        return () # no segment
    return seg

def scenario(xc, p, args):
    scen = item_context_element(xc, args, u"scenario")
    if scen is None:
        return () # no segment
    return scen

def context_scenario(xc, p, args):
    scen = parent_child(args, u"context", u"scenario")
    if scen is None:
        return () # no segment
    return scen

def precision(xc, p, args):
    return infer_precision_decimals(xc, p, args, u"precision")

def decimals(xc, p, args):
    return infer_precision_decimals(xc, p, args, u"decimals")

def infer_precision_decimals(xc, p, args, attrName):
    if len(args) != 1: raise XPathContext.FunctionNumArgs()
    if len(args[0]) != 1: raise XPathContext.FunctionArgType(1,u"xbrl:item",args[0])
    modelItem = xc.modelItem(args[0][0])
    ...
    if modelConcept.isNumeric:
        p = inferredPrecision(modelItem) if attrName == u"precision" else inferredDecimals(modelItem)
        if isinf(p):
            return u'INF'
        if isnan(p):
            raise XPathContext.XPathException(p, u'xfie:ItemIsNotNumeric', _(u'Argument 1 {0} is not inferrable.').format(attrName))
        return p
    raise XPathContext.XPathException(p, u'xfie:ItemIsNotNumeric', _(u'Argument 1 is not reported with {0}.').format(attrName))

def numeric(xc, p, args):
    return conceptProperty(xc, p, args, u"numeric")

def non_numeric(xc, p, args):
    return conceptProperty(xc, p, args, u"non-numeric")

def fraction(xc, p, args):
    return conceptProperty(xc, p, args, u"fraction")
...
def conceptProperty(xc, p, args, property):
    ...
    if qn:
        ...
        if modelConcept is not None:
            if property == u"numeric": return modelConcept.isNumeric or modelConcept.isFraction
            if property == u"non-numeric": return modelConcept.isItem and not (modelConcept.isNumeric or modelConcept.isFraction)
            if property == u"fraction": return modelConcept.isFraction
    return False
...
def checkXffFunctionUse(xc, p, functionName):
    ...
    if xc.progHeader is not None and xc.progHeader.element is not None:
        ...
        if (modelResourceElt is None or
            modelResourceElt.localName not in (u"formula", u"consistencyAssertion", u"valueAssertion", u"precondition", u"message")):
            raise XPathContext.XPathException(p, u'xffe:invalidFunctionUse', _(u'Function xff:uncovered-aspect cannot be used on an XPath expression associated with a {0}').format(xc.progHeader.element.localName))
            
    if xc.variableSet is not None and xc.variableSet.implicitFiltering  == u"false":
        raise XPathContext.XPathException(p, u'xffe:invalidFunctionUse', _(u'Function xff:uncovered-aspect cannot be used with implicitFiltering=false'))
...
def uncovered_aspect(xc, p, args):
    ...
    aspect = aspectFromToken.get(stringArg(xc, args, 0, u"xs:token").strip())
    if aspect == Aspect.DIMENSIONS:
        qn = qnameArg(xc, p, args, 1, u'QName', emptyFallback=None)
        
    checkXffFunctionUse(xc, p, u"uncovered-aspect")
...
def has_fallback_value(xc, p, args):
    ...
    if len(args) != 1: raise XPathContext.FunctionNumArgs()
    variableQname = qnameArg(xc, p, args, 0, u'QName', emptyFallback=None)
        
    checkXffFunctionUse(xc, p, u"has-fallback-value")
...
def uncovered_aspects(xc, p, args, dimensionAspects=False):
    ...
    # check function use after checking argument types
    if xc.progHeader is not None and xc.progHeader.element is not None:
        if xc.progHeader.element.localName not in (u"formula", u"consistencyAssertion", u"valueAssertion", u"message"):
            raise XPathContext.XPathException(p, u'xffe:invalidFunctionUse', _(u'Function xff:uncovered-aspect cannot be used on an XPath expression associated with a {0}').format(xc.progHeader.element.localName))
        if xc.variableSet is not None and xc.variableSet.implicitFiltering  == u"false":
            raise XPathContext.XPathException(p, u'xffe:invalidFunctionUse', _(u'Function xff:uncovered-aspect cannot be used with implicitFiltering=false'))
    ...
    for i, node1 in enumerate(seq1):
        try:
            ...
            if not isinstance(node1, (ModelObject,ModelAttribute)): 
                raise XPathContext.FunctionArgType(1,u"node()*")
            if not isinstance(node2, (ModelObject,ModelAttribute)): 
                raise XPathContext.FunctionArgType(2,u"node()*")
            if mustBeItems:
                if not isinstance(node1, (ModelFact, ModelInlineFact)) or not node1.isItem: 
                    raise XPathContext.FunctionArgType(1,u"xbrl:item*", errCode=nonItemErrCode)
                if not isinstance(node2, (ModelFact, ModelInlineFact)) or not node2.isItem: 
                    raise XPathContext.FunctionArgType(2,u"xbrl:item*", errCode=nonItemErrCode)
...
def setsEqual(xc, args, test, mustBeItems=False):
    ...
    for node1 in seq1:
        ...
        if mustBeItems and (not isinstance(node1, (ModelFact, ModelInlineFact)) or not node1.isItem): 
            raise XPathContext.FunctionArgType(1,u"xbrl:item*", errCode=u"xfie:NodeIsNotXbrlItem")
    for node2 in seq2:
        if not isinstance(node2, ModelObject): 
            raise XPathContext.FunctionArgType(2,u"node()*")
        if mustBeItems and (not isinstance(node2, (ModelFact, ModelInlineFact)) or not node2.isItem): 
            raise XPathContext.FunctionArgType(2,u"xbrl:item*", errCode=u"xfie:NodeIsNotXbrlItem")
...
def s_equal_test(node1, node2):
    if (isinstance(node1, (ModelFact, ModelInlineFact)) and node1.isItem and
        ...
        return (c_equal_test(node1, node2) and u_equal_test(node1, node2) and
                ...
                node1.xAttributes.get(u"precision") == node2.xAttributes.get(u"precision") and
                node1.xAttributes.get(u"decimals") == node2.xAttributes.get(u"decimals"))
...
def u_equal(xc, p, args):
    return nodesEqual(xc, args, u_equal_test, mustBeItems=True, nonItemErrCode=u"xfie:NodeIsNotXbrlItem")
...
def v_equal(xc, p, args):
    return nodesEqual(xc, args, v_equal_test, mustBeItems=True, nonItemErrCode=u"xfie:NodeIsNotXbrlItem")

def v_equal_test(modelItem1, modelItem2):
    return modelItem1.isVEqualTo(modelItem2)

def c_equal(xc, p, args):
    return nodesEqual(xc, args, c_equal_test, mustBeItems=True, nonItemErrCode=u"xfie:NodeIsNotXbrlItem")
...
def p_equal_test(node1, node2):
    if not isinstance(node1, (ModelFact, ModelInlineFact)) or not (node1.isItem or node1.isTuple): 
        raise XPathContext.FunctionArgType(1,u"xbrli:item or xbrli:tuple", errCode=u"xfie:ElementIsNotXbrlConcept")
    if not isinstance(node2, (ModelFact, ModelInlineFact)) or not (node1.isItem or node1.isTuple): 
        raise XPathContext.FunctionArgType(2,u"xbrli:item or xbrli:tuple", errCode=u"xfie:ElementIsNotXbrlConcept")
    return node1.parentElement == node2.parentElement

def cu_equal(xc, p, args):
    return nodesEqual(xc, args, cu_equal_test, mustBeItems=True, nonItemErrCode=u"xfie:NodeIsNotXbrlItem")
...
def pc_equal(xc, p, args):
    return nodesEqual(xc, args, pc_equal_test, mustBeItems=True, nonItemErrCode=u"xfie:NodeIsNotXbrlItem")
...
def pcu_equal(xc, p, args):
    return nodesEqual(xc, args, pcu_equal_test, mustBeItems=True, nonItemErrCode=u"xfie:NodeIsNotXbrlItem")
...
def date_equal_test(xc, p, args, instantEndDate):
    if len(args) != 2: raise XPathContext.FunctionNumArgs()
    date1 = atomicArg(xc, p, args, 0, u"xbrldi:dateUnion", missingArgFallback=(), emptyFallback=None)
    if not isinstance(date1, (DateTime,datetime.date)): 
        raise XPathContext.FunctionArgType(1,u"xbrldi:dateUnion")
    date2 = atomicArg(xc, p, args, 1, u"xbrldi:dateUnion", missingArgFallback=(), emptyFallback=None)
    if not isinstance(date1, (DateTime,datetime.date)): 
        raise XPathContext.FunctionArgType(2,u"xbrldi:dateUnion")
    return dateUnionEqual(date1, date2, instantEndDate)

def nodes_correspond(xc, p, args):
    if len(args) != 2: raise XPathContext.FunctionNumArgs()
    node1 = nodeArg(xc, args, 0, u"node()?", missingArgFallback=(), emptyFallback=())
    node2 = nodeArg(xc, args, 1, u"node()?", missingArgFallback=(), emptyFallback=())
    if node1 == ():
...
def concept(xc, p, args):
    qnConcept = qnameArg(xc, p, args, 0, u'QName', emptyFallback=None)
    ...
    if srcConcept is None or not (srcConcept.isItem or srcConcept.isTuple) or srcConcept.qname is None or srcConcept.qname.namespaceURI == XbrlConst.xbrli: 
        raise XPathContext.XPathException(p, u'xfie:invalidConceptQName', _(u'Argument 1 {0} is not a concept in the DTS.').format(qnConcept))
    return srcConcept

def concept_balance(xc, p, args):
    if len(args) != 1: raise XPathContext.FunctionNumArgs()
    balance = concept(xc,p,args).get(u"{http://www.xbrl.org/2003/instance}balance")
    if balance is None:
        balance = u""
    return balance
...
def concept_period_type(xc, p, args):
    if len(args) != 1: raise XPathContext.FunctionNumArgs()
    return concept(xc,p,args).get(u"{http://www.xbrl.org/2003/instance}periodType")

def concept_custom_attribute(xc, p, args):
    if len(args) != 2: raise XPathContext.FunctionNumArgs()
    qnAttr = qnameArg(xc, p, args, 1, u'QName', emptyFallback=None)
    if qnAttr is None: raise XPathContext.FunctionArgType(2,u"xs:QName")
...
def concept_data_type_derived_from(xc, p, args):
    if len(args) != 2: raise XPathContext.FunctionNumArgs()
    qnType = qnameArg(xc, p, args, 1, u'QName', emptyFallback=None)
    if qnType is None: raise XPathContext.FunctionArgType(2,u"xs:QName")
...
def filter_member_network_selection(xc, p, args):
    ...
    qnDim = qnameArg(xc, p, args, 0, u'QName', emptyFallback=None)
    qnMem = qnameArg(xc, p, args, 1, u'QName', emptyFallback=None)
    linkroleURI = stringArg(xc, args, 2, u"xs:string")
    arcroleURI = stringArg(xc, args, 3, u"xs:string")
    axis = stringArg(xc, args, 4, u"xs:string")
    if not axis in (u'descendant-or-self', u'child-or-self', u'descendant', u'child'):
        return ()
    dimConcept = xc.modelXbrl.qnameConcepts.get(qnDim)
    if dimConcept is None or not dimConcept.isDimensionItem:
        raise XPathContext.XPathException(p, u'xfie:invalidDimensionQName', _(u'Argument 1 {0} is not a dimension concept QName.').format(qnDim))
    ...
    if relationshipSet is not None:
        ...
        linkQnames = set()
        arcQnames = set()
        if axis.endswith(u"-or-self"):
            members.add(qnMem)
        fromRels = relationshipSet.fromModelObject(memConcept)
        if fromRels is not None:
            filter_member_network_members(relationshipSet, fromRels, axis.startswith(u"descendant"), members, linkQnames, arcQnames)
            u''' removed 2011-03-10:
...
def filter_member_DRS_selection(xc, p, args):
    if len(args) != 5: raise XPathContext.FunctionNumArgs()
    qnDim = qnameArg(xc, p, args, 0, u'QName', emptyFallback=None)
    qnPriItem = qnameArg(xc, p, args, 1, u'QName', emptyFallback=None)
    qnMem = qnameArg(xc, p, args, 2, u'QName', emptyFallback=None)
    linkroleURI = stringArg(xc, args, 3, u"xs:string")
    if not linkroleURI:  # '' or ()
        linkroleURI = None  # select all ELRs
    axis = stringArg(xc, args, 4, u"xs:string")
    if not axis in (u'DRS-descendant', u'DRS-child'):
        return ()
    memSelectionQnames = set()
    dimConcept = xc.modelXbrl.qnameConcepts.get(qnDim)
    if dimConcept is None or dimConcept.qname is None or dimConcept.qname.namespaceURI == XbrlConst.xbrli:
        raise XPathContext.XPathException(p, u'xfie:invalidDimensionQName', _(u'Argument 1 {0} is not in the DTS.').format(qnDim))
    ...
    if priItemConcept is None  or priItemConcept.qname is None  or priItemConcept.qname.namespaceURI == XbrlConst.xbrli:
        raise XPathContext.XPathException(p, u'xfie:invalidPrimaryItemConceptQName', _(u'Argument 1 {0} is not in the DTS.').format(qnPriItem))
    memConcept = xc.modelXbrl.qnameConcepts.get(qnMem)
...
def filter_member_DRS_members(xc, fromRels, axis, memConcept, inSelection, visited, memSelectionQnames):
    for rel in fromRels:
        ...
        if toConceptQname not in visited and (not nestedSelection or axis == u"DRS-descendant"):
...
def fact_dim_value(xc, p, args, dimType):
    context = item_context(xc, args)
    qnDim = qnameArg(xc, p, args, 1, u'QName', emptyFallback=None)
    dimConcept = xc.modelXbrl.qnameConcepts.get(qnDim)
    if dimConcept is None or not dimConcept.isDimensionItem:
        raise XPathContext.XPathException(p, 
                                          u'xfie:invalid{0}DimensionQName'.format(dimType), 
                                          _(u'Argument 1 {0} is not a dimension concept QName.').format(qnDim))
    if context is not None:
...
def fact_explicit_dimension_value_value(xc, p, args):
    context = item_context(xc, args)
    if context is not None:
        qn = qnameArg(xc, p, args, 1, u'QName', emptyFallback=())
        if qn == (): raise XPathContext.FunctionArgType(2,u"xbrl:QName")
        ...
        if dimConcept is None or not dimConcept.isExplicitDimension:
            raise XPathContext.XPathException(p, u'xfie:invalidExplicitDimensionQName', _(u'dimension does not specify an explicit dimension'))
        ...
        if isinstance(dimValue, ModelDimensionValue) and dimValue.isExplicit:
            ...
            u''' removed 2011-03-01 FWG clarification that default always applies
...
def fact_has_explicit_dimension_value(xc, p, args):
    ...
    return qnameArg(xc, p, args, 2, u'QName', emptyFallback=()) == fact_explicit_dimension_value_value(xc, p, args)
...
def fact_typed_dimension_value(xc, p, args):
    ...
    if context is not None:
        ...
        if modelConcept is None or not modelConcept.isTypedDimension:
            raise XPathContext.XPathException(p, u'xfie:invalidTypedDimensionQName', _(u'dimension does not specify a typed dimension'))
...
def fact_dimension_s_equal2(xc, p, args):
    ...
    if context1 is not None:
        if context2 is not None:
            qn = qnameArg(xc, p, args, 2, u'QName', emptyFallback=())
            if qn == (): raise XPathContext.FunctionArgType(3,u"xbrl:QName")
            modelConcept = xc.modelXbrl.qnameConcepts.get(qn) # check qname is explicit dimension
        ...
        raise XPathContext.FunctionArgType(2,u"xbrl:item")
    raise XPathContext.FunctionArgType(1,u"xbrl:item")

def linkbase_link_roles(xc, p, args):
    if len(args) > 2: raise XPathContext.FunctionNumArgs()
    inst = instance(xc, p, args, 1)
    arcroleURI = stringArg(xc, args, 0, u"xs:string")
...
def concept_label(xc, p, args):
    ...
    qnSource = qnameArg(xc, p, args, 0, u'QName', emptyFallback=None)
    srcConcept = inst.qnameConcepts.get(qnSource)
    if srcConcept is None:
        return u""
    linkroleURI = stringArg(xc, args, 1, u"xs:string", emptyFallback=u'')
    if not linkroleURI: linkroleURI = XbrlConst.defaultLinkRole
    labelroleURI = stringArg(xc, args, 2, u"xs:string", emptyFallback=u'')
    if not labelroleURI: labelroleURI = XbrlConst.standardLabel
    lang = stringArg(xc, args, 3, u"xs:string", emptyFallback=u'')
...
def arcrole_definition(xc, p, args):
    if len(args) > 2: raise XPathContext.FunctionNumArgs()
    inst = instance(xc, p, args, 1)
    arcroleURI = stringArg(xc, args, 0, u"xs:string", emptyFallback=u'')
...
def role_definition(xc, p, args):
    if len(args) > 2: raise XPathContext.FunctionNumArgs()
    inst = instance(xc, p, args, 1)
    roleURI = stringArg(xc, args, 0, u"xs:string", emptyFallback=u'')
...
def fact_footnotes(xc, p, args):
    ...
    linkroleURI = stringArg(xc, args, 1, u"xs:string", emptyFallback=u'')
    if not linkroleURI: linkroleURI = XbrlConst.defaultLinkRole
    arcroleURI = stringArg(xc, args, 2, u"xs:string", emptyFallback=u'')
    if not arcroleURI: arcroleURI = XbrlConst.factFootnote
    footnoteroleURI = stringArg(xc, args, 3, u"xs:string", emptyFallback=u'')
    if not footnoteroleURI: footnoteroleURI = XbrlConst.footnote
    lang = stringArg(xc, args, 4, u"xs:string", emptyFallback=u'')
...
def concept_relationships(xc, p, args, nestResults=False):
    ...
    qnSource = qnameArg(xc, p, args, 0, u'QName', emptyFallback=None)
    linkroleURI = stringArg(xc, args, 1, u"xs:string")
    if not linkroleURI:
        linkroleURI = XbrlConst.defaultLinkRole
    elif linkroleURI == u"XBRL-all-linkroles":
        linkroleURI = None
    arcroleURI = stringArg(xc, args, 2, u"xs:string")
    axis = stringArg(xc, args, 3, u"xs:string")
    if not axis in (u'descendant', u'child', u'ancestor', u'parent', u'sibling', u'sibling-or-self'):
        return ()
    if qnSource != XbrlConst.qnXfiRoot:
        ...
        if srcConcept is None:
            return ()
    if lenArgs > 4:
        generations = numericArg(xc, p, args, 4, u"xs:integer", convertFallback=0)
    elif axis in (u'child', u'parent', u'sibling', u'sibling-or-self'):
        generations = 1
    else:
        generations = 0
    if axis in (u'child',u'parent',u'sibling', u'sibling-or-self'):
        generations = 1
    if axis == u'child':
        axis = u'descendant'
    elif axis == u'parent':
        axis = u'ancestor'
    if lenArgs > 5:
        qnLink = qnameArg(xc, p, args, 5, u'QName', emptyFallback=None)
    else:
        qnLink = None
    if lenArgs > 5:
        qnArc = qnameArg(xc, p, args, 6, u'QName', emptyFallback=None)
    else:
        qnArc = None
        
    removeSelf = axis == u'sibling'
    relationshipSet = inst.relationshipSet(arcroleURI, linkroleURI, qnLink, qnArc)
    if relationshipSet:
        result = []
        visited = set([qnSource])
        if qnSource == XbrlConst.qnXfiRoot:
            if axis in (u'sibling', u'sibling-or-self', u'ancestor'):
                return []
            roots = relationshipSet.rootConcepts
            visited = set(c.qname for c in roots)
            rels = [rel for c in roots for rel in relationshipSet.fromModelObject(c)]
            if generations == 1:
                return rels
            if generations > 1:
                generations -= 1
        elif axis == u'descendant':
            rels = relationshipSet.fromModelObject(srcConcept)
        elif axis == u'ancestor': # includes first pass on parents of object to get sibling
            rels = relationshipSet.toModelObject(srcConcept)
        elif axis in (u'sibling', u'sibling-or-self'):
            rels = relationshipSet.toModelObject(srcConcept)
            if rels:
...
def concept_relationships_step(xc, inst, relationshipSet, rels, axis, generations, result, visited, nestResults):
    if rels:
        for modelRel in rels:
            concept = modelRel.toModelObject if axis == u'descendant' else modelRel.fromModelObject
            ...
            if generations > 1 or (generations == 0 and conceptQname not in visited):
                ...
                if axis == u'descendant':
                    if relationshipSet.arcrole == u"XBRL-dimensions":
                        stepRelationshipSet = inst.relationshipSet(u"XBRL-dimensions", modelRel.consecutiveLinkrole)
                    else: 
                        stepRelationshipSet = relationshipSet
                    stepRels = stepRelationshipSet.fromModelObject(concept)
                else:
                    if relationshipSet.arcrole == u"XBRL-dimensions":
                        stepRelationshipSet = inst.relationshipSet(u"XBRL-dimensions")
...
def relationship_from_concept(xc, p, args):
    if len(args) != 1: raise XPathContext.FunctionNumArgs()
    modelRel = anytypeArg(xc, args, 0, u"arelle:ModelRelationship", None)
    if modelRel is not None:
        return modelRel.fromModelObject.qname
    raise XPathContext.FunctionArgType(1,u"arelle:modelRelationship")
...
def distinct_nonAbstract_parent_concepts(xc, p, args):
    ...
    linkroleURI = stringArg(xc, args, 0, u"xs:string")
    if not linkroleURI:
        linkroleURI = XbrlConst.defaultLinkRole
    arcroleURI = stringArg(xc, args, 1, u"xs:string")
...
def relationship_element_attribute(xc, p, args, elementParent=False):
    if len(args) != 2: raise XPathContext.FunctionNumArgs()
    modelRel = anytypeArg(xc, args, 0, u"arelle:ModelRelationship", None)
    if modelRel is None: raise XPathContext.FunctionArgType(1,u"arelle:modelRelationship")
    qnAttr = qnameArg(xc, p, args, 1, u'QName', emptyFallback=None)
    if qnAttr is None: raise XPathContext.FunctionArgType(2,u"xs:QName")
...
def  format_number(xc, p, args):
    ...
    try:
        ...
        raise XPathContext.XPathException(p, u'xfie:invalidPictureSyntax', unicode(err) )
...
def  create_element(xc, p, args):
    ...
    if attrArg:
        if (len(attrArg) & 1 or 
            any((not isinstance(arg, (QName, _STR_BASE))) or
                ...
                for i in xrange(0, len(attrArg),2)
                for arg in (attrArg[i],))):
            raise XPathContext.FunctionArgType(1,u"((xs:qname|xs:string),xs:anyAtomicValue)", errCode=u"xfie:AttributesNotNameValuePairs")
        else:
    ...
    value = atomicArg(xc, p, args, 2, u"xs:anyAtomicType", emptyFallback=u'') 
    ...
    if value and childElements:
        raise XPathContext.FunctionArgType(1,unicode(value), errCode=u"xfie:MixedContentError")
    
    # scratchpad instance document emulates fn:doc( ) to hold XML nodes
    scratchpadXmlDocUrl = u"http://www.xbrl.org/2012/function/creation/xml_scratchpad.xml"
    if scratchpadXmlDocUrl in xc.modelXbrl.urlDocs:
        ...
        modelDocument = ModelDocument.create(xc.modelXbrl, 
                                             ...
                                             scratchpadXmlDocUrl,
                                             initialXml=u"<xfc:dummy xmlns:xfc='http://www.xbrl.org/2012/function/creation'/>")
...
xfiFunctions = {
    u'context': context,
    u'unit': unit,
    u'unit-numerator': unit_numerator,
    u'unit-denominator': unit_denominator,
    u'measure-name': measure_name,
    u'period': period,
    u'context-period': context_period,
    u'is-start-end-period': is_start_end_period,
    u'is-forever-period': is_forever_period,
    u'is-duration-period': is_duration_period,
    u'is-instant-period': is_instant_period,
    u'period-start': period_start,
    u'period-end': period_end,
    u'period-instant': period_instant,
    u'entity' : entity,
    u'context-entity' : context_entity,
    u'identifier': identifier,
    u'context-identifier': context_identifier,
    u'entity-identifier': entity_identifier,
    u'identifier-value': identifier_value,
    u'identifier-scheme': identifier_scheme,
    u'segment': segment,
    u'entity-segment': entity_segment,
    u'context-segment': context_segment,
    u'scenario': scenario,
    u'context-scenario': context_scenario,
    u'fact-identifier-value': fact_identifier_value,
    u'fact-identifier-scheme': fact_identifier_scheme,
    u'is-non-numeric' : non_numeric,
    u'is-numeric' : numeric,
    u'is-fraction' : fraction,
    u'precision': precision,
    u'decimals': decimals,
    u'uncovered-aspect' : uncovered_aspect,
    u'has-fallback-value' : has_fallback_value,
    u'uncovered-non-dimensional-aspects' : uncovered_non_dimensional_aspects,
    u'uncovered-dimensional-aspects': uncovered_dimensional_aspects,
    u'identical-nodes': identical_nodes,
    u's-equal': s_equal,
    u'u-equal': u_equal,
    u'v-equal': v_equal,
    u'c-equal': c_equal,
    u'identical-node-set' : identical_node_set,
    u's-equal-set': s_equal_set,
    u'v-equal-set': v_equal_set,
    u'c-equal-set': c_equal_set,
    u'u-equal-set': u_equal_set,
    u'x-equal': x_equal,
    u'duplicate-item': duplicate_item,
    u'duplicate-tuple': duplicate_tuple,
    u'p-equal': p_equal,
    u'cu-equal': cu_equal,
    u'pc-equal': pc_equal,
    u'pcu-equal': pcu_equal,
    u'start-equal': start_equal,
    u'end-equal': end_equal,
    u'nodes-correspond': nodes_correspond,
    u'facts-in-instance': facts_in_instance,
    u'items-in-instance': items_in_instance,
    u'tuples-in-instance': tuples_in_instance,
    u'items-in-tuple': items_in_tuple,
    u'tuples-in-tuple': tuples_in_tuple,
    u'non-nil-facts-in-instance': non_nil_facts_in_instance,
    u'concept-balance': concept_balance,
    u'concept-period-type': concept_period_type,
    u'concept-custom-attribute': concept_custom_attribute,
    u'concept-data-type': concept_data_type,
    u'concept-data-type-derived-from' : concept_data_type_derived_from,
    u'concept-substitutions': concept_substitutions,
    u'filter-member-network-selection' : filter_member_network_selection,
    u'filter-member-DRS-selection' : filter_member_DRS_selection,
    u'fact-segment-remainder': fact_segment_remainder,
    u'fact-scenario-remainder': fact_scenario_remainder,
    u'fact-has-explicit-dimension': fact_has_explicit_dimension,
    u'fact-has-typed-dimension': fact_has_typed_dimension,
    u'fact-has-explicit-dimension-value': fact_has_explicit_dimension_value,
    u'fact-explicit-dimension-value': fact_explicit_dimension_value,
    u'fact-typed-dimension-value': fact_typed_dimension_value,
    u'fact-explicit-dimensions': fact_explicit_dimensions,
    u'fact-typed-dimensions': fact_typed_dimensions,
    u'fact-dimension-s-equal2': fact_dimension_s_equal2,
    u'linkbase-link-roles': linkbase_link_roles,
    u'concept-label': concept_label,
    u'arcrole-definition': arcrole_definition,
    u'role-definition': role_definition,
    u'fact-footnotes': fact_footnotes,
    u'concept-relationships': concept_relationships,
    u'relationship-from-concept': relationship_from_concept,
    u'relationship-to-concept': relationship_to_concept,
    u'distinct-nonAbstract-parent-concepts': distinct_nonAbstract_parent_concepts,
    u'relationship-attribute': relationship_attribute,
    u'relationship-link-attribute': relationship_link_attribute,
    u'relationship-name': relationship_name,
    u'relationship-link-name': relationship_link_name,
    u'xbrl-instance': xbrl_instance,
    u'format-number':  format_number,
    u'create-element': create_element,
     }
