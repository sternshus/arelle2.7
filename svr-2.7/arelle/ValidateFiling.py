...
class ValidateFiling(ValidateXbrl.ValidateXbrl):
    def __init__(self, modelXbrl):
        ...
        if datePattern is None:
            datePattern = re.compile(ur"([12][0-9]{3})-([01][0-9])-([0-3][0-9])")
            GFMcontextDatePattern = re.compile(ur"^[12][0-9]{3}-[01][0-9]-[0-3][0-9]$")
            # note \u20zc = euro, \u00a3 = pound, \u00a5 = yen
            signOrCurrencyPattern = re.compile(u"^(-)[0-9]+|[^eE](-)[0-9]+|(\\()[0-9].*(\\))|([$\u20ac\u00a3\00a5])")
            instanceFileNamePattern = re.compile(ur"^(\w+)-([12][0-9]{3}[01][0-9][0-3][0-9]).xml$")
            linkroleDefinitionStatementSheet = re.compile(ur"[^-]+-\s+Statement\s+-\s+.*", # no restriction to type of statement
                                                          re.IGNORECASE)
            efmCIKpattern = re.compile(ur"^[0-9]{10}$")
    ...
    def validate(self, modelXbrl, parameters=None):
        ...
        if modelXbrl.modelManager.disclosureSystem.SBRNL:
            for pluginXbrlMethod in pluginClassMethods(u"Validate.SBRNL.Start"):
                pluginXbrlMethod(self, modelXbrl)
            self.qnSbrLinkroleorder = ModelValue.qname(u"http://www.nltaxonomie.nl/5.0/basis/sbr/xbrl/xbrl-syntax-extension",u"linkroleOrder")
        ...
        modelXbrl.modelManager.showStatus(_(u"validating {0}").format(disclosureSystem.name))
        ...
        self.modelXbrl.profileActivity(u"... cache lbl, pre, ref relationships", minTimeToShow=1.0)
        
        validateInlineXbrlGFM = (modelXbrl.modelDocument.type == ModelDocument.Type.INLINEXBRL and
                                 self.validateGFM)
        validateEFMpragmatic = disclosureSystem.names and u"efm-pragmatic" in disclosureSystem.names
        self.validateLoggingSemantic = validateLoggingSemantic = (
              modelXbrl.isLoggingEffectiveFor(level=u"WARNING-SEMANTIC") or 
              modelXbrl.isLoggingEffectiveFor(level=u"ERROR-SEMANTIC"))
        
        if self.validateEFM:
            for pluginXbrlMethod in pluginClassMethods(u"Validate.EFM.Start"):
        ...
        if modelXbrl.modelDocument.type == ModelDocument.Type.INSTANCE or \
           modelXbrl.modelDocument.type == ModelDocument.Type.INLINEXBRL:
            ...
            if self.validateEFM and self.parameters:
                p = self.parameters.get(ModelValue.qname(u"CIK",noPrefixIsNoNamespace=True))
                if p and len(p) == 2 and p[1] not in (u"null", u"None"):
                    paramFilerIdentifier = p[1]
                p = self.parameters.get(ModelValue.qname(u"cikList",noPrefixIsNoNamespace=True))
                if p and len(p) == 2:
                    paramFilerIdentifiers = p[1].split(u",")
                p = self.parameters.get(ModelValue.qname(u"cikNameList",noPrefixIsNoNamespace=True))
                if p and len(p) == 2:
                    paramFilerNames = p[1].split(u"|Edgar|")
                    if paramFilerIdentifiers and len(paramFilerIdentifiers) != len(paramFilerNames):
                        self.modelXbrl.error((u"EFM.6.05.24.parameters", u"GFM.3.02.02"),
                            _(u"parameters for cikList and cikNameList different list entry counts: %(cikList)s, %(cikNameList)s"),
                            modelXbrl=modelXbrl, cikList=paramFilerIdentifiers, cikNameList=paramFilerNames)
                p = self.parameters.get(ModelValue.qname(u"submissionType",noPrefixIsNoNamespace=True))
                if p and len(p) == 2:
                    submissionType = p[1]
                p = self.parameters.get(ModelValue.qname(u"exhibitType",noPrefixIsNoNamespace=True))
                if p and len(p) == 2:
            ...
            if m:
                ...
                if not self.fileNameBasePart:
                    modelXbrl.error((self.EFM60303, u"GFM.1.01.01"),
                        _(u'Invalid instance document base name part (ticker or mnemonic name) in "{base}-{yyyymmdd}.xml": %(filename)s'),
                        modelObject=modelXbrl.modelDocument, filename=modelXbrl.modelDocument.basename,
                        messageCodes=(u"EFM.6.03.03", u"EFM.6.23.01", u"GFM.1.01.01"))
                else:
                    try:
                        self.fileNameDate = datetime.datetime.strptime(self.fileNameDatePart,u"%Y%m%d").date()
                    except ValueError:
                        modelXbrl.error((self.EFM60303, u"GFM.1.01.01"),
                            _(u'Invalid instance document base name part (date) in "{base}-{yyyymmdd}.xml": %(filename)s'),
                            modelObject=modelXbrl.modelDocument, filename=modelXbrl.modelDocument.basename,
                            messageCodes=(u"EFM.6.03.03", u"EFM.6.23.01", u"GFM.1.01.01"))
            else:
                modelXbrl.error((self.EFM60303, u"GFM.1.01.01"),
                    _(u'Invalid instance document name, must match "{base}-{yyyymmdd}.xml": %(filename)s'),
                    modelObject=modelXbrl.modelDocument, filename=modelXbrl.modelDocument.basename,
                    messageCodes=(u"EFM.6.03.03", u"EFM.6.23.01", u"GFM.1.01.01"))
            ...
            if disclosureSystem.identifierValueName:   # omit if no checks
                for entityIdentifierElt in xbrlInstDoc.iterdescendants(u"{http://www.xbrl.org/2003/instance}identifier"):
                    if isinstance(entityIdentifierElt,ModelObject):
                        schemeAttr = entityIdentifierElt.get(u"scheme")
                        ...
                        if not disclosureSystem.identifierSchemePattern.match(schemeAttr):
                            try:
                                ...
                                contextId = u"not available"
                            modelXbrl.error((u"EFM.6.05.01", u"GFM.1.02.01"),
                                _(u"Invalid entity identifier scheme %(scheme)s in context %(context)s for identifier %(identifier)s"),
                        ...
                        if not disclosureSystem.identifierValuePattern.match(entityIdentifier):
                            modelXbrl.error((u"EFM.6.05.02", u"GFM.1.02.02"),
                                _(u"Invalid entity identifier %(entityIdentifierName)s: %(entityIdentifer)s"),
                        ...
                        if not entityIdentifierValue:
                            ...
                            if self.validateEFM and not efmCIKpattern.match(entityIdentifierValue):
                                self.modelXbrl.error(u"EFM.6.05.23.cikValue",
                                    _(u"EntityIdentifier %(entityIdentifer)s must be 10 digits."),
                                    modelObject=entityIdentifierElt, entityIdentifer=entityIdentifierValue)
                        elif entityIdentifier != entityIdentifierValue:
                            modelXbrl.error((u"EFM.6.05.03", u"GFM.1.02.03"),
                                _(u"Multiple %(entityIdentifierName)ss: %(entityIdentifer)s, %(entityIdentifer2)s"),
                                ...
                                filerIdentifier=u",".join(paramFilerIdentifiers or [])) 
                self.modelXbrl.profileActivity(u"... filer identifier checks", minTimeToShow=1.0)
            ...
            for context in contexts:
                ...
                if h in uniqueContextHashes:
                    if context.isEqualTo(uniqueContextHashes[h]):
                        modelXbrl.error((u"EFM.6.05.07", u"GFM.1.02.07"),
                            _(u"Context ID %(context)s is equivalent to context ID %(context2)s"),
                ...
                if self.validateGFM:
                    for dateElt in XmlUtil.children(context, XbrlConst.xbrli, (u"startDate", u"endDate", u"instant")):
                        dateText = XmlUtil.text(dateElt)
                        if not GFMcontextDatePattern.match(dateText):
                            modelXbrl.error(u"GFM.1.02.25",
                                _(u"Context id %(context)s %(elementName)s invalid content %(value)s"),
                                modelObject=dateElt, context=contextID, 
                                elementName=dateElt.prefixedName, value=dateText)
                #6.5.4 scenario
                hasSegment = XmlUtil.hasChild(context, XbrlConst.xbrli, u"segment")
                hasScenario = XmlUtil.hasChild(context, XbrlConst.xbrli, u"scenario")
                notAllowed = None
                if disclosureSystem.contextElement == u"segment" and hasScenario:
                    notAllowed = _(u"Scenario")
                elif disclosureSystem.contextElement == u"scenario" and hasSegment:
                    notAllowed = _(u"Segment")
                elif disclosureSystem.contextElement == u"either" and hasSegment and hasScenario:
                    notAllowed = _(u"Both segment and scenario")
                elif disclosureSystem.contextElement == u"none" and (hasSegment or hasScenario):
                    notAllowed = _(u"Neither segment nor scenario")
                if notAllowed:
                    if validateEFMpragmatic:
                        contextsWithDisallowedOCEs.append(context)
                    else:
                        modelXbrl.error((u"EFM.6.05.04", u"GFM.1.02.04", u"SBR.NL.2.3.5.06"),
                            _(u"%(elementName)s element not allowed in context Id: %(context)s"),
                            modelObject=context, elementName=notAllowed, context=contextID, count=1)
        
                #6.5.5 segment only explicit dimensions
                for contextName in {u"segment": (u"{http://www.xbrl.org/2003/instance}segment",),
                                    u"scenario": (u"{http://www.xbrl.org/2003/instance}scenario",),
                                    u"either": (u"{http://www.xbrl.org/2003/instance}segment",u"{http://www.xbrl.org/2003/instance}scenario"),
                                    u"both": (u"{http://www.xbrl.org/2003/instance}segment",u"{http://www.xbrl.org/2003/instance}scenario"),
                                    u"none": [], None:[]
                                    }[disclosureSystem.contextElement]:
                    for segScenElt in context.iterdescendants(contextName):
                        if isinstance(segScenElt,ModelObject):
                            childTags = u", ".join([child.prefixedName for child in segScenElt.iterchildren()
                                                   if isinstance(child,ModelObject) and 
                                                   child.tag != u"{http://xbrl.org/2006/xbrldi}explicitMember"])
                            if len(childTags) > 0:
                                if validateEFMpragmatic:
                                    contextsWithDisallowedOCEcontent.append(context)
                                else:
                                    modelXbrl.error((u"EFM.6.05.05", u"GFM.1.02.05"),
                                                    _(u"%(elementName)s of context Id %(context)s has disallowed content: %(content)s"),
                                                    modelObject=context, context=contextID, content=childTags, 
                                                    elementName=contextName.partition(u"}")[2].title())
                #6.5.38 period forever
                if context.isForeverPeriod:
                    self.modelXbrl.error(u"EFM.6.05.38",
                        _(u"Context %(contextID)s has a forever period."),
            ...
            if validateEFMpragmatic: # output combined count message
                if contextsWithDisallowedOCEs:
                    modelXbrl.error((u"EFM.6.05.04", u"GFM.1.02.04"),
                        _(u"%(count)s contexts contain disallowed %(elementName)s: %(context)s"),
                        modelObject=contextsWithDisallowedOCEs, elementName=notAllowed, 
                        count=len(contextsWithDisallowedOCEs), context=u', '.join(c.id for c in contextsWithDisallowedOCEs))
                if contextsWithDisallowedOCEcontent:
                    modelXbrl.error((u"EFM.6.05.05", u"GFM.1.02.05"),
                        _(u"%(count)s contexts contain disallowed %(elementName)s content: %(context)s"),
                        modelObject=contextsWithDisallowedOCEcontent, elementName=disclosureSystem.contextElement, 
                        count=len(contextsWithDisallowedOCEcontent), context=u', '.join(c.id for c in contextsWithDisallowedOCEcontent))
            del uniqueContextHashes, contextsWithDisallowedOCEs, contextsWithDisallowedOCEcontent
            self.modelXbrl.profileActivity(u"... filer context checks", minTimeToShow=1.0)
            ...
            deiCheckLocalNames = set([
                u"EntityRegistrantName", 
                u"EntityCommonStockSharesOutstanding",
                u"EntityCurrentReportingStatus", 
                u"EntityVoluntaryFilers", 
                disclosureSystem.deiCurrentFiscalYearEndDateElement, 
                u"EntityFilerCategory", 
                u"EntityWellKnownSeasonedIssuer", 
                u"EntityPublicFloat", 
                disclosureSystem.deiDocumentFiscalYearFocusElement, 
                u"DocumentFiscalPeriodFocus",
                u"EntityReportingCurrencyISOCode",])
            ...
            for f in modelXbrl.facts:
                ...
                if context is not None: # tests do not apply to tuples
                    if not context.hasSegment and not context.hasScenario: 
                        ...
                        if factInDeiNamespace:
                            ...
                            if factElementName == disclosureSystem.deiAmendmentFlagElement:
                                amendmentFlag = value
                                amendmentFlagFact = f
                            elif factElementName == u"AmendmentDescription":
                            ...
                            elif factElementName == u"DocumentType":
                                documentType = value
                                documentTypeFact = f
                                ...
                                if entityIdentifierValue != value:
                                    self.modelXbrl.error((u"EFM.6.05.23", u"GFM.3.02.02"),
                                        _(u"dei:%(elementName)s %(value)s must match the context entity identifier %(entityIdentifier)s"),
                                ...
                                if paramFilerIdentifiers and value not in paramFilerIdentifiers:
                                    self.modelXbrl.error((u"EFM.6.05.23.submissionIdentifier", u"GFM.3.02.02"),
                                        _(u"dei:%(elementName)s %(value)s must match submission: %(filerIdentifier)s"),
                                        modelObject=f, elementName=disclosureSystem.deiFilerIdentifierElement,
                                        value=value, filerIdentifier=u",".join(paramFilerIdentifiers))
                                ...
                                if paramFilerIdentifiers and paramFilerNames and entityIdentifierValue in paramFilerIdentifiers:
                                    ...
                                    if not value.lower().startswith(prefix.lower()):
                                        self.modelXbrl.error((u"EFM.6.05.24", u"GFM.3.02.02"),
                                            _(u"dei:%(elementName)s %(prefix)s should be a case-insensitive prefix of: %(value)s"),
                        ...
                        # segment present
                        isEntityCommonStockSharesOutstanding = factElementName == u"EntityCommonStockSharesOutstanding"
                        ...
                        for dimValue in context.qnameDims.values():
                            if dimValue.isExplicit:
                                ...
                                if (isEntityCommonStockSharesOutstanding and
                                    ...
                                    dimConcept.name == u"StatementClassOfStockAxis"):
                                    commonSharesItemsByStockClass[memConcept.qname].append(f)
                                    u''' per discussion with Dean R, remove use of LB defined members from this test
                    ...
                    if self.validateEFM: # note that this is in the "if context is not None" region
                        for pluginXbrlMethod in pluginClassMethods(u"Validate.EFM.Fact"):
                            pluginXbrlMethod(self, f)
                #6.5.17 facts with precision
                concept = f.concept
                if concept is None:
                    modelXbrl.error((u"EFM.6.04.03", u"GFM.2.01.01"),
                        _(u"Fact %(fact)s of context %(contextID)s has an XBRL error"),
                    ...
                    if concept.isNumeric:
                        if f.precision:
                            modelXbrl.error((u"EFM.6.05.17", u"GFM.1.02.16"),
                    ...
                    if self.validateEFM and concept.type is not None and concept.type.isDomainItemType:
                        modelXbrl.error(u"EFM.6.05.25",
                            _(u"Domain item %(fact)s in context %(contextID)s may not appear as a fact"),
                            modelObject=f, fact=f.qname, contextID=factContextID)
                    
                if validateInlineXbrlGFM:
                    if f.localName == u"nonFraction" or f.localName == u"fraction":
                        syms = signOrCurrencyPattern.findall(f.text)
                        if syms:
                            modelXbrl.error((u"EFM.N/A", u"GFM.1.10.18"),
                                u'ix-numeric Fact %(fact)s of context %(contextID)s has a sign or currency symbol "%(value)s" in "%(text)s"',
                                modelObject=f, fact=f.qname, contextID=factContextID, 
                                value=u"".join(s for t in syms for s in t), text=f.text)
                            
            self.entityRegistrantName = deiItems.get(u"EntityRegistrantName") # used for name check in 6.8.6
                            
            self.modelXbrl.profileActivity(u"... filer fact checks", minTimeToShow=1.0)
    
            if len(contextIDs) > 0: # check if contextID is on any undefined facts
                for undefinedFact in modelXbrl.undefinedFacts:
                    contextIDs.discard(undefinedFact.get(u"contextRef"))
                if len(contextIDs) > 0:
                    modelXbrl.error((u"EFM.6.05.08", u"GFM.1.02.08"),
                                    _(u"The instance document contained a context(s) %(contextIDs)s that was(are) not used in any fact."),
                                    modelXbrl=modelXbrl, contextIDs=u", ".join(contextIDs))
    
            #6.5.9, .10 start-end durations
            if disclosureSystem.GFM or \
               disclosureSystemVersion[0] >= 27 or \
               documentType in set([
                        u'20-F', u'40-F', u'10-Q', u'10-QT', u'10-K', u'10-KT', u'10', u'N-CSR', u'N-CSRS', u'N-Q',
                        u'20-F/A', u'40-F/A', u'10-Q/A', u'10-QT/A', u'10-K/A', u'10-KT/A', u'10/A', u'N-CSR/A', u'N-CSRS/A', u'N-Q/A']):
                u'''
                for c1 in contexts:
                    ...
                    if end is not None:
                        if cntx.isStartEndPeriod:
                            ...
                            if probCntxs:
                                ...
                                startEndCntxsByEnd[end] |= set([cntx])
                                probCntxs.clear()
                        if self.validateEFM and cntx.isInstantPeriod:
                            ...
                            if probCntxs:
                                probInstantCntxsByEnd[end] |= ( probCntxs | set([cntx]) )
                                probCntxs.clear()
                del probCntxs
                for end, probCntxs in probStartEndCntxsByEnd.items():
                    endCntxs = startEndCntxsByEnd[end]
                    modelXbrl.error((u"EFM.6.05.09", u"GFM.1.2.9"),
                        _(u"Context endDate %(endDate)s, and startDate(s) have a duration of one day, for end context(s): %(endContexts)s and start context(s): %(startContexts)s; that is inconsistent with document type %(documentType)s."),
                        modelObject=probCntxs, endDate=XmlUtil.dateunionValue(end, subtractOneDay=True), 
                        endContexts=u', '.join(sorted(c.id for c in endCntxs)),
                        startContexts=u', '.join(sorted(c.id for c in probCntxs)), 
                        documentType=documentType)
                if disclosureSystemVersion[0] < 27:
                    for end, probCntxs in probInstantCntxsByEnd.items():
                        modelXbrl.error(u"EFM.6.05.10",
                            _(u"Context instant date %(endDate)s startDate has a duration of one day,with end (instant) of context(s): %(contexts)s; that is inconsistent with document type %(documentType)s."),
                            modelObject=probCntxs, endDate=XmlUtil.dateunionValue(end, subtractOneDay=True), 
                            contexts=u', '.join(sorted(c.id for c in probCntxs)), 
                            documentType=documentType)
                del probStartEndCntxsByEnd, startEndCntxsByEnd, probInstantCntxsByEnd
                del durationCntxStartDatetimes
                self.modelXbrl.profileActivity(u"... filer instant-duration checks", minTimeToShow=1.0)
            ...
            if not foundRequiredContext:
                modelXbrl.error((u"EFM.6.05.19", u"GFM.1.02.18"),
                    _(u"Required context (no segment) not found for document type %(documentType)s."),
            ...
            for unit in self.modelXbrl.units.values():
                ...
                if h in uniqueUnitHashes:
                    if unit.isEqualTo(uniqueUnitHashes[h]):
                        modelXbrl.error((u"EFM.6.05.11", u"GFM.1.02.10"),
                            _(u"Units %(unitID)s and %(unitID2)s are equivalent."),
                            modelObject=(unit, uniqueUnitHashes[h]), unitID=unit.id, unitID2=uniqueUnitHashes[h].id)
                else:
                    uniqueUnitHashes[h] = unit
                if self.validateEFM:  # 6.5.38
                    for measureElt in unit.iterdescendants(tag=u"{http://www.xbrl.org/2003/instance}measure"):
                        if isinstance(measureElt.xValue, ModelValue.QName) and len(measureElt.xValue.localName) > 65:
                            l = len(measureElt.xValue.localName.encode(u"utf-8"))
                            if l > 200:
                                modelXbrl.error(u"EFM.6.05.36",
                                    _(u"Unit has a measure  with localName length (%(length)s) over 200 bytes long in utf-8, %(measure)s."),
                                    modelObject=measureElt, unitID=unit.id, measure=measureElt.xValue.localName, length=l)
            del uniqueUnitHashes
            self.modelXbrl.profileActivity(u"... filer unit checks", minTimeToShow=1.0)
            ...
            for f1 in modelXbrl.facts:
                ...
                if not f1.isNil:
                    langTestKey = u"{0},{1},{2}".format(f1.qname, f1.contextID, f1.unitID)
                    ...
                    if f1.isNumeric and f1.decimals and f1.decimals != u"INF" and not f1.isNil and getattr(f1,u"xValid", 0) == 4:
                        try:
                            ...
                            if insignificance: # if not None, returns (truncatedDigits, insiginficantDigits)
                                modelXbrl.error((u"EFM.6.05.37", u"GFM.1.02.26"),
                                    _(u"Fact %(fact)s of context %(contextID)s decimals %(decimals)s value %(value)s has nonzero digits in insignificant portion %(insignificantDigits)s."),
                            ...
                            modelXbrl.error((u"EFM.6.05.37", u"GFM.1.02.26"),
                                _(u"Fact %(fact)s of context %(contextID)s decimals %(decimals)s value %(value)s causes Value Error exception."),
                ...
                if h in factForConceptContextUnitLangHash:
                    ...
                    if f1.qname == f2.qname and \
                       ...
                       f1.xmlLang == f2.xmlLang:
                        modelXbrl.error((u"EFM.6.05.12", u"GFM.1.02.11"),
                            u"Facts %(fact)s of context %(contextID)s and %(contextID2)s are equivalent.",
            ...
            for keyNotDefaultLang, factNotDefaultLang in keysNotDefaultLang.items():
                ...
                if not anyDefaultLangFact:
                    self.modelXbrl.error((u"EFM.6.05.14", u"GFM.1.02.13"),
                        _(u"Fact %(fact)s of context %(contextID)s has text of xml:lang '%(lang)s' without corresponding %(lang2)s text"),
            ...
            if not labelsRelationshipSet:
                self.modelXbrl.error((u"EFM.6.10.01.missingLabelLinkbase", u"GFM.1.05.01"),
                    _(u"A label linkbase is required but was not found"), 
                    modelXbrl=modelXbrl)
            ...
            if self.validateEFMorGFM:
                ...
                if amendmentFlag is None:
                    modelXbrl.log(u"WARNING" if validateEFMpragmatic else u"ERROR",
                                  (u"EFM.6.05.20.missingAmendmentFlag", u"GFM.3.02.01"),
                        _(u"%(elementName)s is not found in the default context"),
                        modelXbrl=modelXbrl, elementName=disclosureSystem.deiAmendmentFlagElement)
        
                if not documentPeriodEndDate:
                    modelXbrl.error((u"EFM.6.05.20.missingDocumentPeriodEndDate", u"GFM.3.02.01"),
                        _(u"%(elementName)s is required and was not found in the default context"),
                    ...
                    if not dateMatch or dateMatch.lastindex != 3:
                        modelXbrl.error((u"EFM.6.05.20", u"GFM.3.02.01"),
                            _(u"%(elementName)s is in the default context is incorrect '%(date)s'"),
                            modelXbrl=modelXbrl, elementName=disclosureSystem.deiDocumentPeriodEndDateElement,
                            date=documentPeriodEndDate)
            self.modelXbrl.profileActivity(u"... filer label and text checks", minTimeToShow=1.0)
    
            if self.validateEFM:
                if amendmentFlag == u"true" and amendmentDescription is None:
                    modelXbrl.log(u"WARNING" if validateEFMpragmatic else u"ERROR",
                                  u"EFM.6.05.20.missingAmendmentDescription",
                        _(u"AmendmentFlag is true in context %(contextID)s so AmendmentDescription is also required"),
                        modelObject=amendmentFlagFact, contextID=amendmentFlagFact.contextID if amendmentFlagFact is not None else u"unknown")
        
                if amendmentDescription is not None and amendmentFlag != u"true":
                    modelXbrl.log(u"WARNING" if validateEFMpragmatic else u"ERROR",
                                  u"EFM.6.05.20.extraneous",
                        _(u"AmendmentDescription can not be provided when AmendmentFlag is not true in context %(contextID)s"),
                        modelObject=amendmentDescriptionFact, contextID=amendmentDescriptionFact.contextID)
                    
                if documentType is None:
                    modelXbrl.error(u"EFM.6.05.20.missingDocumentType",
                        _(u"DocumentType is required and was not found in the default context"), 
                        modelXbrl=modelXbrl)
                elif documentType not in set([
                                            u"497",
                                            u"10-12B",
                                            u"10-12B/A",
                                            u"10-12G",
                                            ...
                                            u"SP 15D2",
                                            u"SP 15D2/A"]):
                    modelXbrl.error(u"EFM.6.05.20.documentTypeValue",
                        _(u"DocumentType '%(documentType)s' of context %(contextID)s was not recognized"),
                        modelObject=documentTypeFact, contextID=documentTypeFact.contextID, documentType=documentType)
                elif submissionType:
                    expectedDocumentTypes = { 
                                            u"10-12B": (u"10-12B", u"Other"),
                                            u"10-12B/A": (u"10-12B/A", u"Other"),
                                            u"10-12G": (u"10-12G", u"Other"),
                                            u"10-12G/A": (u"10-12G/A", u"Other"),
                                            u"10-K": (u"10-K",),
                                            u"10-K/A": (u"10-K", u"10-K/A"),
                                            u"10-KT": (u"10-K",u"10-KT",u"Other"),
                                            u"10-KT/A": (u"10-K", u"10-KT", u"10-KT/A", u"Other"),
                                            u"10-Q": (u"10-Q",),
                                            u"10-Q/A": (u"10-Q", u"10-Q/A"),
                                            u"10-QT": (u"10-Q", u"10-QT", u"Other"),
                                            u"10-QT/A": (u"10-Q", u"10-QT", u"10-QT/A", u"Other"),
                                            u"20-F": (u"20-F",),
                                            u"20-F/A": (u"20-F", u"20-F/A"),
                                            u"20FR12B": (u"20FR12B", u"Other"),
                                            u"20FR12B/A": (u"20FR12B/A", u"Other"),
                                            u"20FR12G": (u"20FR12G", u"Other"),
                                            u"20FR12G/A": (u"20FR12G/A", u"Other"),
                                            u"40-F": (u"40-F",),
                                            u"40-F/A": (u"40-F", u"40-F/A"),
                                            u"40FR12B": (u"40FR12B", u"Other"),
                                            u"40FR12B/A": (u"40FR12B/A", u"Other"),
                                            u"40FR12G": (u"40FR12G", u"Other"),
                                            u"40FR12G/A": (u"40FR12G/A", u"Other"),
                                            u"485BPOS": (u"485BPOS",),
                                            u"497": (u"497", u"Other"),
                                            u"6-K": (u"6-K",),
                                            u"6-K/A": (u"6-K", u"6-K/A"),
                                            u"8-K": (u"8-K",),
                                            u"8-K/A": (u"8-K", u"8-K/A"),
                                            u"8-K12B": (u"8-K12B", u"Other"),
                                            u"8-K12B/A": (u"8-K12B/A", u"Other"),
                                            u"8-K12G3": (u"8-K12G3", u"Other"),
                                            u"8-K12G3/A": (u"8-K12G3/A", u"Other"),
                                            u"8-K15D5": (u"8-K15D5", u"Other"),
                                            u"8-K15D5/A": (u"8-K15D5/A", u"Other"),
                                            u"F-1": (u"F-1",),
                                            u"F-1/A": (u"F-1", u"F-1/A"),
                                            u"F-10": (u"F-10",),
                                            u"F-10/A": (u"F-10", u"F-10/A"),
                                            u"F-10EF": (u"F-10EF", u"Other"),
                                            u"F-10POS": (u"F-10POS", u"Other"),
                                            u"F-1MEF": (u"F-1MEF",),
                                            u"F-3": (u"F-3",),
                                            u"F-3/A": (u"F-3", u"F-3/A"),
                                            u"F-3ASR": (u"F-3", u"F-3ASR"),
                                            u"F-3D": (u"F-3", u"F-3D"),
                                            u"F-3DPOS": (u"F-3", u"F-3DPOS"),
                                            u"F-3MEF": (u"F-3MEF",),
                                            u"F-4": (u"F-4",),
                                            u"F-4 POS": (u"F-4", u"F-4 POS"),
                                            u"F-4/A": (u"F-4", u"F-4/A"),
                                            u"F-4EF": (u"F-4", u"F-4EF"),
                                            u"F-4MEF": (u"F-4MEF",),
                                            u"F-9": (u"F-9",),
                                            u"F-9 POS": (u"F-9", u"F-9 POS"),
                                            u"F-9/A": (u"F-9", u"F-9/A"),
                                            u"F-9EF": (u"F-9", u"F-9EF"),
                                            u"N-1A": (u"N-1A",),
                                            u"N-1A/A": (u"N-1A/A", u"Other"),
                                            u"N-CSR": (u"N-CSR",),
                                            u"N-CSR/A": (u"N-CSR/A",),
                                            u"N-CSRS": (u"N-CSRS",),
                                            u"N-CSRS/A": (u"N-CSRS/A",),
                                            u"N-Q": (u"N-Q",),
                                            u"N-Q/A": (u"N-Q/A",),
                                            u"POS AM": (u"F-1", u"F-3", u"F-4", u"F-6", u"Other", 
                                                       u"POS AM", u"S-1", u"S-11", u"S-20", u"S-3", u"S-4", u"S-B"),
                                            u"POS EX": (u"F-3", u"F-4", u"Other", 
                                                       u"POS EX", u"S-1", u"S-3", u"S-4"),
                                            u"POS462B": (u"F-1MEF", u"F-3MEF", u"F-4MEF", u"Other", 
                                                        u"POS462B", u"POS462C", u"S-11MEF", u"S-1MEF", u"S-3MEF", u"S-BMEF"),
                                            u"POSASR": (u"F-3", u"Other", u"POSASR", u"S-3"),
                                            u"S-1": (u"S-1",),
                                            u"S-1/A": (u"S-1", u"S-1/A"),
                                            u"S-11": (u"S-11",),
                                            u"S-11/A": (u"S-11/A",),
                                            u"S-11MEF": (u"S-11MEF",),
                                            u"S-1MEF": (u"S-1MEF",),
                                            u"S-3": (u"S-3",),
                                            u"S-3/A": (u"S-3", u"S-3/A"),
                                            u"S-3ASR": (u"S-3", u"S-3ASR"),
                                            u"S-3D": (u"S-3", u"S-3D"),
                                            u"S-3DPOS": (u"S-3", u"S-3DPOS"),
                                            u"S-3MEF": (u"S-3MEF",),
                                            u"S-4": (u"S-4",),
                                            u"S-4 POS": (u"S-4", u"S-4 POS"),
                                            u"S-4/A": (u"S-4", u"S-4/A"),
                                            u"S-4EF": (u"S-4", u"S-4EF"),
                                            u"S-4MEF": (u"S-4MEF",),
                                            u"SD": (u"SD",),
                                            u"SD/A": (u"SD/A",),
                                            u"SP 15D2": (u"SP 15D2",),
                                            u"SP 15D2/A": (u"SP 15D2/A",)
                            }.get(submissionType)
                    if expectedDocumentTypes and documentType not in expectedDocumentTypes:
                        modelXbrl.error(u"EFM.6.05.20.submissionDocumentType" if self.exhibitType != u"EX-2.01" else u"EFM.6.23.03",
                            _(u"DocumentType '%(documentType)s' of context %(contextID)s inapplicable to submission form %(submissionType)s"),
                            ...
                            messageCodes=(u"EFM.6.05.20.submissionDocumentType", u"EFM.6.23.03"))
                if self.exhibitType:
                    if (documentType in (u"SD", u"SD/A")) != (self.exhibitType == u"EX-2.01"):
                        modelXbrl.error({u"EX-100":u"EFM.6.23.04",
                                         u"EX-101":u"EFM.6.23.04",
                                         u"EX-2.01":u"EFM.6.23.05"}.get(self.exhibitType,u"EX-101"),
                            _(u"The value for dei:DocumentType, %(documentType)s, is not allowed for %(exhibitType)s attachments."),
                            modelObject=documentTypeFact, contextID=documentTypeFact.contextID, documentType=documentType, exhibitType=self.exhibitType,
                            messageCodes=(u"EFM.6.23.04", u"EFM.6.23.04", u"EFM.6.23.05"))
                    
                # 6.5.21
                for doctypesRequired, deiItemsRequired in (
                      ((u"10-K", u"10-KT", u"10-Q", u"10-QT", u"20-F", u"40-F",
                        u"10-K/A", u"10-KT/A", u"10-Q/A", u"10-QT/A", u"20-F/A", u"40-F/A",
                        u"6-K", u"NCSR", u"N-CSR", u"N-CSRS", u"N-Q",
                        u"6-K/A", u"NCSR/A", u"N-CSR/A", u"N-CSRS/A", u"N-Q/A",
                        u"10", u"S-1", u"S-3", u"S-4", u"S-11", u"POS AM",
                        u"10/A", u"S-1/A", u"S-3/A", u"S-4/A", u"S-11/A", 
                        u"8-K", u"F-1", u"F-3", u"F-10", u"497", u"485BPOS",
                        u"8-K/A", u"F-1/A", u"F-3/A", u"F-10/A", 
                        u"Other"),
                        (u"EntityRegistrantName", u"EntityCentralIndexKey")),
                      ((u"10-K", u"10-KT", u"20-F", u"40-F",
                        u"10-K/A", u"10-KT/A", u"20-F/A", u"40-F/A"),
                       (u"EntityCurrentReportingStatus",)),
                     ((u"10-K", u"10-KT", u"10-K/A", u"10-KT/A",),
                      (u"EntityVoluntaryFilers", u"EntityPublicFloat")),
                      ((u"10-K", u"10-KT", u"10-Q", u"10-QT", u"20-F", u"40-F",
                        u"10-K/A", u"10-KT/A", u"10-Q/A", u"10-QT/A", u"20-F/A", u"40-F/A",
                        u"6-K", u"NCSR", u"N-CSR", u"N-CSRS", u"N-Q",
                        u"6-K/A", u"NCSR/A", u"N-CSR/A", u"N-CSRS/A", u"N-Q/A"),
                        (u"CurrentFiscalYearEndDate", u"DocumentFiscalYearFocus", u"DocumentFiscalPeriodFocus")),
                      ((u"10-K", u"10-KT", u"10-Q", u"10-QT", u"20-F",
                        u"10-K/A", u"10-KT/A", u"10-Q/A", u"10-QT/A", u"20-F/A",
                        u"10", u"S-1", u"S-3", u"S-4", u"S-11", u"POS AM",
                        u"10/A", u"S-1/A", u"S-3/A", u"S-4/A", u"S-11/A"),
                        (u"EntityFilerCategory",)),
                       ((u"10-K", u"10-KT", u"20-F", u"10-K/A", u"10-KT/A", u"20-F/A"),
                         (u"EntityWellKnownSeasonedIssuer",)),
                       ((u"SD", u"SD/A"),
                         (u"EntityReportingCurrencyISOCode", ))
                ):
                    if documentType in doctypesRequired:
                        for deiItem in deiItemsRequired:
                            if deiItem not in deiItems or not deiItems[deiItem]: #must exist and value must be non-empty (incl not nil)
                                modelXbrl.log((u"WARNING" if validateEFMpragmatic and deiItem in set([
                                                 u"CurrentFiscalYearEndDate", u"DocumentFiscalPeriodFocus", u"DocumentFiscalYearFocus",
                                                 u"EntityCurrentReportingStatus", u"EntityFilerCategory", u"EntityPublicFloat", 
                                                 u"EntityVoluntaryFilers", u"EntityWellKnownSeasonedIssuer"]) else u"ERROR"), 
                                              (u"EFM.6.05.21.{0}".format(deiItem) if validateEFMpragmatic and deiItem in set([
                                                 u"CurrentFiscalYearEndDate", u"DocumentFiscalPeriodFocus", u"DocumentFiscalYearFocus",
                                                 u"EntityRegistrantName", u"EntityCentralIndexKey",
                                                 u"EntityCurrentReportingStatus", u"EntityFilerCategory", u"EntityPublicFloat", 
                                                 u"EntityVoluntaryFilers", u"EntityWellKnownSeasonedIssuer"]) else u"EFM.6.23.36" if deiItem == u"EntityReportingCurrencyISOCode"
                                                  else u"EFM.6.05.21"),
                                                _(u"dei:%(elementName)s is required for DocumentType '%(documentType)s' of context %(contextID)s"),
                        modelObject=documentTypeFact, contextID=documentTypeFact.contextID, documentType=documentType,
                        elementName=deiItem,
                        messageCodes=(u"EFM.6.05.21.CurrentFiscalYearEndDate", u"EFM.6.05.21.DocumentFiscalPeriodFocus", u"EFM.6.05.21.DocumentFiscalYearFocus",
                                      u"EFM.6.05.21.EntityRegistrantName", u"EFM.6.05.21.EntityCentralIndexKey",
                                      u"EFM.6.05.21.EntityCurrentReportingStatus", u"EFM.6.05.21.EntityFilerCategory", u"EFM.6.05.21.EntityPublicFloat", 
                                      u"EFM.6.05.21.EntityVoluntaryFilers", u"EFM.6.05.21.EntityWellKnownSeasonedIssuer",
                                      u"EFM.6.23.36", u"EFM.6.05.21"))
                                
                if documentType in set([u"10-K", u"10-KT", u"10-Q", u"10-QT", u"20-F", u"40-F",
                                    u"10-K/A", u"10-KT/A", u"10-Q/A", u"10-QT/A", u"20-F/A", u"40-F/A"]):
                    defaultContextSharesOutstandingValue = deiItems.get(u"EntityCommonStockSharesOutstanding")
                    errLevel = u"WARNING" if validateEFMpragmatic else u"ERROR"
                    if commonSharesClassMembers:
                        if defaultContextSharesOutstandingValue: # checks that it exists and is not empty or nil
                            modelXbrl.log(errLevel, u"EFM.6.05.26",
                                _(u"dei:EntityCommonStockSharesOutstanding is required for DocumentType '%(documentType)s' but not in the default context because there are multiple classes of common shares"),
                                modelObject=documentTypeFact, contextID=documentTypeFact.contextID, documentType=documentType)
                        elif len(commonSharesClassMembers) == 1: # and not hasDefinedStockAxis:
                            modelXbrl.log(errLevel, u"EFM.6.05.26",
                                _(u"dei:EntityCommonStockSharesOutstanding is required for DocumentType '%(documentType)s' but but a default-context because only one class of stock"),
                                modelObject=documentTypeFact, documentType=documentType)
                        u''' per Dean R, this test no longer makes sense because we don't check against def LB defined members
                        ...
                        for mem, facts in commonSharesItemsByStockClass.items():
                            if len(facts) != 1:
                                modelXbrl.log(errLevel, u"EFM.6.05.26",
                                    _(u"dei:EntityCommonStockSharesOutstanding is required for DocumentType '%(documentType)s' but only one per stock class %(stockClass)s"),
                                    modelObject=documentTypeFact, documentType=documentType, stockClass=mem)
                            u''' removed per ARELLE-124 (should check measurement date vs report date)
                            ...
                            modelXbrl.log(errLevel, u"EFM.6.05.26",
                                _(u"dei:EntityCommonStockSharesOutstanding is required for DocumentType '%(documentType)s' but missing for a non-default-context fact"),
                                modelObject=documentTypeFact, documentType=documentType)
                    elif not defaultContextSharesOutstandingValue: # missing, empty, or nil
                        modelXbrl.log(errLevel, u"EFM.6.05.26",
                            _(u"dei:EntityCommonStockSharesOutstanding is required for DocumentType '%(documentType)s' in the default context because there are not multiple classes of common shares"),
                            modelObject=documentTypeFact, documentType=documentType)
                if documentType in (u"SD", u"SD/A"): # SD documentType
                    self.modelXbrl.profileActivity(u"... filer required facts checks (other than SD)", minTimeToShow=1.0)
                    rxdNs = None # find RXD schema
                    rxdDoc = None
                    hasRxdPre = hasRxdDef = False
                    for rxdLoc in disclosureSystem.familyHrefs[u"RXD"]:
                        rxdUri = rxdLoc.href
                        if rxdUri in modelXbrl.urlDocs:
                            if rxdUri.endswith(u".xsd") and rxdLoc.elements == u"1":
                                if rxdNs is None:
                                    rxdDoc = modelXbrl.urlDocs[rxdUri]
                                    rxdNs = rxdDoc.targetNamespace
                                else:
                                    modelXbrl.error(u"EFM.6.23.10",
                                        _(u"The DTS of must use only one version of the RXD schema"),
                                        modelObject=(rxdDoc, modelXbrl.urlDocs[rxdUri]), instance=instanceName)
                            elif u"/rxd-pre-" in rxdUri:
                                hasRxdPre = True
                            elif u"/rxd-def-" in rxdUri:
                                hasRxdDef = True
                    if not hasRxdPre:
                        modelXbrl.error(u"EFM.6.23.08",
                            _(u"The DTS must use a standard presentation linkbase from Family RXD in edgartaxonomies.xml."),
                            modelObject=modelXbrl, instance=instanceName)
                    if not hasRxdDef:
                        modelXbrl.error(u"EFM.6.23.09",
                            _(u"The DTS must use a standard definition linkbase from Family RXD in edgartaxonomies.xml."),
                            modelObject=modelXbrl, instance=instanceName)
                    countryNs = None
                    deiNS = None
                    for url, doc in modelXbrl.urlDocs.items():
                        if doc.type == ModelDocument.Type.SCHEMA:
                            if url.startswith(u"http://xbrl.sec.gov/country/"):
                                if countryNs is None:
                                    countryNs = doc.targetNamespace
                                else:
                                    modelXbrl.error(u"EFM.6.23.11",
                                        _(u"The DTS must use must use only one version of the COUNTRY schema."),
                                        modelObject=(doc
                                                     for url,doc in modelXbrl.urlDocs.items()
                                                     if url.startswith(u"http://xbrl.sec.gov/country/")), instance=instanceName)
                            if disclosureSystem.deiNamespacePattern.match(doc.targetNamespace):
                                deiNS = doc.targetNamespace

                    if rxdNs:
                        qn = ModelValue.qname(rxdNs, u"AmendmentNumber")
                        if amendmentFlag == u"true" and (
                            ...
                            modelXbrl.error(u"EFM.6.23.06",
                                _(u"The value for dei:DocumentType, %(documentType)s, requires a value for rxd:AmendmentNumber in the Required Context."),
                                modelObject=modelXbrl, documentType=documentType)
                    else:
                        modelXbrl.error(u"EFM.6.23.07",
                            _(u"The DTS must use a standard schema from Family RXD in edgartaxonomies.xml."),
                            modelObject=modelXbrl, instance=instanceName)
                    class Rxd(): # fake class of rxd qnames based on discovered rxd namespace
                        def __init__(self):
                            for name in (u"CountryAxis", u"GovernmentAxis", u"PaymentTypeAxis", u"ProjectAxis",u"PmtAxis",
                                        u"AllGovernmentsMember", u"AllProjectsMember",u"BusinessSegmentAxis", u"EntityDomain", 
                                        u"A", u"Cm", u"Co", u"Cu", u"D", u"Gv", u"E", u"K", u"Km", u"P", u"Payments", u"Pr", u"Sm"):
                                setattr(self, name, ModelValue.qname(rxdNs, u"rxd:" + name))
                    ...
                    if f1 is not None and documentPeriodEndDateFact is not None and f1.xValid and documentPeriodEndDateFact.xValid:
                        ...
                        if f1.xValue.month != d.month or f1.xValue.day != d.day:
                            modelXbrl.error(u"EFM.6.23.26",
                                _(u"The dei:CurrentFiscalYearEndDate, %(fyEndDate)s does not match the dei:DocumentReportingPeriod %(reportingPeriod)s"),
                    ...
                    if (documentPeriodEndDateFact is not None and documentPeriodEndDateFact.xValid and
                        not any(f2.xValue == documentPeriodEndDateFact.xValue
                                ...
                                if f2.xValid)):
                        modelXbrl.error(u"EFM.6.23.27",
                            _(u"The dei:DocumentPeriodEndDate %(reportingPeriod)s has no corresponding rxd:D fact."),
                    ...
                    for url,doc in modelXbrl.urlDocs.items():
                        if (url not in disclosureSystem.standardTaxonomiesDict and
                            ...
                            for concept in XmlUtil.children(doc.xmlRootElement, XbrlConst.xsd, u"element"):
                                name = concept.name
                                if not concept.isAbstract and not concept.isTextBlock:
                                    modelXbrl.error(u"EFM.6.23.12",
                                        _(u"Extension concept %(concept)s is non-abstract and not a Text Block."),
                                        modelObject=concept, schemaName=doc.basename, name=concept.name, concept=concept.qname)
                                elif name.endswith(u"Table") or name.endswith(u"Axis") or name.endswith(u"Domain"):
                                    modelXbrl.error(u"EFM.6.23.13",
                                        _(u"Extension concept %(concept)s is not allowed in an extension schema."),
                                        modelObject=concept, schemaName=doc.basename, name=concept.name, concept=concept.qname)
                    self.modelXbrl.profileActivity(u"... SD checks 6-13, 26-27", minTimeToShow=1.0)
                    ...
                    for rel in dimDomRelSet.modelRelationships:
                        if (isinstance(rel.fromModelObject, ModelConcept) and isinstance(rel.toModelObject, ModelConcept) and 
                            not dimDefRelSet.isRelated(rel.fromModelObject, u"child", rel.toModelObject)):
                            modelXbrl.error(u"EFM.6.23.14",
                                _(u"The target of the dimension-domain relationship in role %(linkrole)s from %(source)s to %(target)s must be the default member of %(source)s."),
                            ...
                            for rel in dimDomRelSet.toModelObject(elt):
                                if rel.consecutiveLinkrole == ELR:
                                    ...
                                    if isinstance(dim, ModelConcept) and isinstance(mem, ModelConcept):
                                        if dim.qname == rxd.PaymentTypeAxis and not mem.modelDocument.targetNamespace.startswith(u"http://xbrl.sec.gov/rxd/"):
                                            modelXbrl.error(u"EFM.6.23.17",
                                                _(u"The member %(member)s in dimension rxd:PaymentTypeAxis in linkrole %(linkrole)s must be a QName with namespace that begins with \"http://xbrl.sec.gov/rxd/\". "),
                                                modelObject=(rel, memRel, dim, mem), member=mem.qname, linkrole=rel.linkrole)
                                        if dim.qname == rxd.CountryAxis and not mem.modelDocument.targetNamespace.startswith(u"http://xbrl.sec.gov/country/"):
                                            modelXbrl.error(u"EFM.6.23.18",
                                                _(u"The member %(member)s in dimension rxd:CountryAxis in linkrole %(linkrole)s must be a QName with namespace that begins with \"http://xbrl.sec.gov/country//\". "),
                                                modelObject=(rel, memRel, dim, mem), member=mem.qname, linkrole=rel.linkrole)
                            ...
                            for rel in hasHypRelSet.toModelObject(elt):
                                if rel.consecutiveLinkrole == ELR and isinstance(rel.fromModelObject, ModelConcept):
                                    ...
                                    if (mem,linkrole) not in memDim:
                                        ...
                                        modelXbrl.error(u"EFM.6.23.16",
                                            _(u"The member %(member)s has two dimensions, %(dimension1)s in linkrole %(linkrole1)s and  %(dimension2)s in linkrole %(linkrole2)s. "),
                        ...
                        if isinstance(rel.fromModelObject, ModelConcept) and isinstance(rel.toModelObject, ModelConcept):
                            for rel2 in modelXbrl.relationshipSet(XbrlConst.domainMember, rel.consecutiveLinkrole).fromModelObject(rel.toModelObject):
                                if rel2.fromModelObject is not None and rel2.toModelObject is not None:
                                    modelXbrl.error(u"EFM.6.23.15",
                                        _(u"The domain-member relationship in %(linkrole)s from %(source)s to %(target)s is consecutive with domain-member relationship in %(linkrole2)s to %(target2)s. "),
                                        modelObject=(rel, rel.fromModelObject, rel.toModelObject), 
                    ...
                    self.modelXbrl.profileActivity(u"... SD checks 14-18", minTimeToShow=1.0)
                    qnDeiEntityDomain = ModelValue.qname(deiNS, u"dei:EntityDomain")
                    for relSet, dom, priItem, errCode in ((domMemRelSet, rxd.AllProjectsMember, rxd.Pr, u"EFM.6.23.30"),
                                                          (domMemRelSet, rxd.AllGovernmentsMember, rxd.Gv, u"EFM.6.23.31"),
                                                          (dimDomRelSet, rxd.BusinessSegmentAxis, rxd.Sm, u"EFM.6.23.33"),
                                                          (domMemRelSet, qnDeiEntityDomain, rxd.E, u"EFM.6.23.34")):
                        for f in modelXbrl.factsByQname[priItem]:
                            if (not f.isNil and f.xValid and
                                not relSet.isRelated(dom, u"descendant", f.xValue, isDRS=True)):
                                modelXbrl.error(errCode,
                                    _(u"The %(fact)s %(value)s in context %(context)s is not a %(domain)s."),
                                    modelObject=f, fact=priItem, value=f.xValue, context=f.context.id, domain=dom,
                                    messageCodes=(u"EFM.6.23.30", u"EFM.6.23.31", u"EFM.6.23.33", u"EFM.6.23.34"))
                    self.modelXbrl.profileActivity(u"... SD checks 30, 31, 33, 34", minTimeToShow=1.0)
                    cntxEqualFacts = defaultdict(list)
                    for f in modelXbrl.facts:
                        if f.context is not None:
                            cntxEqualFacts[f.context.contextDimAwareHash].append(f)
                    self.modelXbrl.profileActivity(u"... SD prepare facts by context", minTimeToShow=1.0)
                    
                    qnCurrencyMeasure = XbrlConst.qnIsoCurrency(deiItems.get(u"EntityReportingCurrencyISOCode"))
                    currencyMeasures = ([qnCurrencyMeasure],[])
                    qnAllCountriesDomain = ModelValue.qname(countryNs, u"country:AllCountriesDomain")
                    for cntxFacts in cntxEqualFacts.values():
                        ...
                        for dim, priItem, errCode in ((rxd.PmtAxis, rxd.P, u"EFM.6.23.20"),
                                                      (rxd.GovernmentAxis, rxd.Payments, u"EFM.6.23.22")):
                            if context.hasDimension(dim) and (priItem not in qnameFacts or qnameFacts[priItem].isNil): 
                                modelXbrl.error(errCode,
                                    _(u"The Context %(context)s has dimension %(dimension)s member %(member)s but is missing required fact %(fact)s"),
                                    modelObject=context, context=context.id, dimension=dim, member=context.dimMemberQname(dim), fact=priItem,
                                    messageCodes=(u"EFM.6.23.20", u"EFM.6.23.22"))
                        if (rxd.Co in qnameFacts and not qnameFacts[rxd.Co].isNil and
                            not domMemRelSet.isRelated(qnAllCountriesDomain, u"descendant", qnameFacts[rxd.Co].xValue, isDRS=True)):
                            modelXbrl.error(u"EFM.6.23.44",
                                _(u"Fact rxd:Co value %(value)s in context %(context)s is not in the domain of country:AllCountriesDomain"),
                                modelObject=f, context=context.id, value=qnameFacts[rxd.Co].value)
                        # required present facts based on other present fact
                        for qnF, fNilOk, qnG, gNilOk, errCode in ((rxd.A, True, rxd.Cu, False, u"EFM.6.23.24"),
                                                                  (rxd.A, True, rxd.D, False, u"EFM.6.23.25"),
                                                                  (rxd.A, False, rxd.Gv, False, u"EFM.6.23.28"),
                                                                  (rxd.A, False, rxd.Co, False, u"EFM.6.23.29"),
                                                                  (rxd.Km, False, rxd.K, False, u"EFM.6.23.35"),
                                                                  (rxd.K, False, rxd.Km, False, u"EFM.6.23.35"),
                                                                  (rxd.Cm, False, rxd.Cu, False, u"EFM.6.23.39"),
                                                                  (rxd.K, False, rxd.A, False, u"EFM.6.23.42"),
                                                                  (rxd.Pr, False, rxd.A, False, u"EFM.6.23.43")):
                            if (qnF in qnameFacts and (fNilOk or not qnameFacts[qnF].isNil) and
                                ...
                                modelXbrl.error(errCode,
                                    _(u"The Context %(context)s has a %(fact1)s and is missing required %(fact2NotNil)sfact %(fact2)s"),
                                    modelObject=qnameFacts[qnF], context=context.id, fact1=qnF, fact2=qnG, fact2NotNil=u"" if gNilOk else u"non-nil ",
                                    messageCodes=(u"EFM.6.23.24", u"EFM.6.23.25", u"EFM.6.23.28", u"EFM.6.23.29", u"EFM.6.23.35",
                                                  u"EFM.6.23.35", u"EFM.6.23.39", u"EFM.6.23.42", u"EFM.6.23.43"))
                        for f in cntxFacts:
                            if (not context.hasDimension(rxd.PmtAxis) and f.isNumeric and 
                                f.unit is not None and f.unit.measures != currencyMeasures):
                                modelXbrl.error(u"EFM.6.23.37",
                                    _(u"Fact %(fact)s in context %(context)s has unit %(unit)s not matching dei:EntityReportingCurrencyISOCode %(currency)s"),
                        ...
                        if (rxd.A in qnameFacts and not qnameFacts[rxd.A].isNil and
                            ...
                            modelXbrl.error(u"EFM.6.23.38",
                                _(u"A value cannot be given for rxd:Cm in context %(context)s because the payment is in the reporting currency %(currency)s."),
                        ...
                        if (rxd.A in qnameFacts and 
                            ...
                            modelXbrl.error(u"EFM.6.23.41",
                                _(u"The unit %(unit)s of rxd:A in context %(context)s is not consistent with the value %(currency)s of rxd:Cu."),
                        ...
                        if (context.hasDimension(rxd.ProjectAxis) and
                            not any(f.xValue == m
                                    ...
                                    if f.context is not None)):
                            modelXbrl.error(u"EFM.6.23.19",
                                _(u"The Context %(context)s has dimension %(dimension)s but is missing any payment."),
                        ...
                        if (context.hasDimension(rxd.GovernmentAxis) and
                            not any(f.xValue == m and f.context.hasDimension(rxd.PmtAxis)
                                    ...
                                    if f.context is not None)):
                            modelXbrl.error(u"EFM.6.23.21",
                                _(u"The Context %(context)s has dimension %(dimension)s member %(member)s but is missing any payment."),
                        ...
                        if rxd.P in qnameFacts and not any(f.context is not None and not f.context.hasSegment
                            ...
                            modelXbrl.error(u"EFM.6.23.23",
                                _(u"The Context %(context)s has payment type %(paymentType)s but is missing a corresponding fact in the required context."),
                        ...
                        if not context.hasDimension(rxd.PmtAxis) and rxd.A in qnameFacts and not qnameFacts[rxd.A].isNil:
                            modelXbrl.error(u"EFM.6.23.40",
                                _(u"There is a non-nil rxd:A in context %(context)s but missing a dimension rxd:PmtAxis."),
                                modelObject=(context, qnameFacts[rxd.A]), context=context.id)
                    self.modelXbrl.profileActivity(u"... SD by context for 19-25, 28-29, 35, 37-39, 40-44", minTimeToShow=1.0)
                    for f in modelXbrl.factsByQname[rxd.D]:
                        if not f.isNil and f.xValid and f.xValue + datetime.timedelta(1) != f.context.endDatetime: # date needs to be midnite to compare to datetime
                            modelXbrl.error(u"EFM.6.23.32",
                                _(u"The rxd:D %(value)s in context %(context)s does not match the context end date %(endDate)s."),
                    ...
                    self.modelXbrl.profileActivity(u"... SD checks 32 (last SD check)", minTimeToShow=1.0)
                ...
                for deiItem in (
                    ...
                    if deiItem not in deiItems or deiItems[deiItem] == u"":
                        modelXbrl.error(u"GFM.3.02.01",
                            _(u"dei:%(elementName)s is required in the default context"),
                            modelXbrl=modelXbrl, elementName=deiItem)
            if documentType not in (u"SD", u"SD/A"):
                self.modelXbrl.profileActivity(u"... filer required facts checks", minTimeToShow=1.0)
    
            #6.5.27 footnote elements, etc
            footnoteLinkNbr = 0
            for footnoteLinkElt in xbrlInstDoc.iterdescendants(tag=u"{http://www.xbrl.org/2003/linkbase}footnoteLink"):
                if isinstance(footnoteLinkElt,ModelObject):
                    footnoteLinkNbr += 1
                    
                    linkrole = footnoteLinkElt.get(u"{http://www.w3.org/1999/xlink}role")
                    if linkrole != XbrlConst.defaultLinkRole:
                        modelXbrl.error((u"EFM.6.05.28.linkrole", u"GFM.1.02.20"),
                            _(u"FootnoteLink %(footnoteLinkNumber)s has disallowed role %(linkrole)s"),
                    ...
                    relationshipSet = modelXbrl.relationshipSet(u"XBRL-footnotes", linkrole)
                    ...
                    for child in footnoteLinkElt:
                        if isinstance(child,ModelObject):
                            xlinkType = child.get(u"{http://www.w3.org/1999/xlink}type")
                            if child.namespaceURI != XbrlConst.link or \
                               xlinkType not in (u"locator", u"resource", u"arc") or \
                               child.localName not in (u"loc", u"footnote", u"footnoteArc"):
                                    modelXbrl.error((u"EFM.6.05.27", u"GFM.1.02.19"),
                                        _(u"FootnoteLink %(footnoteLinkNumber)s has disallowed child element %(elementName)s"),
                                        modelObject=child, footnoteLinkNumber=footnoteLinkNbr, elementName=child.prefixedName)
                            elif xlinkType == u"locator":
                                locNbr += 1
                                locrole = child.get(u"{http://www.w3.org/1999/xlink}role")
                                if locrole is not None and (disclosureSystem.GFM or \
                                                            not disclosureSystem.uriAuthorityValid(locrole)): 
                                    modelXbrl.error((u"EFM.6.05.29", u"GFM.1.02.21"),
                                        _(u"FootnoteLink %(footnoteLinkNumber)s loc %(locNumber)s has disallowed role %(role)s"),
                                        modelObject=child, footnoteLinkNumber=footnoteLinkNbr, 
                                        xlinkLabel=child.xlinkLabel,
                                        locNumber=locNbr, role=locrole)
                                href = child.get(u"{http://www.w3.org/1999/xlink}href")
                                if not href.startswith(u"#"): 
                                    modelXbrl.error((u"EFM.6.05.32", u"GFM.1.02.23"),
                                        _(u"FootnoteLink %(footnoteLinkNumber)s loc %(locNumber)s has disallowed href %(locHref)s"),
                                        modelObject=child, footnoteLinkNumber=footnoteLinkNbr, locNumber=locNbr, locHref=href,
                                        locLabel=child.get(u"{http://www.w3.org/1999/xlink}label"))
                                else:
                                    label = child.get(u"{http://www.w3.org/1999/xlink}label")
                            elif xlinkType == u"arc":
                                arcNbr += 1
                                arcrole = child.get(u"{http://www.w3.org/1999/xlink}arcrole")
                                if (self.validateEFM and not disclosureSystem.uriAuthorityValid(arcrole)) or \
                                   (disclosureSystem.GFM  and arcrole != XbrlConst.factFootnote and arcrole != XbrlConst.factExplanatoryFact): 
                                    modelXbrl.error((u"EFM.6.05.30", u"GFM.1.02.22"),
                                        _(u"FootnoteLink %(footnoteLinkNumber)s arc %(arcNumber)s has disallowed arcrole %(arcrole)s"),
                                        modelObject=child, footnoteLinkNumber=footnoteLinkNbr, arcNumber=arcNbr, 
                                        arcToLabel=child.get(u"{http://www.w3.org/1999/xlink}to"),
                                        arcrole=arcrole)
                            elif xlinkType == u"resource": # footnote
                                footnoterole = child.get(u"{http://www.w3.org/1999/xlink}role")
                                if footnoterole == u"":
                                    modelXbrl.error((u"EFM.6.05.28.missingRole", u"GFM.1.2.20"),
                                        _(u"Footnote %(xlinkLabel)s is missing a role"),
                                        modelObject=child, xlinkLabel=child.get(u"{http://www.w3.org/1999/xlink}label"))
                                    ...
                                    modelXbrl.error((u"EFM.6.05.28", u"GFM.1.2.20"),
                                        _(u"Footnote %(xlinkLabel)s has disallowed role %(role)s"),
                                        modelObject=child, xlinkLabel=child.get(u"{http://www.w3.org/1999/xlink}label"),
                                        role=footnoterole)
                                if self.validateEFM:
                                    ...
                                    if relationshipSet:
                                        for relationship in relationshipSet.toModelObject(child):
                                            if isinstance(relationship.fromModelObject, ModelFact):
                                                ...
                                                break
                                    if not foundFact:
                                        modelXbrl.error((u"EFM.6.05.33", u"GFM.1.02.24"),
                                            _(u"FootnoteLink %(footnoteLinkNumber)s footnote %(footnoteLabel)s has no linked fact"),
                                            modelObject=child, footnoteLinkNumber=footnoteLinkNbr, 
                                            footnoteLabel=child.get(u"{http://www.w3.org/1999/xlink}label"),
                                            text=XmlUtil.text(child)[:100])
            self.modelXbrl.profileActivity(u"... filer rfootnotes checks", minTimeToShow=1.0)
        ...
        # entry point schema checks
            ...
            if self.validateSBRNL:
                ...
                if not any(hrefElt.localName == u"linkbaseRef" and hrefElt.get(u"{http://www.w3.org/1999/xlink}role") == u"http://www.xbrl.org/2003/role/presentationLinkbaseRef"
                           for hrefElt, hrefDoc, hrefId in modelXbrl.modelDocument.hrefObjects):
                    modelXbrl.error(u"SBR.NL.2.2.10.01",
                        u'Entrypoint schema must have a presentation linkbase', modelObject=modelXbrl.modelDocument)
        ...
        for concept in modelXbrl.qnameConcepts.values():
            ...
            for modelLabelRel in labelsRelationshipSet.fromModelObject(concept):
                ...
                if role == XbrlConst.documentationLabel:
                    if concept.modelDocument.targetNamespace in disclosureSystem.standardTaxonomiesDict:
                        modelXbrl.error((u"EFM.6.10.05", u"GFM.1.05.05", u"SBR.NL.2.1.0.08"),
                            _(u"Concept %(concept)s of a standard taxonomy cannot have a documentation label: %(text)s"),
                    ...
                    if role == XbrlConst.standardLabel:
                        if text in defaultLangStandardLabels:
                            ...
                            modelXbrl.error((u"EFM.6.10.04", u"GFM.1.05.04"),
                                _(u"Same labels for concepts %(concept)s and %(concept2)s for %(lang)s standard role: %(text)s."),
                    ...
                    if len(text) > 511:
                        modelXbrl.error((u"EFM.6.10.06", u"GFM.1.05.06"),
                            _(u"Label for concept %(concept)s role %(role)s length %(length)s must be shorter than 511 characters: %(text)s"),
                    ...
                    if match:
                        modelXbrl.error((u"EFM.6.10.06", u"GFM.1.05.07", u"SBR.NL.2.3.8.07"),
                            u'Label for concept %(concept)s role %(role)s has disallowed characters: "%(text)s"',
                ...
                if (text is not None and len(text) > 0 and 
                   ...
                   (modelXbrl.modelManager.disclosureSystem.labelTrimPattern.match(text[0]) or \
                    ...
                    modelXbrl.error((u"EFM.6.10.08", u"GFM.1.05.08"),
                        _(u"Label for concept %(concept)s role %(role)s lang %(lang)s is not trimmed: %(text)s"),
            ...
            for modelRefRel in referencesRelationshipSetWithProhibits.fromModelObject(concept):
                ...
                if concept.modelDocument.targetNamespace not in disclosureSystem.standardTaxonomiesDict:
                    modelXbrl.error((u"EFM.6.18.01", u"GFM.1.9.1"),
                        _(u"References for extension concept %(concept)s are not allowed: %(text)s"),
                    ...
                    modelXbrl.error((u"EFM.6.18.02", u"SBR.NL.2.1.0.08"),
                        _(u"References for standard taxonomy concept %(concept)s are not allowed in an extension linkbase: %(text)s"),
                        modelObject=modelReference, concept=concept.qname, text=text, xml=XmlUtil.xmlstring(modelReference, stripXmlns=True, contentsOnly=True))
            if self.validateSBRNL and (concept.isItem or concept.isTuple):
                if concept.modelDocument.targetNamespace not in disclosureSystem.standardTaxonomiesDict:
                    if not conceptHasDefaultLangStandardLabel:
                        modelXbrl.error(u"SBR.NL.2.2.2.26",
                            _(u"Concept %(concept)s missing standard label in local language."),
                            modelObject=concept, concept=concept.qname)
                    subsGroup = concept.get(u"substitutionGroup")
                    if ((not concept.isAbstract or subsGroup == u"sbr:presentationItem") and
                        not (presentationRelationshipSet.toModelObject(concept) or
                             presentationRelationshipSet.fromModelObject(concept))):
                        modelXbrl.error(u"SBR.NL.2.2.2.04",
                            _(u"Concept %(concept)s not referred to by presentation relationship."),
                            modelObject=concept, concept=concept.qname)
                    elif ((concept.isDimensionItem or
                          (subsGroup and (subsGroup.endswith(u":domainItem") or subsGroup.endswith(u":domainMemberItem")))) and
                        not (presentationRelationshipSet.toModelObject(concept) or
                             presentationRelationshipSet.fromModelObject(concept))):
                        modelXbrl.error(u"SBR.NL.2.2.10.03",
                            _(u"DTS concept %(concept)s not referred to by presentation relationship."),
                            modelObject=concept, concept=concept.qname)
                    if (concept.substitutionGroupQname and 
                        ...
                        modelXbrl.error(u"SBR.NL.2.2.2.05",
                            _(u"Concept %(concept)s has a substitutionGroup of a non-standard concept."),
                            modelObject=concept, concept=concept.qname)
                    ...
                    if concept.isTuple: # verify same presentation linkbase nesting
                        for missingQname in set(concept.type.elements) ^ pLinkedNonAbstractDescendantQnames(modelXbrl, concept):
                            modelXbrl.error(u"SBR.NL.2.3.4.01",
                                _(u"Tuple %(concept)s has mismatch between content and presentation children: %(missingQname)s."),
        ...
        for roleURI, modelRoleTypes in modelXbrl.roleTypes.items():
            if len(modelRoleTypes) > 1:
                modelXbrl.error((u"EFM.6.07.10", u"GFM.1.03.10"),
                    _(u"RoleType %(roleType)s is defined in multiple taxonomies"),
        ...
        for arcroleURI, modelRoleTypes in modelXbrl.arcroleTypes.items():
            if len(modelRoleTypes) > 1:
                modelXbrl.error((u"EFM.6.07.14", u"GFM.1.03.16"),
                    _(u"ArcroleType %(arcroleType)s is defined in multiple taxonomies"),
                    modelObject=modelRoleTypes, arcroleType=arcroleURI, numberOfDeclarations=len(modelRoleTypes) )
                    

        self.modelXbrl.profileActivity(u"... filer concepts checks", minTimeToShow=1.0)
        ...
        u''' removed RH 2011-12-23, corresponding use of nameWordsTable in ValidateFilingDTS
        if self.validateSBRNL:
            del self.nameWordsTable
        '''
        self.modelXbrl.profileActivity(u"... filer DTS checks", minTimeToShow=1.0)
        ...
        if self.validateEFM:
            ...
            for conflictClass, modelDocuments in self.standardNamespaceConflicts.items():
                if len(modelDocuments) > 1:
                    modelXbrl.error(u"EFM.6.22.03",
                        _(u"References for conflicting standard %(conflictClass)s taxonomies %(namespaceConflicts)s are not allowed in same DTS"),
                        modelObject=modelXbrl, conflictClass=conflictClass, 
                        namespaceConflicts=sorted((d.targetNamespace for d in modelDocuments),
                                                  key=lambda ns: ns.rpartition(u'/')[2]))
        ...
        for arcroleFilter in (XbrlConst.summationItem, XbrlConst.parentChild, u"*"):
            for baseSetKey, baseSetModelLinks  in modelXbrl.baseSets.items():
                arcrole, ELR, linkqname, arcqname = baseSetKey
                if ELR and linkqname and arcqname and not arcrole.startswith(u"XBRL-"):
                    # assure summationItem, then parentChild, then others
                    if not (arcroleFilter == arcrole or
                            arcroleFilter == u"*" and arcrole not in (XbrlConst.summationItem, XbrlConst.parentChild)):
                        continue
                    if self.validateEFMorGFM or (self.validateSBRNL and arcrole == XbrlConst.parentChild):
                        ...
                        for modelRel in ineffectiveArcs:
                            if modelRel.fromModelObject is not None and modelRel.toModelObject is not None:
                                modelXbrl.error((u"EFM.6.09.03", u"GFM.1.04.03", u"SBR.NL.2.3.4.06"),
                                    _(u"Ineffective arc %(arc)s in \nlink role %(linkrole)s \narcrole %(arcrole)s \nfrom %(conceptFrom)s \nto %(conceptTo)s \n%(ineffectivity)s"),
                    ...
                    if arcrole == XbrlConst.parentChild:
                        isStatementSheet = any(linkroleDefinitionStatementSheet.match(roleType.definition or u'')
                        ...
                        for relFrom, siblingRels in parentChildRels.fromModelObjects().items():
                            ...
                            for rel in siblingRels:
                                ...
                                if relTo in conceptsUsed:
                                    ...
                                    if preferredLabel and preferredLabel != u"":
                                        conceptRelsUsedWithPreferredLabels[relTo].append(rel)
                                        if self.validateSBRNL and preferredLabel in (u"periodStart",u"periodEnd"):
                                            modelXbrl.error(u"SBR.NL.2.3.4.03",
                                                _(u"Preferred label on presentation relationships not allowed"), modelObject=modelRel)
                                    ...
                                    if (preferredLabel in preferredLabels or
                                        ...
                                        if preferredLabel in preferredLabels:
                                            ...
                                            rel2 = relTo2 = None
                                        modelXbrl.error((u"EFM.6.12.05", u"GFM.1.06.05", u"SBR.NL.2.3.4.06"),
                                            _(u"Concept %(concept)s has duplicate preferred label %(preferredLabel)s in link role %(linkrole)s"),
                                ...
                                if order in orderRels:
                                    modelXbrl.error((u"EFM.6.12.02", u"GFM.1.06.02", u"SBR.NL.2.3.4.05"),
                                        _(u"Duplicate presentation relations from concept %(conceptFrom)s for order %(order)s in base set role %(linkrole)s to concept %(conceptTo)s and to concept %(conceptTo2)s"),
                                        modelObject=(rel, orderRels[order]), conceptFrom=relFrom.qname, order=rel.arcElement.get(u"order"), linkrole=rel.linkrole, linkroleDefinition=modelXbrl.roleTypeDefinition(rel.linkrole),
                                ...
                                if self.validateSBRNL and not relFrom.isTuple:
                                    if relTo in localPreferredLabels:
                                        if set([None, preferredLabel]) & localPreferredLabels[relTo]:
                                            self.modelXbrl.error(u"SBR.NL.2.3.4.06",
                                                _(u"Non-distinguished preferredLabel presentation relations from concept %(conceptFrom)s in base set role %(linkrole)s"),
                        ...
                        if self.validateEFMorGFM:
                            ...
                            for relFrom, rels in fromRelationships.items():
                                ...
                                for rel in rels:
                                    ...
                                    if isinstance(relTo, ModelConcept) and relFrom.periodType != relTo.periodType:
                                        self.modelXbrl.error((u"EFM.6.14.03", u"GFM.1.07.03"),
                                            u"Calculation relationship period types mismatched in base set role %(linkrole)s from %(conceptFrom)s to %(conceptTo)s",
                                    ...
                                    if order in orderRels and disclosureSystem.GFM:
                                        self.modelXbrl.error((u"EFM.N/A", u"GFM.1.07.06"),
                                            _(u"Duplicate calculations relations from concept %(conceptFrom)s for order %(order)s in base set role %(linkrole)s to concept %(conceptTo)s and to concept %(conceptTo2)s"),
                                            modelObject=(rel, orderRels[order]), linkrole=rel.linkrole, conceptFrom=relFrom.qname, order=order,
                                            conceptTo=rel.toModelObject.qname, conceptTo2=orderRels[order].toModelObject.qname)
                                    else:
                                        orderRels[order] = rel
                                directedCycleRels = self.directedCycle(relFrom,relFrom,fromRelationships,set([relFrom]))
                                if directedCycleRels is not None:
                                    self.modelXbrl.error((u"EFM.6.14.04", u"GFM.1.07.04"),
                                        _(u"Calculation relationships have a directed cycle in base set role %(linkrole)s starting from %(concept)s"),
                                ...
                                if rels and relFrom in conceptsUsed:
                                    ...
                                    if relFromAndTos in usedCalcFromTosELR:
                                        ...
                                        self.modelXbrl.log(u"WARNING-SEMANTIC", (u"EFM.6.15.04", u"GFM.2.06.04"),
                                            _(u"Calculation relationships should have a same set of targets in %(linkrole)s and %(linkrole2)s starting from %(concept)s"),
                            ...
                            for modelRel in self.modelXbrl.relationshipSet(arcrole, ELR).modelRelationships:
                                self.modelXbrl.error(u"SBR.NL.2.3.9.01",
                                    _(u"Calculation linkbase linkrole %(linkrole)s"),
                        ...
                        for relFrom, rels in fromRelationships.items():
                            for rel in rels:
                                ...
                                if not (isinstance(relTo, ModelConcept) and relTo.type is not None and relTo.type.isDomainItemType) and not self.isStandardUri(rel.modelDocument.uri):
                                    self.modelXbrl.error((u"EFM.6.16.03", u"GFM.1.08.03"),
                                        _(u"Definition relationship from %(conceptFrom)s to %(conceptTo)s in role %(linkrole)s requires domain item target"),
                        ...
                        if arcrole == XbrlConst.dimensionDefault:
                            for modelRel in self.modelXbrl.relationshipSet(arcrole).modelRelationships:
                                self.modelXbrl.error(u"SBR.NL.2.3.6.05",
                                    _(u"Dimension-default in from %(conceptFrom)s to %(conceptTo)s in role %(linkrole)s is not allowed"),
                    ...
                    if XbrlConst.isDefinitionOrXdtArcrole(arcrole) and disclosureSystem.GFM: 
                        ...
                        for relFrom, rels in fromRelationships.items():
                            ...
                            for rel in rels:
                                ...
                                order = rel.order
                                if order in orderRels and disclosureSystem.GFM:
                                    self.modelXbrl.error(u"GFM.1.08.10",
                                        _(u"Duplicate definitions relations from concept %(conceptFrom)s for order %(order)s in base set role %(linkrole)s to concept %(conceptTo)s and to concept %(conceptTo2)s"),
                                ...
                                if (arcrole not in (XbrlConst.dimensionDomain, XbrlConst.domainMember) and
                                    rel.get(u"{http://xbrl.org/2005/xbrldt}usable") == u"false"):
                                    self.modelXrl.error(u"GFM.1.08.11",
                                        _(u"Disallowed xbrldt:usable='false' attribute on %(arc)s relationship from concept %(conceptFrom)s in base set role %(linkrole)s to concept %(conceptTo)s"),
        ...
        self.modelXbrl.profileActivity(u"... filer relationships checks", minTimeToShow=1.0)

                                
        # checks on dimensions
        ValidateFilingDimensions.checkDimensions(self, drsELRs)
        self.modelXbrl.profileActivity(u"... filer dimensions checks", minTimeToShow=1.0)
                                        
        for concept, hasPresentationRelationship in conceptsUsed.items():
            if not hasPresentationRelationship:
                self.modelXbrl.error((u"EFM.6.12.03", u"GFM.1.6.3"),
                    _(u"Concept used in instance %(concept)s does not participate in an effective presentation relationship"),
        ...
        for fromIndx, toIndxs in usedCalcsPresented.items():
            for toIndx in toIndxs:
                ...
                if contextId is None:
                    contextId = backupId
                self.modelXbrl.error((u"EFM.6.14.05", u"GFM.1.7.5"),
                    _(u"Used calculation relationship from %(conceptFrom)s to %(conceptTo)s does not participate in an effective presentation relationship"),
            ...
            for concept, preferredLabelRels in conceptRelsUsedWithPreferredLabels.items():
                for preferredLabelRel in preferredLabelRels:
                    ...
                    if not hasDefaultLangPreferredLabel:
                        self.modelXbrl.error(u"GFM.1.06.04", # 6.12.04 now reserved: ("EFM.6.12.04", "GFM.1.06.04"),
                            _(u"Concept %(concept)s missing %(lang)s preferred labels for role %(preferredLabel)s"),
        ...
        self.modelXbrl.profileActivity(u"... filer preferred label checks", minTimeToShow=1.0)
        
        u''' try moving to plug-in
        if self.validateSBRNL:
        ...
        if self.validateEFM:
            for pluginXbrlMethod in pluginClassMethods(u"Validate.EFM.Finally"):
                pluginXbrlMethod(self, conceptsUsed)
        elif self.validateSBRNL:
            for pluginXbrlMethod in pluginClassMethods(u"Validate.SBRNL.Finally"):
                pluginXbrlMethod(self, conceptsUsed)
        self.modelXbrl.profileActivity(u"... plug in '.Finally' checks", minTimeToShow=1.0)
        self.modelXbrl.profileStat(_(u"validate{0}").format(modelXbrl.modelManager.disclosureSystem.validationType))
        ...
        try:
            ...
            isStd = (uri in self.disclosureSystem.standardTaxonomiesDict or
                     (not isHttpUrl(uri) and 
                      ...
                      u"/basis/sbr/" in uri.replace(u"\\",u"/")
                      ))
    ...
    u''' this may be unused now, if common stock class members are only taken from observed facts, not defined members, per Dean R
    ...
    def checkConceptLabels(self, modelXbrl, labelsRelationshipSet, disclosureSystem, concept):
        ...
        for modelLabelRel in labelsRelationshipSet.fromModelObject(concept):
            ...
            if modelLabel is not None and modelLabel.xmlLang:
                ...
                dupDetectKey = ( (modelLabel.role or u''), modelLabel.xmlLang)
                if dupDetectKey in dupLabels:
                    modelXbrl.error((u"EFM.6.10.02", u"GFM.1.5.2", u"SBR.NL.2.2.1.05"),
                        _(u"Concept %(concept)s has duplicated labels for role %(role)s lang %(lang)s."),
                ...
                if modelLabel.role in (XbrlConst.periodStartLabel, XbrlConst.periodEndLabel):
                    modelXbrl.error(u"SBR.NL.2.3.8.03",
                        _(u"Concept %(concept)s has label for semantical role %(role)s."),
        ...
        if self.validateSBRNL: # check for missing nl labels
            for role, lang in dupLabels.keys():
                if role and lang != disclosureSystem.defaultXmlLang and (role,disclosureSystem.defaultXmlLang) not in dupLabels:
                    modelXbrl.error(u"SBR.NL.2.3.8.05",
                        _(u"Concept %(concept)s has en but no nl label in role %(role)s."),
        ...
        if not hasDefaultLangStandardLabel:
            modelXbrl.error((u"EFM.6.10.01", u"GFM.1.05.01"),
                _(u"Concept used in facts %(concept)s is missing an %(lang)s standard label."),
        ...
        try:
            dupLabels[(u"zzzz",disclosureSystem.defaultXmlLang)] = None #to allow following loop
            priorRole = None
            priorLang = None
            hasDefaultLang = True
            for role, lang in sorted(dupLabels.keys()):
                if role != priorRole:
                    if not hasDefaultLang:
                        modelXbrl.error((u"EFM.6.10.03", u"GFM.1.5.3"),
                            _(u"Concept %(concept)s is missing an %(lang)s label for role %(role)s."),
    ...
    def presumptionOfTotal(self, rel, siblingRels, iSibling, isStatementSheet, nestedInTotal, checkLabelRoleOnly):
        ...
        if isinstance(concept, ModelConcept) and concept.isNumeric:
            ...
            if XbrlConst.isTotalRole(preferredLabel):
                return _(u"preferredLabel {0}").format(os.path.basename(preferredLabel))
            if concept.isMonetary and not checkLabelRoleOnly: 
                effectiveLabel = concept.label(lang=u"en-US", fallbackToQname=False, preferredLabel=preferredLabel)
                u''' word total in label/name does not seem to be a good indicator, 
                ...
                if (len(siblingRels) > 1 and
                    ...
                    parent.name not in set([
                        u"SupplementalCashFlowInformationAbstract"])):
                    ...
                    if preceedingSibling is not None and preceedingSibling.isMonetary:
                        ...
                        if isStatementSheet:
                            ...
                            if (parent.isAbstract or not parent.isMonetary) and not nestedInTotal:
                                return _(u"last monetary item in statement sheet monetary line items parented by nonMonetary concept")
                            elif effectiveLabel and u'Total' in effectiveLabel: 
                                return _(u"last monetary item in statement sheet monetary line items with word 'Total' in effective label {0}").format(effectiveLabel)
                            elif u'Total' in concept.name:
                                return _(u"last monetary item in statement sheet monetary line items with word 'Total' in concept name {0}").format(concept.name)
                            elif self.summationItemRelsSetAllELRs.isRelated(concept, u"child", preceedingSibling):
                                return _(u"last monetary item in statement sheet monetary line items is calc sum of previous line item")
                        u''' for now unreliable to use total words for notes
                        else:
                            if 'Total' in effectiveLabel: # also check for Net ???
    ...
    def checkForCalculations(self, parentChildRels, siblingRels, iSibling, totalConcept, totalRel, reasonPresumedTotal, isStatementSheet, conceptsUsed, nestedItems, contributingItems):
        ...
        for iContributingRel in xrange(iSibling - 1, -1, -1):
            ...
            if siblingConcept is not None:
                if siblingConcept is totalConcept: # direct cycle loop likely, possibly among children of abstract sibling
                    break
                if self.summationItemRelsSetAllELRs.isRelated(totalConcept, u'ancestral-sibling', siblingConcept):
                    break # sibling independently contributes as sibling of totalConcept to grandfather total
                if any(self.summationItemRelsSetAllELRs.isRelated(contributingItem, u'child', siblingConcept)
            ...
            for compatibleItemConcepts, compatibleFacts in compatibleItemsFacts.items():
                ...
                for ELR in self.summationItemRelsSetAllELRs.linkRoleUris:
                    ...
                    missingItems -= set(concept
                                        ...
                                        if relSet.isRelated(totalConcept, u"sibling-or-descendant", concept))
                    # items not required in sum
                    unrequiredItems = set(concept
                                          for concept in missingItems
                                          if concept.name in (u"CommitmentsAndContingencies"))
                ...
                if not foundSummationItemSet:
                    ...
                    reasonIssueIsWarning = u""
                    msgCode = u"ERROR-SEMANTIC"
                    if isStatementSheet:
                        errs = (u"EFM.6.15.02,6.13.02,6.13.03", u"GFM.2.06.02,2.05.02,2.05.03")
                        msg = _(u"Financial statement calculation relationship missing from total concept to item concepts, based on required presentation of line items and totals.  "
                                u"%(reasonIssueIsWarning)s"
                        ...
                        errs = (u"EFM.6.15.03,6.13.02,6.13.03", u"GFM.2.06.03,2.05.02,2.05.03")
                        msg = _(u"Notes calculation relationship missing from total concept to item concepts, based on required presentation of line items and totals. "
                                u"%(reasonIssueIsWarning)s"
                                u"\n\nPresentation link role: \n%(linkrole)s \n%(linkroleDefinition)s."
                                u"\n\nTotal concept: \n%(conceptSum)s.  "
                                u"\n\nReason presumed total: \n%(reasonPresumedTotal)s.  "
                    ...
                    if all(f.isNil for f in compatibleFacts if f.concept in leastMissingItemsSet):
                        reasonIssueIsWarning = _(u"\n\nMissing items are nil, which doesn't affect validity but may impair analysis of concept semantics from calculation relationships.  ")
                        msgCode = u"WARNING-SEMANTIC"
                        errs = tuple(e + u'.missingItemsNil' for e in errs)
                    if u"parenthetical" in linkroleDefinition.lower():
                        reasonIssueIsWarning += _(u"\n\nLink role is parenthetical.  ")
                        msgCode = u"WARNING-SEMANTIC"
                        errs = tuple(e + u'.parenthetical' for e in errs)
                    u"""@messageCatalog=[
...
%(contextIDs)s"]]"""
                    self.modelXbrl.log(msgCode, errs, msg,
                        ...
                        itemConcepts=u', \n'.join(sorted(set(unicode(c.qname) for c in compatibleItemConcepts))),
                        missingConcepts = u', \n'.join(sorted(set(unicode(c.qname) for c in leastMissingItemsSet))),
                        contextIDs=u', '.join(sorted(set(f.contextID for f in compatibleFacts))))
    ...
    @property 
    def EFM60303(self):
        if self.exhibitType == u"EX-2.01": # only applicable for edgar production and parameterized testcases
            return u"EFM.6.23.01"
        else:
            return u"EFM.6.03.03"
