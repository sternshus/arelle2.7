...
class fnFunctionNotAvailable(Exception):
    def __init__(self):
        self.args =  (u"custom function not available",)
    def __repr__(self):
        return self.args[0]
...
def call(xc, p, qname, contextItem, args):
    try:
        ...
        raise XPathContext.FunctionNotAvailable(u"custom function:{0}".format(unicode(qname)))
...
def callCfi(xc, p, qname, cfSig, contextItem, args):
    ...
    if traceEvaluation:
        xc.modelXbrl.info(u"formula:trace",
                            _(u"%(cfi)s(%(arguments)s)"),
                            modelObject=cfi,
                            cfi=qname, 
                            arguments=u', '.join(u"{}={}".format(argName, args[i])
                                                for i, argName in enumerate(inputNames)))
    ...
    if result is None:  # atomic value failed the result cast expression
        raise XPathContext.FunctionArgType(u"output",cfSig.outputType,result)
    return result
...
def  my_fn_PDxEV(xc, p, contextItem, args):
    ...
    dimQname = qname(u"{http://www.example.com/wgt-avg}ExposuresDimension")
    PDxEV = []
    for pd in PDseq:
...
customFunctions = {
    qname(u"{http://www.example.com/wgt-avg/function}my-fn:PDxEV"): my_fn_PDxEV
}
