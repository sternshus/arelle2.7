...
class ModelManager(object):
    u"""ModelManager provides an interface between modelXbrl's and the controller.  Model manager is a 
    ...
    def __init__(self, cntlr):
        ...
        self.locale = Locale.getUserLocale(cntlr.config.get(u"userInterfaceLocaleOverride",u""))
        self.defaultLang = Locale.getLanguageCode()

    def shutdown(self):
        self.status = u"shutdown"
        
    def addToLog(self, message, messageCode=u"", file=u"", level=logging.INFO):
    ...
    def showStatus(self, message, clearAfter=None):
        u"""Provide user feedback on status line of GUI or web page according to type of controller.
    ...
    def viewModelObject(self, modelXbrl, objectId):
        u"""Notify any active views to show and highlight selected object.  Generally used
    ...
    def reloadViews(self, modelXbrl):
        u"""Notify all active views to reload and redisplay their entire contents.  May be used
    ...
    def load(self, filesource, nextaction=None):
        u"""Load an entry point modelDocument object(s), which in turn load documents they discover 
        ...
        """
        try:
            if filesource.url.startswith(u"urn:uuid:"): # request for an open modelXbrl
                for modelXbrl in self.loadedModelXbrls:
                    if not modelXbrl.isClosed and modelXbrl.uuid == filesource.url:
                        return modelXbrl
                raise IOError(_(u"Open file handle is not open: {0}").format(filesource.url))
        except AttributeError:
            pass # filesource may be a string, which has no url attribute
        self.filesource = filesource
        modelXbrl = None # loaded modelXbrl
        for customLoader in pluginClassMethods(u"ModelManager.Load"):
    ...
    def validate(self):
        u"""Validates the most recently loaded modelXbrl (according to validation properties).
        ...
        try:
            ...
            self.addToLog(_(u"[exception] Validation exception: {0} at {1}").format(
                           err,
                           traceback.format_tb(sys.exc_info()[2])))
        
    def compareDTSes(self, versReportFile, writeReportFile=True):
        u"""Compare two most recently loaded DTSes, saving versioning report in to the file name provided.
    ...
    def close(self, modelXbrl=None):
        u"""Closes the specified or most recently loaded modelXbrl
