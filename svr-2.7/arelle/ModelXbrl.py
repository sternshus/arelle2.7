...
AUTO_LOCATE_ELEMENT = u'771407c0-1d0c-11e1-be5e-028037ec0200' # singleton meaning choose best location for new element
DEFAULT = intern(_STR_8BIT(u"default"))
NONDEFAULT = intern(_STR_8BIT(u"non-default"))
DEFAULTorNONDEFAULT = intern(_STR_8BIT(u"default-or-non-default"))
    

def load(modelManager, url, nextaction=None, base=None, useFileSource=None, errorCaptureLevel=None, **kwargs):
    u"""Each loaded instance, DTS, testcase, testsuite, versioning report, or RSS feed, is represented by an 
   ...
   """
    if nextaction is None: nextaction = _(u"loading")
    ...
    modelManager.showStatus(_(u"xbrl loading finished, {0}...").format(nextaction))
    return modelXbrl
...
def create(modelManager, newDocumentType=None, url=None, schemaRefs=None, createModelDocument=True, isEntry=False, errorCaptureLevel=None, initialXml=None, initialComment=None, base=None):
    ...
    if newDocumentType:
        ...
        if createModelDocument:
            modelXbrl.modelDocument = ModelDocument.create(modelXbrl, newDocumentType, unicode(url), schemaRefs=schemaRefs, isEntry=isEntry, initialXml=initialXml, initialComment=initialComment, base=base)
            if isEntry:
                del modelXbrl.entryLoadingUrl
                loadSchemalocatedSchemas(modelXbrl)
    return modelXbrl
...
class ModelXbrl(object):
    ...
    def init(self, keepViews=False, errorCaptureLevel=None):
        ...
        self.errorCaptureLevel = (errorCaptureLevel or logging._checkLevel(u"INCONSISTENCY"))
        self.errors = []
        self.logCount = {}
        ...
        if not keepViews:
            self.views = []
        self.langs = set([self.modelManager.defaultLang])
        from arelle.XbrlConst import standardLabel
        self.labelroles = set([standardLabel])
        self.hasXDT = False
        ...
        self.logRefObjectProperties = getattr(self.logger, u"logRefObjectProperties", False)
        self.logRefHasPluginAttrs = any(True for m in pluginClassMethods(u"Logging.Ref.Attributes"))
        self.logRefHasPluginProperties = any(True for m in pluginClassMethods(u"Logging.Ref.Properties"))
        self.profileStats = {}
        self.schemaDocsToValidate = set()
        self.modelXbrl = self # for consistency in addressing modelXbrl
        for pluginXbrlMethod in pluginClassMethods(u"ModelXbrl.Init"):
    ...
    def close(self):
        ...
        if not self.isClosed:
            ...
            if hasattr(self,u"fileSource") and self.closeFileSource:
                self.fileSource.close()
            modelDocument = getattr(self,u"modelDocument",None)
            urlDocs = getattr(self,u"urlDocs",None)
    ...
    def reload(self,nextaction,reloadCache=False):
        u"""Reloads all model objects from their original entry point URL, preserving any open views (which are reloaded).
        ...
        self.modelManager.showStatus(_(u"xbrl loading finished, {0}...").format(nextaction),5000)
        self.modelManager.reloadViews(self)
            
    def closeViews(self):
        u"""Close views associated with this modelXbrl
        """
        if not self.isClosed:
            for view in xrange(len(self.views)):
                if len(self.views) > 0:
                    self.views[0].close()
        
    def relationshipSet(self, arcrole, linkrole=None, linkqname=None, arcqname=None, includeProhibits=False):
        u"""Returns a relationship set matching specified parameters (only arcrole is required).
    ...
    def baseSetModelLink(self, linkElement):
        for modelLink in self.baseSets[(u"XBRL-footnotes",None,None,None)]:
    ...
    def matchSubstitutionGroup(self, elementQname, subsGrpMatchTable):
        u"""Resolve a subsitutionGroup for the elementQname from the match table
    ...
    def isInSubstitutionGroup(self, elementQname, subsGrpQnames):
        u"""Determine if element is in substitution group(s)
        ...
        """
        return self.matchSubstitutionGroup(elementQname, dict((
                  qn, (qn is not None)) for qn in (subsGrpQnames if hasattr(subsGrpQnames, u'__iter__') else (subsGrpQnames,)) + (None,)))
    
    def createInstance(self, url=None):
        u"""Creates an instance document for a DTS which didn't have an instance document, such as
    ...
    def saveInstance(self, overrideFilepath=None):
        u"""Saves current instance document file.
    ...
    def prefixedNamespaces(self):
        u"""Dict of prefixes for namespaces defined in DTS
        """
        prefixedNamespaces = {}
    ...
    def matchContext(self, entityIdentScheme, entityIdentValue, periodType, periodStart, periodEndInstant, dims, segOCCs, scenOCCs):
        u"""Finds matching context, by aspects, as in formula usage, if any
        ...
        for c in self.contexts.values():
            if (c.entityIdentifier == (entityIdentScheme, entityIdentValue) and
                ((c.isInstantPeriod and periodType == u"instant" and dateUnionEqual(c.instantDatetime, periodEndInstant, instantEndDate=True)) or
                 (c.isStartEndPeriod and periodType == u"duration" and dateUnionEqual(c.startDatetime, periodStart) and dateUnionEqual(c.endDatetime, periodEndInstant, instantEndDate=True)) or
                 (c.isForeverPeriod and periodType == u"forever")) and
                 ...
                 all(
                   all([sEqual(self, cOCCs[i], mOCCs[i]) for i in xrange(len(mOCCs))])
    ...
    def createContext(self, entityIdentScheme, entityIdentValue, periodType, periodStart, periodEndInstant, priItem, dims, segOCCs, scenOCCs,
        ...
        u"""Creates a new ModelContext and validates (integrates into modelDocument object model).
        ...
        if afterSibling == AUTO_LOCATE_ELEMENT:
            afterSibling = XmlUtil.lastChild(xbrlElt, XbrlConst.xbrli, (u"schemaLocation", u"roleType", u"arcroleType", u"context"))
        cntxId = id if id else u'c-{0:02n}'.format( len(self.contexts) + 1)
        newCntxElt = XmlUtil.addChild(xbrlElt, XbrlConst.xbrli, u"context", attributes=(u"id", cntxId),
                                      afterSibling=afterSibling, beforeSibling=beforeSibling)
        entityElt = XmlUtil.addChild(newCntxElt, XbrlConst.xbrli, u"entity")
        XmlUtil.addChild(entityElt, XbrlConst.xbrli, u"identifier",
                            attributes=(u"scheme", entityIdentScheme),
                            text=entityIdentValue)
        periodElt = XmlUtil.addChild(newCntxElt, XbrlConst.xbrli, u"period")
        if periodType == u"forever":
            XmlUtil.addChild(periodElt, XbrlConst.xbrli, u"forever")
        elif periodType == u"instant":
            XmlUtil.addChild(periodElt, XbrlConst.xbrli, u"instant", 
                             text=XmlUtil.dateunionValue(periodEndInstant, subtractOneDay=True))
        elif periodType == u"duration":
            XmlUtil.addChild(periodElt, XbrlConst.xbrli, u"startDate", 
                             text=XmlUtil.dateunionValue(periodStart))
            XmlUtil.addChild(periodElt, XbrlConst.xbrli, u"endDate", 
                             text=XmlUtil.dateunionValue(periodEndInstant, subtractOneDay=True))
        segmentElt = None
        scenarioElt = None
        from arelle.ModelInstanceObject import ModelDimensionValue
        if dims: # requires primary item to determin ambiguous concepts
            u''' in theory we have to check full set of dimensions for validity in source or any other
            ...
            if priItem is not None: # creating concept for a specific fact
                ...
                if not isFactDimensionallyValid(self, fp, setPrototypeContextElements=True):
                    self.info(u"arelle:info",
                        _(u"Create context for %(priItem)s, cannot determine valid context elements, no suitable hypercubes"), 
            ...
            for dimQname in sorted(fpDims.keys()):
                ...
                if isinstance(dimValue, (DimValuePrototype,ModelDimensionValue)):
                    ...
                    dimMemberQname = None
                    contextEltName = None
                if contextEltName == u"segment":
                    if segmentElt is None: 
                        segmentElt = XmlUtil.addChild(entityElt, XbrlConst.xbrli, u"segment")
                    contextElt = segmentElt
                elif contextEltName == u"scenario":
                    if scenarioElt is None: 
                        scenarioElt = XmlUtil.addChild(newCntxElt, XbrlConst.xbrli, u"scenario")
                    contextElt = scenarioElt
                else:
                    self.info(u"arelleLinfo",
                        _(u"Create context, %(dimension)s, cannot determine context element, either no all relationship or validation issue"), 
                        modelObject=self, dimension=dimQname),
                    continue
                dimAttr = (u"dimension", XmlUtil.addQnameValue(xbrlElt, dimQname))
                if dimValue.isTyped:
                    dimElt = XmlUtil.addChild(contextElt, XbrlConst.xbrldi, u"xbrldi:typedMember", 
                                              attributes=dimAttr)
                    ...
                    dimElt = XmlUtil.addChild(contextElt, XbrlConst.xbrldi, u"xbrldi:explicitMember",
                                              attributes=dimAttr,
    ...
    def matchUnit(self, multiplyBy, divideBy):
        u"""Finds matching unit, by measures, as in formula usage, if any
    ...
    def createUnit(self, multiplyBy, divideBy, afterSibling=None, beforeSibling=None, id=None):
        u"""Creates new unit, by measures, as in formula usage, if any
        ...
        xbrlElt = self.modelDocument.xmlRootElement
        if afterSibling == AUTO_LOCATE_ELEMENT:
            afterSibling = XmlUtil.lastChild(xbrlElt, XbrlConst.xbrli, (u"schemaLocation", u"roleType", u"arcroleType", u"context", u"unit"))
        unitId = id if id else u'u-{0:02n}'.format( len(self.units) + 1)
        newUnitElt = XmlUtil.addChild(xbrlElt, XbrlConst.xbrli, u"unit", attributes=(u"id", unitId),
                                      afterSibling=afterSibling, beforeSibling=beforeSibling)
        if len(divideBy) == 0:
            for multiply in multiplyBy:
                XmlUtil.addChild(newUnitElt, XbrlConst.xbrli, u"measure", text=XmlUtil.addQnameValue(xbrlElt, multiply))
        else:
            divElt = XmlUtil.addChild(newUnitElt, XbrlConst.xbrli, u"divide")
            numElt = XmlUtil.addChild(divElt, XbrlConst.xbrli, u"unitNumerator")
            denElt = XmlUtil.addChild(divElt, XbrlConst.xbrli, u"unitDenominator")
            for multiply in multiplyBy:
                XmlUtil.addChild(numElt, XbrlConst.xbrli, u"measure", text=XmlUtil.addQnameValue(xbrlElt, multiply))
            for divide in divideBy:
                XmlUtil.addChild(denElt, XbrlConst.xbrli, u"measure", text=XmlUtil.addQnameValue(xbrlElt, divide))
    ...
    def nonNilFactsInInstance(self): # indexed by fact (concept) qname
        u"""Facts in the instance which are not nil, cached
    ...
    def factsByQname(self): # indexed by fact (concept) qname
        u"""Facts in the instance indexed by their QName, cached
    ...
    def factsByDatatype(self, notStrict, typeQname): # indexed by fact (concept) qname
        u"""Facts in the instance indexed by data type QName, cached as types are requested
    ...
    def factsByPeriodType(self, periodType): # indexed by fact (concept) qname
        u"""Facts in the instance indexed by periodType, cached
    ...
    def factsByDimMemQname(self, dimQname, memQname=None): # indexed by fact (concept) qname
        u"""Facts in the instance indexed by their Dimension  and Member QName, cached
    ...
    def matchFact(self, otherFact, unmatchedFactsStack=None, deemP0inf=False):
        u"""Finds matching fact, by XBRL 2.1 duplicate definition (if tuple), or by
    ...
    def createFact(self, conceptQname, attributes=None, text=None, parent=None, afterSibling=None, beforeSibling=None, validate=True):
        u"""Creates new fact, as in formula output instance creation, and validates into object model
        ...
        # update cached sets
        if not newFact.isNil and hasattr(self, u"_nonNilFactsInInstance"):
            self._nonNilFactsInInstance.add(newFact)
        if hasattr(self, u"_factsByQname"):
            self._factsByQname[newFact.qname].add(newFact)
        if newFact.concept is not None:
            if hasattr(self, u"_factsByDatatype"):
                del self._factsByDatatype # would need to iterate derived type ancestry to populate
            if hasattr(self, u"_factsByPeriodType"):
                self._factsByPeriodType[newFact.concept.periodType].add(newFact)
            if hasattr(self, u"_factsByDimQname"):
                del self._factsByDimQname
        return newFact    
        
    def modelObject(self, objectId):
        u"""Finds a model object by an ordinal ID which may be buried in a tkinter view id string (e.g., 'somedesignation_ordinalnumber').
        ...
        try:
            return self.modelObjects[_INT(objectId.rpartition(u"_")[2])]
        except (IndexError, ValueError):
            return None
    
    # UI thread viewModelObject
    def viewModelObject(self, objectId):
        u"""Finds model object, if any, and synchronizes any views displaying it to bring the model object into scrollable view region and highlight it
        :param objectId: string which includes _ordinalNumber, produced by ModelObject.objectId(), or integer object index
        :type objectId: str or int
        """
        modelObject = u""
        try:
            ...
            elif isinstance(objectId, unicode) and objectId.startswith(u"_"):
            ...
            self.modelManager.addToLog(_(u"Exception viewing properties {0} {1} at {2}").format(
                            modelObject,
    ...
    def effectiveMessageCode(self, messageCodes):        
        ...
        for argCode in messageCodes if isinstance(messageCodes,tuple) else (messageCodes,):
            if (isinstance(argCode, ModelValue.QName) or
                (self.modelManager.disclosureSystem.EFM and argCode.startswith(u"EFM")) or
                (self.modelManager.disclosureSystem.GFM and argCode.startswith(u"GFM")) or
                (self.modelManager.disclosureSystem.HMRC and argCode.startswith(u"HMRC")) or
                (self.modelManager.disclosureSystem.SBRNL and argCode.startswith(u"SBR.NL")) or
                argCode[0:3] not in (u"EFM", u"GFM", u"HMR", u"SBR")):
    ...
    def isLoggingEffectiveFor(self, **kwargs): # args can be messageCode(s) and level
        logger = self.logger
        if u"messageCodes" in kwargs or u"messageCode" in kwargs:
            if u"messageCodes" in kwargs:
                messageCodes = kwargs[u"messageCodes"]
            else:
                messageCodes = kwargs[u"messageCode"]
            ...
            codeEffective = True
        if u"level" in kwargs and logger.messageLevelFilter:
            levelEffective = logger.messageLevelFilter.match(kwargs[u"level"].lower())
        else:
            levelEffective = True
        return codeEffective and levelEffective

    def logArguments(self, codes, msg, codedArgs):
        u""" Prepares arguments for logger function as per info() below.
        ...
        def propValues(properties):
            ...
            return [(p[0],unicode(p[1])) if len(p) == 2 else (p[0],unicode(p[1]),propValues(p[2]))
        ...
        fmtArgs = {}
        extras = {u"messageCode":messageCode}

        for argName, argValue in codedArgs.items():
            if argName in (u"modelObject", u"modelXbrl", u"modelDocument"):
                try:
                ...
                for arg in (argValue if isinstance(argValue, (tuple,list,set)) else (argValue,)):
                    if arg is not None:
                        ...
                        if isinstance(arg,(ModelObject, ObjectPropertyViewWrapper)):
                            ...
                            ref[u"href"] = file + u"#" + XmlUtil.elementFragmentIdentifier(_arg)
                            ref[u"sourceLine"] = _arg.sourceline
                            ref[u"objectId"] = _arg.objectId()
                            if self.logRefObjectProperties:
                                try:
                                    ref[u"properties"] = propValues(arg.propertyView)
                                except AttributeError:
                                    pass # is a default properties entry appropriate or needed?
                            if self.logRefHasPluginProperties:
                                refProperties = ref.get(u"properties", {})
                                for pluginXbrlMethod in pluginClassMethods(u"Logging.Ref.Properties"):
                                    pluginXbrlMethod(arg, refProperties, codedArgs)
                                if refProperties:
                                    ref[u"properties"] = refProperties
                        else:
                            ref[u"href"] = file
                            try:
                                ref[u"sourceLine"] = arg.sourceline
                            except AttributeError:
                                pass # arg may not have sourceline, ignore if so
                        if self.logRefHasPluginAttrs:
                            refAttributes = {}
                            for pluginXbrlMethod in pluginClassMethods(u"Logging.Ref.Attributes"):
                                pluginXbrlMethod(arg, refAttributes, codedArgs)
                            if refAttributes:
                                ref[u"customAttributes"] = refAttributes
                        refs.append(ref)
                extras[u"refs"] = refs
            elif argName == u"sourceFileLine":
                # sourceFileLines is pairs of file and line numbers, e.g., ((file,line),(file2,line2),...)
                ref = {}
                if isinstance(argValue, (tuple,list)):
                    ref[u"href"] = unicode(argValue[0])
                    if len(argValue) > 1 and argValue[1]:
                        ref[u"sourceLine"] = unicode(argValue[1])
                else:
                    ref[u"href"] = unicode(argValue)
                extras[u"refs"] = [ref]
            elif argName == u"sourceFileLines":
                ...
                for arg in (argValue if isinstance(argValue, (tuple,list)) else (argValue,)):
                    ...
                    if isinstance(arg, (tuple,list)):
                        ref[u"href"] = unicode(arg[0])
                        if len(arg) > 1 and arg[1]:
                            ref[u"sourceLine"] = unicode(arg[1])
                    else:
                        ref[u"href"] = unicode(arg)
                    refs.append(ref)
                extras[u"refs"] = refs
            elif argName == u"sourceLine":
                if isinstance(argValue, _INT_TYPES):    # must be sortable with int's in logger
                    extras[u"sourceLine"] = argValue
            elif argName not in (u"exc_info", u"messageCodes"):
                if isinstance(argValue, (ModelValue.QName, ModelObject, bool, FileNamedStringIO,
                                         ...
                                         tuple, list, set)):
                    fmtArgs[argName] = unicode(argValue)
                elif argValue is None:
                    fmtArgs[argName] = u"(none)"
                elif isinstance(argValue, _INT_TYPES):
                    # need locale-dependent formatting
                    fmtArgs[argName] = format_string(self.modelManager.locale, u'%i', argValue)
                elif isinstance(argValue,float):
                    # need locale-dependent formatting
                    fmtArgs[argName] = format_string(self.modelManager.locale, u'%f', argValue)
                else:
                    fmtArgs[argName] = argValue
        if u"refs" not in extras:
            try:
                ...
                try:
                    ...
                    file = u""
            extras[u"refs"] = [{u"href": file}]
        return (messageCode, 
                (msg, fmtArgs) if fmtArgs else (msg,), 
                extras)

    def info(self, codes, msg, **args):
        u"""Same as error(), but as info
        """
        u"""@messageCatalog=[]"""
        self.log(u'INFO', codes, msg, **args)
                    
    def warning(self, codes, msg, **args):
        u"""Same as error(), but as warning, and no error code saved for Validate
        """
        u"""@messageCatalog=[]"""
        self.log(u'WARNING', codes, msg, **args)
                    
    def log(self, level, codes, msg, **args):
        u"""Same as error(), but level passed in as argument
        """
        logger = self.logger
        messageCode, logArgs, extras = self.logArguments(codes, msg, args)
        if messageCode == u"asrtNoLog":
            self.errors.append(args[u"assertionResults"])
            ...
            u"""@messageCatalog=[]"""
            logger.log(numericLevel, *logArgs, exc_info=args.get(u"exc_info"), extra=extras)
                    
    def error(self, codes, msg, **args):
        u"""Logs a message as info, by code, logging-system message text (using %(name)s named arguments 
        ...
        """
        u"""@messageCatalog=[]"""
        self.log(u'ERROR', codes, msg, **args)

    def exception(self, codes, msg, **args):
        u"""Same as error(), but as exception
        """
        u"""@messageCatalog=[]"""
        self.log(u'CRITICAL', codes, msg, **args)
        
    def logProfileStats(self):
        u"""Logs profile stats that were collected
        """
        timeTotal = format_string(self.modelManager.locale, _(u"%.3f secs"), self.profileStats.get(u"total", (0,0,0))[1])
        timeEFM = format_string(self.modelManager.locale, _(u"%.3f secs"), self.profileStats.get(u"validateEFM", (0,0,0))[1])
        self.info(u"info:profileStats",
                _(u"Profile statistics \n") +
                u' \n'.join(format_string(self.modelManager.locale, _(u"%s %.3f secs, %.0fK"), (statName, statValue[1], statValue[2]), grouping=True)
    ...
    def profileActivity(self, activityCompleted=None, minTimeToShow=0):
        u"""Used to provide interactive GUI messages of long-running processes.
        ...
        try:
            if activityCompleted:
                ...
                if timeTaken > minTimeToShow:
                    self.modelManager.addToLog(u"{0} {1:.2f} secs".format(activityCompleted, timeTaken), messageCode=u"info:profileActivity")
        except AttributeError:
            pass
        self._startedProfiledActivity = time.time()

    def saveDTSpackage(self):
        u"""Contributed program to save DTS package as a zip file.  Refactored into a plug-in (and may be removed from main code).
        """ 
        ...
        pkgFilename = entryFilename + u".zip" 
        with ZipFile(pkgFilename, u'w') as zip:
            numFiles = 0
            for fileUri in sorted(self.urlDocs.keys()): 
                if not isHttpUrl(fileUri): 
                    ...
                    izip.write(fileUri, os.path.basename(fileUri)) 
        self.info(u"info",
                  _(u"DTS of %(entryFile)s has %(numberOfFiles)s files packaged into %(packageOutputFile)s"), 
                modelObject=self,
