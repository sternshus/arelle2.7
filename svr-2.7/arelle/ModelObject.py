...
class ModelObject(etree.ElementBase):
    u"""ModelObjects represent the XML elements within a document, and are implemented as custom 
    ...
    def _init(self):
        ...
        if parent is not None and hasattr(parent, u"modelDocument"):
    ...
    def setNamespaceLocalName(self):
        ns, sep, self._localName = self.tag.rpartition(u"}")
        if sep:
            self._namespaceURI = ns[1:]
        else:
            self._namespaceURI = None
        if self.prefix:
            self._prefixedName = self.prefix + u":" + self.localName
        else:
    ...
    def stringValue(self):    # "string value" of node, text of all Element descendants
        return u''.join(self._textNodes(recurse=True))  # return text of Element descendants
    
    @property
    def textValue(self):  # xml axis text() differs from string value, no descendant element text
        return u''.join(self._textNodes())  # no text nodes returns ''
    ...
    def elementAttributesStr(self):
        return u', '.join([u"{0}='{1}'".format(name,value) for name,value in self.items()])

    def resolveUri(self, hrefObject=None, uri=None, dtsModelXbrl=None):
        u"""Returns the modelObject within modelDocment that resolves a URI based on arguments relative
        to this element
    ...
    def genLabel(self,role=None,fallbackToQname=False,fallbackToXlinkLabel=False,lang=None,strip=False,linkrole=None):
        ...
        if role == XbrlConst.conceptNameLabelRole: return unicode(self.qname)
        ...
        if fallbackToQname:
            return unicode(self.qname)
        elif fallbackToXlinkLabel and hasattr(self,u"xlinkLabel"):
    ...
    def propertyView(self):
        return ((u"QName", self.qname),
                (u"id", self.id))
        
    def __repr__(self):
        return (u"{0}[{1}, {2} line {3})".format(type(self).__name__, self.objectIndex, self.modelDocument.basename, self.sourceline))

from arelle.ModelValue import qname, qnameEltPfxName, QName
    
class ModelComment(etree.CommentBase):
    u"""ModelConcept is a custom proxy objects for etree.
    """
    def _init(self):
        self.isChanged = False
        parent = self.getparent()
...
class ModelProcessingInstruction(etree.PIBase):
    u"""ModelProcessingInstruction is a custom proxy object for etree.
    """
    def _init(self):
        pass
...
class ModelAttribute(object):
    ...
    """
    __slots__ = (u"modelElement", u"attrTag", u"xValid", u"xValue", u"sValue", u"text")
...
class ObjectPropertyViewWrapper(object):  # extraProperties = ( (p1, v1), (p2, v2), ... )
    __slots__ = (u"modelObject", u"extraProperties")
    ...
    def __repr__(self):
        return u"objectPropertyViewWrapper({}, extraProperties={})".format(self.modelObject, self.extraProperties)
