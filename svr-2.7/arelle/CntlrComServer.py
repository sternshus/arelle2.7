...
class CntlrComServer(Cntlr.Cntlr):
    _public_methods_ = [ u'Load' ]
    _public_attrs_ = [ ]
    _readonly_attrs_ = [ ]
    _reg_progid_ = u"Arelle.XbrlServer"
    _reg_clsid_ = u"{C0E35073-789A-406B-B93B-3C5698CE4314}"
    _reg_desc_ = u"Arelle Open Source XBRL COM Server"
    ...
    def __init__(self, logFileName=None):
        ...
        self.last = u"({0})".format(self.startedAt)
        pass
    ...
    def Load(self, url):
        last = self.last
        self.last = url
        return u"inst: {0} this: {1}  prev: {2}".format(self.startedAt, url, last)
