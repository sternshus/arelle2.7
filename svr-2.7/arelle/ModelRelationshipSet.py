...
USING_EQUIVALENCE_KEY = intern(_STR_8BIT(u"using_equivalence_key")) # indicates hash entry replaced with keyed entry
    ...
    for modelLink in baseSetModelLinks:
        for linkChild in modelLink:
            if (isinstance(linkChild,ModelObject) and 
                linkChild.get(u"{http://www.w3.org/1999/xlink}type") == u"arc" and 
                arcrole == linkChild.get(u"{http://www.w3.org/1999/xlink}arcrole") and
                (arcqname is None or arcqname == linkChild)):
                fromLabel = linkChild.get(u"{http://www.w3.org/1999/xlink}from")
                toLabel = linkChild.get(u"{http://www.w3.org/1999/xlink}to")
    ...
    for hashEquivRelList in hashEquivalentRels.values():
        ...
        if len(hashEquivRelList) == 1:
            if hashEquivRelList[0].prohibitedUseSortKey == 2:
                ...
                ineffective.ineffectivity = _(u"prohibited arc (priority {0}) has no other arc to prohibit").format(
                ...
                for rel in sorted( equivalentRels ):
                    if rel[1] == 2: # this rel is prohibited
                        if priorRel is None:
                            ...
                            ineffective.ineffectivity = _(u"prohibited arc (priority {0}, {1} - {2}) has an equivalent prohibited arc (priority {3}, {4} - {5})\n").format(
                        ...
                        if priorRel is not None and priorRel[1] != 2:
                            ineffective = keyEquivRelList[priorRel[2]]
                            effective = keyEquivRelList[rel[2]]
                            ineffective.ineffectivity = _(u"arc (priority {0}, {1} - {2}) is ineffective due to equivalent arc (priority {3}, {4} - {5})\n").format(
                                                     ineffective.priority, ineffective.modelDocument.basename, ineffective.sourceline,
                                                     effective.priority, effective.modelDocument.basename, effective.sourceline)
...
def labelroles(modelXbrl, includeConceptName=False):
    ...
    return sorted(set((XbrlConst.labelroleLabel(r),r) 
                        for r in (modelXbrl.labelroles | (set([XbrlConst.conceptNameLabelRole]) if includeConceptName else set()))
                        if r is not None))
    
def baseSetRelationship(arcElement):
    modelXbrl = arcElement.modelXbrl
    arcrole = arcElement.get(u"{http://www.w3.org/1999/xlink}arcrole")
    ELR = arcElement.getparent().get(u"{http://www.w3.org/1999/xlink}role")
    for rel in modelXbrl.relationshipSet(arcrole, ELR).modelRelationships:
        if rel.arcElement == arcElement:
            return rel
    return None

class ModelRelationshipSet(object):
    __slots__ = (u"isChanged", u"modelXbrl", u"arcrole", u"linkrole", u"linkqname", u"arcqname",
                 u"modelRelationshipsFrom", u"modelRelationshipsTo", u"modelConceptRoots", u"modellinkRoleUris",
                 u"modelRelationships", u"_testHintedLabelLinkrole")
    ...
    def __init__(self, modelXbrl, arcrole, linkrole=None, linkqname=None, arcqname=None, includeProhibits=False):
        ...
        # gather arcs
        relationships = {}
        isDimensionRel =  self.arcrole == u"XBRL-dimensions" # all dimensional relationship arcroles
        isFormulaRel =  self.arcrole == u"XBRL-formulae" # all formula relationship arcroles
        isTableRenderingRel = self.arcrole == u"Table-rendering"
        isFootnoteRel =  self.arcrole == u"XBRL-footnotes" # all footnote relationship arcroles
        if not isinstance(arcrole,(tuple,frozenset)):
            arcrole = (arcrole,)
        ...
        for modelLink in modelLinks:
            ...
            for linkChild in modelLink:
                linkChildArcrole = linkChild.get(u"{http://www.w3.org/1999/xlink}arcrole")
                if linkChild.get(u"{http://www.w3.org/1999/xlink}type") == u"arc" and linkChildArcrole:
                    if isFootnoteRel:
            ...
            for arcElement in arcs:
                fromLabel = arcElement.get(u"{http://www.w3.org/1999/xlink}from")
                toLabel = arcElement.get(u"{http://www.w3.org/1999/xlink}to")
                for fromResource in modelLink.labeledResources[fromLabel]:
                    for toResource in modelLink.labeledResources[toLabel]:
    ...
    def __nonzero__(self):  # some modelRelationships exist
    ...
    def isRelated(self, modelFrom, axis, modelTo=None, visited=None, isDRS=False): # either model concept or qname
        ...
        if axis.endswith(u"self") and (modelTo is None or modelFrom == modelTo):
            return True
        isDescendantAxis = u"descendant" in axis
        if axis.startswith(u"ancestral-"): # allow ancestral-sibling...
            ...
            return isRel
        if axis.startswith(u"sibling"):  # allow sibling-or-self or sibling-or-descendant
