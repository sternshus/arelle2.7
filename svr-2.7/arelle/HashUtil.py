...
class Md5Sum(object):
    ...
    def __init__(self, initialValue=0):
        if isinstance(initialValue, int):
            ...
            raise ValueError(u"MD5Sum called with {} but must be an MD5Sum or hex number"
        ...
        if isinstance(_arg, Md5Sum):
            ...
            if isinstance(_arg, QName):
                if _arg.namespaceURI:
                    _md5.update(_arg.namespaceURI.encode(u'utf-8',u'replace'))
                    _md5.update('\x1F')
                _md5.update(_arg.localName.encode(u'utf-8',u'replace'))
            elif isinstance(_arg, _STR_UNICODE):
                _md5.update(_arg.encode(u'utf-8',u'replace'))
            elif isinstance(_arg, datetime): # always in isodate format
                _md5.update(u"{0.year:04}-{0.month:02}-{0.day:02}T{0.hour:02}:{0.minute:02}:{0.second:02}".format(_arg).encode(u'utf-8',u'replace'))
            elif isinstance(_arg, date): 
                _md5.update(u"{0.year:04}-{0.month:02}-{0.day:02}".format(_arg).encode(u'utf-8',u'replace'))
            elif isinstance(_arg, ModelObject):
                # use inner text list
                _md5.update(u'\x1F'.join(text.strip() 
                                        for text in XmlUtil.innerTextNodes(_arg, True, False, True))
                            .encode(u'utf-8',u'replace'))
    if firstMd5arg:
        md5sum = MD5SUM0
    else:
