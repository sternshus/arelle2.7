...
def viewRoleTypes(modelXbrl, outfile, header, isArcrole=False, lang=None):
    modelXbrl.modelManager.showStatus(_(u"viewing arcrole types") if isArcrole else _(u"viewing role types"))
...
class ViewRoleTypes(ViewFile.View):
    ...
    def view(self):
        ...
        if self.isArcrole:
            heading = [u"Arcrole URI", u"Definition", u"Cycles Allowed", u"Used On"]
            xmlRowElementName = u"arcroleType"
            uriAttr = u"arcroleURI"
        else:
            heading = [u"Role URI", u"Definition", u"Used On"]
            xmlRowElementName = u"roleType"
            uriAttr = u"roleURI"
        ...
        for roleUri in sorted(roletypes.keys()):
            for modelRoleType in roletypes[roleUri]:
                attr = {u"definedIn": modelRoleType.modelDocument.basename}
                cols = [roleUri, modelRoleType.genLabel(lang=self.lang, strip=True) or modelRoleType.definition or u'']
                if self.isArcrole:
                    cols.append(modelRoleType.cyclesAllowed)
                cols.append(u', '.join(unicode(usedOn)
