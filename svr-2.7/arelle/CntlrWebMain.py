...
def startWebserver(_cntlr, options):
    u"""Called once from main program in CmtlrCmdLine to initiate web server on specified local port.
    ...
    app.route(u'/rest/login', GET, login_form)
    app.route(u'/rest/login', POST, login_submit)
    app.route(u'/rest/logout', GET, logout)
    app.route(u'/favicon.ico', GET, arelleIcon)
    app.route(u'/rest/xbrl/<file:path>/open', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/close', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/validation/xbrl', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/DTS', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/concepts', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/pre', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/cal', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/dim', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/facts', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/factTable', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/roleTypes', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/arcroleTypes', GETorPOST, validation)
    app.route(u'/rest/xbrl/<file:path>/formulae', GETorPOST, validation)
    app.route(u'/rest/xbrl/validation', GETorPOST, validation)
    app.route(u'/rest/xbrl/view', GETorPOST, validation)
    app.route(u'/rest/xbrl/open', GETorPOST, validation)
    app.route(u'/rest/xbrl/close', GETorPOST, validation)
    app.route(u'/images/<imgFile>', GET, image)
    app.route(u'/rest/xbrl/diff', GET, diff)
    app.route(u'/rest/configure', GET, configure)
    app.route(u'/rest/stopWebServer', GET, stopWebServer)
    app.route(u'/quickbooks/server.asmx', POST, quickbooksServer)
    app.route(u'/rest/quickbooks/<qbReport>/xbrl-gl/<file:path>', GET, quickbooksGLrequest)
    app.route(u'/rest/quickbooks/<qbReport>/xbrl-gl/<file:path>/view', GET, quickbooksGLrequest)
    app.route(u'/rest/quickbooks/<qbReport>/xbrl-gl/view', GET, quickbooksGLrequest)
    app.route(u'/rest/quickbooks/response', GET, quickbooksGLresponse)
    app.route(u'/quickbooks/server.html', GET, quickbooksWebPage)
    app.route(u'/quickbooks/localhost.crt', GET, localhostCertificate)
    app.route(u'/localhost.crt', GET, localhostCertificate)
    app.route(u'/help', GET, helpREST)
    app.route(u'/about', GET, about)
    app.route(u'/', GET, indexPageREST)
    if server == u"cgi":
        # catch a non-REST interface by cgi Interface (may be a cgi app exe module, etc)
        app.route(u'<cgiAppPath:path>', GETorPOST, cgiInterface)
    if server == u"wsgi":
        return app
    elif server == u"cgi":
...
def cgiInterface(cgiAppPath):
    ...
    if not request.query:  # no parameters, index page
        return indexPageCGI()
    elif u'about' in request.query:
        return about(cgiAppPath + u"?image=arelle32.gif")
    elif u'help' in request.query:
        return helpREST()
    elif u'image' in request.query:
...
def login_form():
    u"""Request for a login form (get to */rest/login*).  Corresponds to login from other providers of XBRL validation services, but 
    ...
    """
    return _(u'''<html><body><form method="POST"><table>
...
def login_submit():
    u"""Login of fields from login form (post to */rest/login*).  Saves user ID for future use.
    
    :param name: User ID
    :param password: Password
    """
    name     = request.forms.get(u'name')
    password = request.forms.get(u'password')
    if checkLogin(name, password):
        return _(u"<p>You are logged in as user: {0}</p>").format(name)
    else:
        return _(u"<p>Login failed</p>")
    
def checkLogin(_user, _password):
    u"""Save user ID for future use.  Password not currently processed.
...
def logout():
    u"""Request to log out (get */rest/logout*).  Removes any proior user ID from session.
    
    :returns: html -- Message that user has logged out
    """
    global user
    user = None
    return _(u"<p>You are logged out.</p>")

def arelleIcon():
    u"""Request for icon for URL display (get */favicon.ico*).
    
    :returns: ico -- Icon file for browsers
    """
    return static_file(u"arelle.ico", root=imagesDir, mimetype=u'image/vnd.microsoft.icon')

def image(imgFile):
    u"""Request for an image file for URL display (get */images/<imgFile>*).
...
validationOptions = {
    ...
    u"efm": (u"validateEFM", True),
    u"efm-pragmatic": (u"disclosureSystemName", u"efm-pragmatic"),
    u"efm-strict": (u"disclosureSystemName", u"efm-strict"),
    u"disclosure-system": (u"disclosureSystemName", None),
    u"ifrs": (u"gfmName", u"ifrs"),
    u"hmrc": (u"gfmName", u"hmrc"),
    u"sbr-nl": (u"gfmName", u"sbr-nl"),
    u"utr": (u"utrValidate", True),
    u"infoset": (u"infosetValidate", True),
    # these parameters pass through the value after + in query
    u"import": (u"importFiles", None),
                     }

class Options():
    u"""Class to emulate options needed by CntlrCmdLine.run"""
    def __init__(self):
        for option, defaultValue in optionsPrototype.items():
            setattr(self, option, defaultValue)
            
supportedViews = set([u'DTS', u'concepts', u'pre', u'cal', u'dim', u'facts', u'factTable', u'formulae', u'roleTypes', u'arcroleTypes'])

def validation(file=None):
    u"""REST request to validate, by *get* or *post*, to URL patterns including */rest/xbrl/<file:path>/{open|close|validation|DTS...}*,
    ...
    """
    errors = []
    flavor = request.query.flavor or u'standard'
    media = request.query.media or u'html'
    requestPathParts = request.urlparts[2].split(u'/')
    isValidation = u'validation' == requestPathParts[-1] or u'validation' == requestPathParts[-2]
    view = request.query.view
    viewArcrole = request.query.viewArcrole
    if request.method == u'POST':
        sourceZipStream = request.body
        mimeType = request.get_header(u"Content-Type")
        if mimeType not in (u'application/zip', u'application/x-zip', u'application/x-zip-compressed', u'multipart/x-zip'):
            errors.append(_(u"POST must provide a zip file, Content-Type '{0}' not recognized as a zip file.").format(mimeType))
    else:
    ...
    if isValidation:
        if view or viewArcrole:
            errors.append(_(u"Only validation or one view can be specified in one requested."))
        if media not in (u'xml', u'xhtml', u'html', u'json', u'text'):
            errors.append(_(u"Media '{0}' is not supported for validation (please select xhtml, html, xml, json or text)").format(media))
    elif view or viewArcrole:
        if media not in (u'xml', u'xhtml', u'html', u'csv', u'json'):
            errors.append(_(u"Media '{0}' is not supported for view (please select xhtml, html, xml, csv, or json)").format(media))
    elif requestPathParts[-1] not in (u"open", u"close"):                
        errors.append(_(u"Neither validation nor view requested, nothing to do."))
    if (flavor not in (u'standard', u'standard-except-formula', u'formula-compile-only', u'formula-compile-and-run')
        and not flavor.startswith(u'edgar') and not flavor.startswith(u'sec')):
        errors.append(_(u"Flavor '{0}' is not supported").format(flavor)) 
    if view and view not in supportedViews:
        errors.append(_(u"View '{0}' is not supported").format(view))
    if errors:
        errors.insert(0, _(u"URL: ") + (file or request.query.file or u'(no file)'))
        return errorReport(errors, media)
    options = Options() # need named parameters to simulate options
    isFormulaOnly = False
    for key, value in request.query.items():
        if key == u"file":
            setattr(options, u"entrypointFile", value)
        elif key == u"flavor":
            if value.startswith(u"sec") or value.startswith(u"edgar"):
                setattr(options, u"validateEFM", True)
            elif value == u"formula-compile-only":
                isFormulaOnly = True
                setattr(options, u"formulaAction", u"validate")
            elif value == u"formula-compile-and-run":
                isFormulaOnly = True
                setattr(options, u"formulaAction", u"run")
            elif value == u"standard-except-formula":
                setattr(options, u"formulaAction", u"none")
        elif key in(u"media", u"view", u"viewArcrole"):
            pass
    ...
    if file:
        setattr(options, u"entrypointFile", file.replace(u';',u'/'))
    requestPathParts = set(request.urlparts[2].split(u'/'))
    viewFile = None
    if isValidation:
        if not isFormulaOnly:
            setattr(options, u"validate", True)
    elif view:
        viewFile = FileNamedStringIO(media)
        setattr(options, view + u"File", viewFile)
    elif viewArcrole:
        viewFile = FileNamedStringIO(media)
        setattr(options, u"viewArcrole", viewArcrole)
        setattr(options, u"viewFile", viewFile)
...
def runOptionsAndGetResult(options, media, viewFile, sourceZipStream=None):
    u"""Execute request according to options, for result in media, with *post*ed file in sourceZipStream, if any.
    ...
    if media == u"xml":
        response.content_type = u'text/xml; charset=UTF-8'
    elif media == u"csv":
        response.content_type = u'text/csv; charset=UTF-8'
    elif media == u"json":
    ...
    if successful and viewFile:
        # defeat re-encoding
        result = viewFile.getvalue().replace(u"&nbsp;",u"\u00A0").replace(u"&shy;",u"\u00AD").replace(u"&amp;",u"&")
        viewFile.close()
    elif media == u"xml":
        ...
        result = htmlBody(tableRows(cntlr.logHandler.getLines(), header=_(u"Messages")))
    return result

def diff():
    u"""Execute versioning diff request for *get* request to */rest/xbrl/diff*.
    
    :returns: xml -- Versioning report.
    """
    if not request.query.fromDTS or not request.query.toDTS or not request.query.report:
        return _(u"From DTS, to DTS, and report must be specified")
    options = Options()
    setattr(options, u"entrypointFile", request.query.fromDTS)
    setattr(options, u"diffFile", request.query.toDTS)
    fh = FileNamedStringIO(request.query.report)
    setattr(options, u"versReportFile", fh)
    cntlr.run(options)
...
def configure():
    u"""Set up features for *get* requests to */rest/configure*, e.g., proxy or plug-ins.
    
    :returns: html -- Status of configuration request (e.g., proxy or plug-ins).
    """
    if not request.query.proxy and not request.query.plugins and not request.query.packages and u'environment' not in request.query:
        return _(u"proxy, plugins, packages or environment must be specified")
    options = Options()
    if request.query.proxy:
        setattr(options, u"proxy", request.query.proxy)
    if request.query.plugins:
        setattr(options, u"plugins", request.query.plugins)
    if request.query.packages:
        setattr(options, u"packages", request.query.packages)
    if u'environment' in request.query:
        setattr(options, u"showEnvironment", True)
    cntlr.run(options)
    response.content_type = u'text/html; charset=UTF-8'
    return htmlBody(tableRows(cntlr.logHandler.getLines(), header=_(u"Configuration Request")))

def stopWebServer():
    u"""Stop the web server by *get* requests to */rest/stopWebServer*.
    ...
    thread.start()
    response.content_type = u'text/html; charset=UTF-8'
    return htmlBody(tableRows((time.strftime(u"Received at %Y-%m-%d %H:%M:%S"),
                               u"Good bye...",), 
                              header=_(u"Stop Request")))
    
    
def quickbooksServer():
    u"""Interface to QuickBooks server responding to  *post* requests to */quickbooks/server.asmx*.
...
def quickbooksGLrequest(qbReport=None, file=None):
    u"""Initiate request to QuickBooks server for *get* requests to */rest/quickbooks/<qbReport>/xbrl-gl/...*.
    ...
    errors = []
    requestPathParts = request.urlparts[2].split(u'/')
    viewRequested = u"view" == requestPathParts[-1]
    media = request.query.media or u'html'
    fromDate = request.query.fromDate
    toDate = request.query.toDate
    if qbReport not in supportedQbReports:
        errors.append(_(u"QuickBooks report '{0}' is not supported (please select from: {1})").format(
                          qbReport, u', '.join(supportedQbReports)))
    if media not in (u'xml', u'xhtml', u'html'):
        errors.append(_(u"Media '{0}' is not supported for xbrl-gl (please select xhtml, html or xml)").format(media))
    if not fromDate or dateTime(fromDate) is None:
        errors.append(_(u"FromDate '{0}' missing or not valid").format(fromDate))
    if not toDate or dateTime(toDate) is None:
        errors.append(_(u"ToDate '{0}' missing or not valid").format(toDate))
    if errors:
        return errorReport(errors, media)
    ticket = qbRequest(qbReport, fromDate, toDate, file)
    result = htmlBody(tableRows([_(u"Request queued for QuickBooks...")], header=_(u"Quickbooks Request")), script=u'''
...
def quickbooksGLresponse():
    u"""Poll for QuickBooks protocol responses for *get* requests to */rest/quickbooks/response*.
    ...
    if not status:
        return htmlBody(tableRows([_(u"QuickBooks ticket not found, request canceled.")], header=_(u"Quickbooks Request")))
    if status.startswith(u"ConnectionErrorMessage: "):
        CntlrQuickBooks.qbRequestStatus.pop(ticket, None)
        return errorReport([status[24:]], media)
    if status != u"Done" or ticket not in CntlrQuickBooks.xbrlInstances:
        return htmlBody(tableRows([_(u"{0}, Waiting 20 seconds...").format(status)], 
                                  header=_(u"Quickbooks Request")), 
                                  script=u'''
<script type="text/javascript">
<!-- 
...
''')
    ...
    options = Options()
    setattr(options, u"entrypointFile", instanceUuid)
    viewFile = FileNamedStringIO(media)
    setattr(options, u"factsFile", viewFile)
    return runOptionsAndGetResult(options, media, viewFile)

def quickbooksWebPage():
    return htmlBody(_(u'''<table width="700p">
...
def localhostCertificate():
    u"""Interface to QuickBooks server responding to  *get* requests for a host certificate */quickbooks/localhost.crt* or */localhost.crt*.
...
def helpREST():
    u"""Help web page for *get* requests to */help*.
    
    :returns: html - Table of CntlrWebMain web API
    """
    return htmlBody(_(u'''<table>
...
''') if cntlr.isGAE else u'') +
u'</table>')

def about(arelleImgFile=None):
    from lxml import etree
    u"""About web page for *get* requests to */about*.
...
</table>''') % (arelleImgFile or u'/images/arelle32.gif',
                cntlr.__version__, 
...
def indexPageREST():
    u"""Index (default) web page for *get* requests to */*.
...
def indexPageCGI():
    u"""Default web page response for *get* CGI request with no parameters.
...
def htmlBody(body, script=u""):
    u"""Wraps body html string in a css-styled html web page
...
def tableRows(lines, header=None):
    u"""Wraps lines of text into a one-column table (for display of text results of operations, such as processing messages and status, to web browser).
    ...
    return u'<table cellspacing="0" cellpadding="4">%s\n</table>' % (
            (u"<tr><th>%s</th></tr>" % header if header else u"") + 
            u"\n".join(u"<tr><td>%s</td></tr>" % line.replace(u"&",u"&amp;").replace(u"<",u"&lt;") for line in lines))

def errorReport(errors, media=u"html"):
    u"""Wraps lines of error text into specified media type for return of result to a request.
    ...
    if media == u"text":
        ...
        response.content_type = u'text/html; charset=UTF-8'
        return htmlBody(tableRows(errors, header=_(u"Messages")))
    
def multipartResponse(parts):
    # call with ( (filename, contentType, content), ...)
    boundary=u'----multipart-boundary-%s----' % (uuid.uuid1(),)
    response.content_type = u'multipart/mixed; boundary=%s' % (boundary,)
    buf = []
    
    for filename, contentType, content in parts:
        buf.append(u"\r\n" + boundary + u"\r\n")
        buf.append(u'Content-Disposition: attachment; filename="{0}";\r\n'.format(filename))
        buf.append(u'Content-Type: {0};\r\n'.format(contentType))
        buf.append(u'Content-Length: {0}\r\n'.format(len(content)))
        buf.append(u'\r\n')
        buf.append(content)
    buf.append(u"\r\n" + boundary + u"\r\n")
    s = u''.join(buf)
