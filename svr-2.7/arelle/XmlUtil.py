...
xmlEncodingPattern = re.compile(ur"\s*<\?xml\s.*encoding=['\"]([^'\"]*)['\"].*\?>")
xpointerFragmentIdentifierPattern = re.compile(ur"([\w.]+)(\(([^)]*)\))?")
xmlnsStripPattern = re.compile(ur'\s*xmlns(:[\w.-]+)?="[^"]*"')
...
def xmlns(element, prefix):
    ...
    if ns:
        return ns
    if prefix == u'xml': # not normally declared explicitly
    ...
    for prefix, NS in element.nsmap.items():
        if NS == ns:
            if prefix is not None:
                ...
                return u""   # prefix none but exists, xml process as zero-length string
    return None
    
def targetNamespace(element):
    treeElt = element
    while treeElt is not None:
        if treeElt.localName == u"schema" and treeElt.namespaceURI == XbrlConst.xsd and treeElt.get(u"targetNamespace"):
            return treeElt.get(u"targetNamespace")
...
def schemaLocation(element, namespace, returnElement=False):
    treeElt = element
    while treeElt is not None:
        sl = treeElt.get(u"{http://www.w3.org/2001/XMLSchema-instance}schemaLocation")
        if sl:
            ns = None
...
def clarkNotationToPrefixNsLocalname(element, clarkName, isAttribute=False):
    ns, sep, localName = clarkName[1:].partition(u'}')
    if sep:
...
def prefixedNameToNamespaceLocalname(element, prefixedName, defaultNsmap=None):
    if prefixedName is None or prefixedName == u"":
        return None
    names = prefixedName.partition(u":")
    if names[2] == u"":
        #default namespace
        prefix = None
        localName = names[0]
...
def prefixedNameToClarkNotation(element, prefixedName):
    ...
    return u"{{{0}}}{1}".format(ns, localname)

def encoding(xml, default=u"utf-8"):
    if isinstance(xml,str):
        ...
        if s.startswith('\xfe\xff'):
            return u'utf-16'
        if s.startswith('\xff\xfe\x00\x00'):
            return u'utf-32'
        if s.startswith('\x00\x00\xfe\xff'):
            return u'utf-32'
        if s.startswith('# -*- coding: utf-8 -*-'):
            return u'utf-8'  # python utf=encoded
        if "x\0m\0l" in s:
            unicode = s.decode(u"utf-16")
        else:
            unicode = s.decode(u"latin-1")
    else:
        unicode = xml[0:80]
    match = xmlEncodingPattern.match(unicode)
...
def innerText(element, ixExclude=False, ixEscape=False, ixContinuation=False, strip=True):   
    try:
        text = u"".join(text for text in innerTextNodes(element, ixExclude, ixEscape, ixContinuation))
        if strip:
            return text.strip()
        return text
    except TypeError:
...
def innerTextList(element, ixExclude=False, ixEscape=False, ixContinuation=False):   
    try:
        return u", ".join(text.strip() for text in innerTextNodes(element, ixExclude, ixEscape, ixContinuation) if len(text.strip()) > 0)
    except TypeError:
        return u""
...
def innerTextNodes(element, ixExclude, ixEscape, ixContinuation):
    ...
    for child in element.iterchildren():
        if isinstance(child,ModelObject) and (
           not ixExclude or 
           (child.localName != u"exclude" and child.namespaceURI != u"http://www.xbrl.org/2008/inlineXBRL")):
            firstChild = True
        ...
        if child.tail:
            yield child.tail
    if ixContinuation:
        contAt = element.get(u"_continuationElement")
...
def escapedNode(elt, start, empty):
    ...
    if not start and not empty:
        s.append(u'/')
    s.append(unicode(elt.qname))
    if start or empty:
        for n,v in elt.items():
            s.append(u' {0}="{1}"'.format(qname(elt,n),v))
...
def collapseWhitespace(s):
    return u' '.join( nonSpacePattern.findall(s) ) 
...
def ancestor(element, ancestorNamespaceURI, ancestorLocalNames):
    treeElt = element.getparent()
    wildNamespaceURI = not ancestorNamespaceURI or ancestorNamespaceURI == u'*'
    if not isinstance(ancestorLocalNames,tuple): ancestorLocalNames = (ancestorLocalNames ,)
    wildLocalName = ancestorLocalNames == (u'*',)
    while isinstance(treeElt,ModelObject):
...
def parent(element, parentNamespaceURI=None, parentLocalNames=None):
    ...
    if parentNamespaceURI or parentLocalNames:
        wildNamespaceURI = not parentNamespaceURI or parentNamespaceURI == u'*'
...
def children(element, childNamespaceURIs, childLocalNames):
    ...
    wildLocalName = childLocalNames == (u'*',)
    wildNamespaceURI = not childNamespaceURIs or childNamespaceURIs == u'*'
...
def child(element, childNamespaceURI=None, childLocalNames=(u"*",)):
    result = children(element, childNamespaceURI, childLocalNames)
    if result and len(result) > 0:
        return result[0]
    return None

def lastChild(element, childNamespaceURI=None, childLocalNames=(u"*",)):
    result = children(element, childNamespaceURI, childLocalNames)
    if result and len(result) > 0:
        return result[-1]
    return None
...
def descendants(element, descendantNamespaceURI, descendantLocalNames, attrName=None, attrValue=None, breakOnFirst=False):
    ...
    wildLocalName = descendantLocalNames == (u'*',)
    wildNamespaceURI = not descendantNamespaceURI or descendantNamespaceURI == u'*'
    if isinstance(element,(ModelObject,etree._ElementTree)):
        for child in (element.iterdescendants() if isinstance(element,ModelObject) else element.iter()):
            if isinstance(child,ModelObject) and \
                ...
                if attrName:
                    if child.get(attrName) == attrValue or (attrValue == u"*" and child.get(attrName) is not None):
...
def schemaDescendantsNames(element, descendantNamespaceURI, descendantLocalName, qnames=None):
    ...
    for child in element.iterdescendants(tag=u"{{{0}}}{1}".format(descendantNamespaceURI,descendantLocalName)):
        if isinstance(child,ModelObject):
            if child.get(u"name"):
                # need to honor attribute/element form default
                qnames.add(qname(targetNamespace(element), child.get(u"name")))
            elif child.get(u"ref"):
                qnames.add(qname(element, child.get(u"ref")))
    return qnames
...
def schemaDescendant(element, descendantNamespaceURI, descendantLocalName, name):
    for child in element.iterdescendants(tag=u"{{{0}}}{1}".format(descendantNamespaceURI,descendantLocalName)):
        if isinstance(child,ModelObject):
            # need to honor attribute/element form default
            if descendantLocalName == u"attribute":
                if child.get(u"name") == (name.localName if isinstance(name,QName) else name):
                    return child
            else:
                if qname(child, child.get(u"name")) == name:
                    return child
    return None

def schemaBaseTypeDerivedFrom(element):
    for child in element.iterchildren():
        if child.tag in (u"{http://www.w3.org/2001/XMLSchema}extension",u"{http://www.w3.org/2001/XMLSchema}restriction"):
            return child.get(u"base") 
        elif child.tag == u"{http://www.w3.org/2001/XMLSchema}union":
            return (child.get(u"memberTypes") or u"").split() + [
                    schemaBaseTypeDerivedFrom(child)
                    for child in element.iterchildren(tag=u"{http://www.w3.org/2001/XMLSchema}simpleType")]
        elif child.tag in (u"{http://www.w3.org/2001/XMLSchema}complexType",
                           u"{http://www.w3.org/2001/XMLSchema}simpleType",
                           u"{http://www.w3.org/2001/XMLSchema}complexContent",
                           u"{http://www.w3.org/2001/XMLSchema}simpleContent"):
    ...
    for child in element.iterchildren():
        if child.tag in facetTags:
            facets.append(child) 
                           ...
                           u"{http://www.w3.org/2001/XMLSchema}simpleType",
                           u"{http://www.w3.org/2001/XMLSchema}restriction",
                           u"{http://www.w3.org/2001/XMLSchema}complexContent",
                           u"{http://www.w3.org/2001/XMLSchema}simpleContent"):
...
def schemaAttributesGroups(element, attributes=None, attributeWildcards=None, attributeGroups=None):
    ...
    for child in element.iterchildren():
        if child.tag == u"{http://www.w3.org/2001/XMLSchema}attribute":
            attributes.append(child) 
        elif child.tag == u"{http://www.w3.org/2001/XMLSchema}anyAttribute":
            attributeWildcards.append(child) 
        elif child.tag == u"{http://www.w3.org/2001/XMLSchema}attributeGroup":
            attributeGroups.append(child) 
        elif child.tag in set([u"{http://www.w3.org/2001/XMLSchema}complexType",
                           u"{http://www.w3.org/2001/XMLSchema}simpleType",
                           u"{http://www.w3.org/2001/XMLSchema}complexContent",
                           u"{http://www.w3.org/2001/XMLSchema}simpleContent",
                           u"{http://www.w3.org/2001/XMLSchema}restriction",
                           u"{http://www.w3.org/2001/XMLSchema}extension"]):
...
def emptyContentModel(element):
    if element.tag in (u"{http://www.w3.org/2001/XMLSchema}complexType",
                     u"{http://www.w3.org/2001/XMLSchema}complexContent"):
        if element.get(u"mixed") == u"true":
            return False
    for child in element.iterchildren():
        if child.tag in (u"{http://www.w3.org/2001/XMLSchema}complexType",
                         u"{http://www.w3.org/2001/XMLSchema}complexContent"):
            if child.get(u"mixed") == u"true":
                return False
            for contentChild in child.iterdescendants():
                if contentChild.tag in (u"{http://www.w3.org/2001/XMLSchema}sequence",
                                        u"{http://www.w3.org/2001/XMLSchema}choice",
                                        u"{http://www.w3.org/2001/XMLSchema}all"):
                    return True
        elif child.tag in (u"{http://www.w3.org/2001/XMLSchema}simpleType",
                           u"{http://www.w3.org/2001/XMLSchema}simpleContent"):
            return False
    return True
...
def addChild(parent, childName1, childName2=None, attributes=None, text=None, afterSibling=None, beforeSibling=None, appendChild=True):
    ...
    if isinstance(childName1, QName):
        ...
        prefix, sep, localName = childName2.partition(u":")
        if localName:
            if existingPrefix is None:
                setXmlns(modelDocument, prefix, childName1)
        else:
            localName = prefix
        child = modelDocument.parser.makeelement(u"{{{0}}}{1}".format(childName1, localName))
    ...
    if attributes:
        for name, value in (attributes.items() if isinstance(attributes, dict) else
            ...
            if isinstance(name,QName):
                ...
                child.set(name.clarkNotation, unicode(value))
            else:
...
def addComment(parent, commentText):
    comment = unicode(commentText)
    if u'--' in comment: # replace -- with - - (twice, in case more than 3 '-' together)
...
def addQnameValue(modelDocument, qnameValue):
    ...
    if hasattr(modelDocument, u"modelDocument"): 
        ...
        if xmlRootElement.tag == u"nsmap": xmlRootElement = xmlRootElement[0]
    ns = qnameValue.namespaceURI or u'' # None can't be used as a no-namespace prefix
    ...
    if existingPrefix is not None:  # namespace is already declared, use that for qnameValue's prefix
        return qnameValue.localName if len(existingPrefix) == 0 else existingPrefix + u':' + qnameValue.localName
    ...
    while (dupNum < 10000): # check if another namespace has prefix already (but don't die if running away)
        ...
        prefix = u"{0}_{1}".format(qnameValue.prefix if qnameValue.prefix else u'', dupNum)
        dupNum += 1
    setXmlns(modelDocument, prefix, ns)
    return qnameValue.localName if len(prefix) == 0 else prefix + u':' + qnameValue.localName
...
def setXmlns(modelDocument, prefix, namespaceURI):
    ...
    if prefix not in root.nsmap:
        if root.tag == u'nsmap': # already have an xmlns-extension root element
            newmap = root.nsmap
            newmap[prefix] = namespaceURI
            newroot = etree.Element(u'nsmap', nsmap=newmap)
            newroot.extend(root)
        else:  # new xmlns-extension root
            newroot = etree.Element(u'nsmap', nsmap={prefix: namespaceURI})
            comments = []
...
def sortKey(parentElement, childNamespaceUri, childLocalNames, childAttributeName=None, qnames=False):
    ...
    if parentElement is not None:
        for childLocalName in childLocalNames if isinstance(childLocalNames,tuple) else (childLocalNames,):
            for child in parentElement.iterdescendants(tag=u"{{{0}}}{1}".format(childNamespaceUri,childLocalName)):
                value = text(child)
                if qnames:
...
def datetimeValue(element, addOneDay=False, none=None):
    ...
    try:
        ...
        if not u"result" in locals(): # if not set yet, punt with max datetime
...
def dateunionValue(datetimeValue, subtractOneDay=False):
    ...
    if isDate or (datetimeValue.hour == 0 and datetimeValue.minute == 0 and datetimeValue.second == 0):
        ...
        return u"{0:04n}-{1:02n}-{2:02n}".format(d.year, d.month, d.day)
    else:
        return u"{0:04n}-{1:02n}-{2:02n}T{3:02n}:{4:02n}:{5:02n}".format(datetimeValue.year, datetimeValue.month, datetimeValue.day, datetimeValue.hour, datetimeValue.minute, datetimeValue.second)
    ...
    for scheme, parenPart, path in matches:
        if scheme and (parenPart is None or len(parenPart) == 0): # shorthand id notation
            if scheme in modelDocument.idObjects:
                ...
                node = modelDocument.xmlDocument.find(u"//*[@id='{0}']".format(scheme))
            if node is not None:
                return node    # this scheme fails
        elif scheme == u"element" and parenPart and path:
            ...
            if len(pathParts) >= 1 and len(pathParts[0]) > 0 and not pathParts[0].isnumeric():
                ...
                if id in modelDocument.idObjects:
                    ...
                    node = modelDocument.xmlDocument.find(u"//*[@id='{0}']".format(id))
                if node is None:
...
def elementFragmentIdentifier(element):
    if isinstance(element,etree.ElementBase) and element.get(u'id'):
        return element.get(u'id')  # "short hand pointer" for element fragment identifier
    else:
        childSequence = [u""] # "" represents document element for / (root) on the join below
        while element is not None:
            if isinstance(element,etree.ElementBase):
                if element.get(u'id'):  # has ID, use as start of path instead of root
                    childSequence[0] = element.get(u'id')
                    break
                try:
                    ...
                    for sibling in element.itersiblings(preceding=True):
                        if isinstance(sibling,etree.ElementBase):
                            siblingPosition += 1
                childSequence.insert(1, unicode(siblingPosition))
            element = element.getparent()
        location = u"/".join(childSequence)
        return u"element({0})".format(location)
...
def xmlstring(elt, stripXmlns=False, prettyPrint=False, contentsOnly=False):
    ...
    if not prettyPrint:
        xml = xml.strip()
    if stripXmlns:
        return xmlnsStripPattern.sub(u'', xml)
    else:
        return xml

def writexml(writer, node, encoding=None, indent=u'', xmlcharrefreplace=False, parentNsmap=None):
    ...
    if isinstance(node,etree._ElementTree):
        ...
        if isinstance(node,ModelObject):
            ...
            ns, sep, localName = node.tag.partition(u'}')
            if sep:
                prefix = xmlnsprefix(node,ns[1:])
                if prefix:
                    tag = prefix + u":" + localName
                else:
                    tag = localName
            else:
                tag = ns
        writer.write(indent+u"<" + tag)
        attrs = {}
        for prefix, ns in sorted((k if k is not None else u'', v) 
                                 # items wrapped in set for 2.7 compatibility
                                 for k, v in (_DICT_SET(node.nsmap.items()) - _DICT_SET(parentNsmap.items()))):
            if prefix:
                attrs[u"xmlns:" + prefix] = ns
            else:
                attrs[u"xmlns"] = ns
        for aTag,aValue in node.items():
            ns, sep, localName = aTag.partition(u'}')
            if sep:
                prefix = xmlnsprefix(node,ns[1:])
                if prefix:
                    prefixedName = prefix + u":" + localName
                else:
        ...
        indentAttrs = (u"\n" + indent + u"  ") if numAttrs > 1 and lenAttrs > 60 else u" "
        for aName in aSortedNames:
            writer.write(u"%s%s=\"" % (indentAttrs, aName))
            if aName != u"xsi:schemaLocation":
                writer.write(attrs[aName].replace(u"&",u"&amp;").replace(u'"',u'&quot;'))
            else:
                indentUri = u"\n" + indent + u"                      "
                for i, a_uri in enumerate(attrs[aName].split()):
                    if i & 1:   #odd
                        writer.write(u" " + a_uri)
                    elif i > 0:   #even
        ...
        text = node.text
        if text is not None:
            text = u''.join(u"&amp;" if c == u"&"
                           else (u"&nbsp;" if xmlcharrefreplace else u"&#160;") if c == u"\u00A0" 
                           else u"&lt;" if c == u"<"
                           else u"&gt;" if c == u">"
                           else (u"&shy;" if xmlcharrefreplace else u"&#173;") if c == u"\u00AD"
                           else u"&#x%x;" % ord(c) if c >= u'\x80' and xmlcharrefreplace
                           else c
                           for c in text)
        for child in node.iterchildren():
            ...
            if firstChild:
                ...
                if text and not text.isspace():
                    writer.write(text)
                firstChild = False
            writexml(writer, child, indent=indent+u'    ', xmlcharrefreplace=xmlcharrefreplace)
        if hasChildNodes:
            writer.write(u"%s</%s>\n" % (indent, tag))
        elif text:
            writer.write(u">%s</%s>\n" % (text, tag))
