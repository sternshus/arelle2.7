...
numberPattern = re_compile(u"[-+]?[0]*([1-9]?[0-9]*)([.])?(0*)([1-9]?[0-9]*)?([eE])?([-+]?[0-9]*)?")
ZERO = decimal.Decimal(0)
ONE = decimal.Decimal(1)
NaN = decimal.Decimal(u"NaN")
floatNaN = float(u"NaN")
floatINF = float(u"INF")

def validate(modelXbrl, inferDecimals=False):
    ValidateXbrlCalcs(modelXbrl, inferDecimals).validate()
    
class ValidateXbrlCalcs(object):
    ...
    def validate(self):
        ...
        if not self.inferDecimals: # infering precision is now contrary to XBRL REC section 5.2.5.2
            self.modelXbrl.info(u"xbrl.5.2.5.2:inferringPrecision",u"Validating calculations inferring precision.")
        ...
        self.modelXbrl.profileActivity(u"... identify equal contexts", minTimeToShow=1.0)
        ...
        self.modelXbrl.profileActivity(u"... identify equal units", minTimeToShow=1.0)
        ...
        self.modelXbrl.profileActivity(u"... identify requires-element and esseance-aliased concepts", minTimeToShow=1.0)

        self.bindFacts(self.modelXbrl.facts,[self.modelXbrl.modelDocument.xmlRootElement])
        self.modelXbrl.profileActivity(u"... bind facts", minTimeToShow=1.0)
                                            ...
                                            if roundedItemsSum  != roundFact(fact, self.inferDecimals):
                                                ...
                                                for modelRel in modelRels:
                                                    ...
                                                    if (itemConcept is not None and 
                                                        ...
                                                        unreportedContribingItemQnames.append(unicode(itemConcept.qname))
                                                self.modelXbrl.log(u'INCONSISTENCY', u"xbrl.5.2.5.2:calcInconsistency",
                                                    _(u"Calculation inconsistent from %(concept)s in link role %(linkrole)s reported sum %(reportedSum)s computed sum %(computedSum)s context %(contextID)s unit %(unitID)s unreportedContributingItems %(unreportedContributors)s"),
                                                    ...
                                                    unreportedContributors=u", ".join(unreportedContribingItemQnames) or u"none")
                                    ...
                                    for eF in self.esAlFacts[essenceFactsKey]:
                                        for aF in self.esAlFacts[aliasFactsKey]:
                                            ...
                                            if essenceUnit != aliasUnit:
                                                self.modelXbrl.log(u'INCONSISTENCY', u"xbrl.5.2.6.2.2:essenceAliasUnitsInconsistency",
                                                    _(u"Essence-Alias inconsistent units from %(essenceConcept)s to %(aliasConcept)s in link role %(linkrole)s context %(contextID)s"),
                                            ...
                                            if not XbrlUtil.vEqual(eF, aF):
                                                self.modelXbrl.log(u'INCONSISTENCY', u"xbrl.5.2.6.2.2:essenceAliasUnitsInconsistency",
                                                    _(u"Essence-Alias inconsistent value from %(essenceConcept)s to %(aliasConcept)s in link role %(linkrole)s context %(contextID)s"),
                                                    modelObject=(modelRel, eF, aF), 
                                                    essenceConcept=essenceConcept.qname, aliasConcept=aliasConcept.qname, 
                                                    linkrole=ELR,
                                                    linkroleDefinition=self.modelXbrl.roleTypeDefinition(ELR),
                        ...
                        for modelRel in relsSet.modelRelationships:
                            ...
                            if sourceConcept in self.requiresElementFacts and \
                               not requiredConcept in self.requiresElementFacts:
                                    self.modelXbrl.log(u'INCONSISTENCY', u"xbrl.5.2.6.2.4:requiresElementInconsistency",
                                        _(u"Requires-Element %(requiringConcept)s missing required fact for %(requiredConcept)s in link role %(linkrole)s"),
                                        modelObject=sourceConcept, 
                                        requiringConcept=sourceConcept.qname, requiredConcept=requiredConcept.qname, 
                                        linkrole=ELR,
                                        linkroleDefinition=self.modelXbrl.roleTypeDefinition(ELR))
        self.modelXbrl.profileActivity(u"... find inconsistencies", minTimeToShow=1.0)
...
def roundFact(fact, inferDecimals=False, vDecimal=None):
    ...
    if dStr == u"INF" or pStr == u"INF":
        ...
        if dStr:
            match = numberPattern.match(vStr if vStr else unicode(vDecimal))
            if match:
...
def inferredPrecision(fact):
    ...
    try:
        vFloat = float(vStr)
        if dStr:
            match = numberPattern.match(vStr if vStr else unicode(vFloat))
            if match:
                nonZeroInt, period, zeroDec, nonZeroDec, e, exp = match.groups()
...
def wrappedFactWithWeight(fact, weight, roundedValue):
    return ObjectPropertyViewWrapper(fact, ( (u"weight", weight), (u"roundedValue", roundedValue)) )
...
def wrappedSummationAndItems(fact, roundedSum, boundSummationItems):
    ...
    u''' ARELLE-281, replace: faster python-based hash (replace with hashlib for fewer collisions)
    ...
    for b in sorted(boundSummationItems, key=lambda b: b.modelObject.qname):
        sha256.update(b.modelObject.qname.namespaceURI.encode(u'utf-8',u'replace')) #qname of erroneous submission may not be utf-8 perfectly encodable
        sha256.update(b.modelObject.qname.localName.encode(u'utf-8',u'replace'))
        sha256.update(unicode(b.extraProperties[1][1]).encode(u'utf-8',u'replace'))
    itemValuesHash = sha256.hexdigest()
    # summation value hash
    sha256 = hashlib.sha256()
    sha256.update(fact.qname.namespaceURI.encode(u'utf-8',u'replace'))
    sha256.update(fact.qname.localName.encode(u'utf-8',u'replace'))
    sha256.update(unicode(roundedSum).encode(u'utf-8',u'replace'))
    ...
    return [ObjectPropertyViewWrapper(fact,
                                      ( (u"sumValueHash", sumValueHash),
                                        (u"itemValuesHash", itemValuesHash),
                                        (u"roundedSum", roundedSum) ))] + \
            boundSummationItems
