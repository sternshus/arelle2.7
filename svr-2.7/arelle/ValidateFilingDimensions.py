...
def checkDimensions(val, drsELRs):
    ...
    for ELR in drsELRs:
        ...
        for hasHypercubeArcrole in (XbrlConst.all, XbrlConst.notAll):
            ...
            for hasHcRels in hasHypercubeRelationships.values():
                for hasHcRel in hasHcRels:
                    ...
                    if hasHypercubeArcrole == XbrlConst.all:
                        ...
                        if not hasHcRel.isClosed:
                            val.modelXbrl.error(u"SBR.NL.2.3.6.04",
                                _(u"All hypercube %(hypercube)s in DRS role %(linkrole)s, does not have closed='true'"),
                                modelObject=hasHcRel, hypercube=hc.qname, linkrole=ELR)
                    elif hasHypercubeArcrole == XbrlConst.notAll:
                        if hasHcRel.isClosed:
                            val.modelXbrl.error((u"EFM.6.16.06", u"GFM.1.08.06"),
                                _(u"Not all hypercube %(hypercube)s in DRS role %(linkrole)s, does not have closed='false'"),
                                modelObject=hasHcRel, hypercube=hc.qname, linkrole=ELR, primaryItem=sourceConcept.qname)
                        if hc in positiveHypercubes:
                            val.modelXbrl.error((u"EFM.6.16.08", u"GFM.1.08.08"),
                                _(u"Not all hypercube %(hypercube)s in DRS role %(linkrole)s, is also the target of a positive hypercube"),
                    ...
                    if dimTargetRequired and len(hcDimRels) == 0:
                        val.modelXbrl.error((u"EFM.6.16.09", u"GFM.1.08.09"),
                            _(u"Table %(hypercube)s in DRS role %(linkrole)s, missing targetrole consecutive relationship"),
                    ...
                    for hcDimRel in hcDimRels:
                        ...
                        if isinstance(dim, ModelConcept):
                            ...
                            if not domELR:
                                if dim.isExplicitDimension:
                                    ...
                                    if val.validateSBRNL:
                                        val.modelXbrl.error(u"SBR.NL.2.3.5.04",
                                            _(u"Hypercube %(hypercube)s in DRS role %(linkrole)s, missing targetrole to dimension %(dimension)s consecutive relationship"),
                                            modelObject=hcDimRel, hypercube=hc.qname, linkrole=ELR, dimension=dim.qname)
                            else:
                                if dim.isTypedDimension and val.validateSBRNL:
                                    val.modelXbrl.error(u"SBR.NL.2.3.5.07",
                                        _(u"Typed dimension %(dimension)s in DRS role %(linkrole)s, has targetrole consecutive relationship"),
                            ...
                            if hasHypercubeArcrole == XbrlConst.all:
                                ...
                                val.modelXbrl.error((u"EFM.6.16.07", u"GFM.1.08.08"),
                                    _(u"Negative table axis %(dimension)s in DRS role %(linkrole)s, not in any positive table in same role"),
                            ...
                            if domTargetRequired and len(dimDomRels) == 0:
                                val.modelXbrl.error((u"EFM.6.16.09", u"GFM.1.08.09"),
                                    _(u"Axis %(dimension)s in DRS role %(linkrole)s, missing targetrole consecutive relationship"),
                            ...
                            if val.validateEFMorGFM:
                                ...
                                if cycleCausingConcept is not None:
                                    ...
                                    val.modelXbrl.error((u"EFM.6.16.04", u"GFM.1.08.04"),
                                        _(u"Dimension relationships have an undirected cycle in DRS role %(linkrole)s \nstarting from table %(hypercube)s, \naxis %(dimension)s, \npath %(path)s"),
                ...
                if hasHypercubeArcrole == XbrlConst.all and len(hasHcRels) > 1:
                    val.modelXbrl.error((u"EFM.6.16.05", u"GFM.1.08.05"),
                        _(u"Multiple tables (%(hypercubeCount)s) DRS role %(linkrole)s, source %(concept)s, only 1 allowed"),
                        ...
                        hypercubes=u', '.join(unicode(r.toModelObject.qname) for r in hasHcRels if isinstance(r.toModelObject, ModelConcept)))
        ...
        for relFrom, rels in fromRelationships.items():
            if relFrom in primaryItems:
                ...
                if cycleCausingConcept is not None:
                    val.modelXbrl.error((u"EFM.6.16.04", u"GFM.1.08.04"),
                        _(u"Domain-member primary-item relationships have an undirected cycle in DRS role %(linkrole)s \nstarting from %(conceptFrom)s, \npath %(path)s"),
            ...
            for rel in rels:
                ...
                if isinstance(toMbr, ModelConcept) and toELR and len(
                    ...
                    val.modelXbrl.error((u"EFM.6.16.09", u"GFM.1.08.09"),
                        _(u"Domain member %(concept)s in DRS role %(linkrole)s, missing targetrole consecutive relationship"),
    ...
    if val.validateSBRNL:
        ...
        for hc in hypercubes:
            for priHcRel in val.modelXbrl.relationshipSet(XbrlConst.all).toModelObject(hc):
                ...
                checkSBRNLMembers(val, hc, priItem, ELR, 
                                  ...
                                  ELR, False)
                if priHcRel.contextElement == u'segment':  
                    val.modelXbrl.error(u"SBR.NL.2.3.5.06",
                        _(u"hypercube %(hypercube)s in segment not allowed, ELR role %(linkrole)s"),
        ...
        for notAllRel in val.modelXbrl.relationshipSet(XbrlConst.notAll).modelRelationships:
            val.modelXbrl.error(u"SBR.NL.2.3.5.05",
                _(u"Notall from primary item %(primaryItem)s in ELR role %(linkrole)s to %(hypercube)s"),
        ...
        for ELR, hypercubes in hypercubesInLinkrole.items():
            ...
            for hc in hypercubes:  # only one member
                for arcrole in (XbrlConst.parentChild, u"XBRL-dimensions"):
                    for modelRel in val.modelXbrl.relationshipSet(arcrole, ELR).modelRelationships:
                        if (modelRel.fromModelObject != hc and modelRel.toModelObject != hc and
                            ...
                            val.modelXbrl.error(u"SBR.NL.2.2.3.05",
                                _(u"ELR role %(linkrole)s, has hypercube %(hypercube)s and a %(arcrole)s relationship not involving the hypercube or primary domain, from %(fromConcept)s to %(toConcept)s"),
                                ...
                                toConcept=(modelRel.toModelObject.qname if isinstance(modelRel.toModelObject, ModelConcept) else u"unknown"))
        domainsInLinkrole = defaultdict(set)
        ...
        for rel in val.modelXbrl.relationshipSet(XbrlConst.dimensionDomain).modelRelationships:
            ...
            if isinstance(relFrom, ModelConcept) and isinstance(relTo, ModelConcept):
                ...
                if rel.isUsable and val.modelXbrl.relationshipSet(XbrlConst.domainMember, rel.targetRole).fromModelObject(relTo):
                    val.modelXbrl.error(u"SBR.NL.2.3.7.05",
                        _(u"Dimension %(dimension)s in DRS role %(linkrole)s, has usable domain with members %(domain)s"),
                        modelObject=rel, dimension=relFrom.qname, linkrole=rel.linkrole, domain=relTo.qname)
                if not relTo.isAbstract:
                    val.modelXbrl.error(u"SBR.NL.2.3.7.02",
                        _(u"Dimension %(dimension)s in DRS role %(linkrole)s, has nonAbsract domain %(domain)s"),
                        modelObject=rel, dimension=relFrom.qname, linkrole=rel.linkrole, domain=relTo.qname)
                if relTo.substitutionGroupQname.localName not in (u"domainItem",u"domainMemberItem"):
                    val.modelXbrl.error(u"SBR.NL.2.2.2.19",
                        _(u"Domain item %(domain)s in DRS role %(linkrole)s, in dimension %(dimension)s is not a domainItem"),
                        modelObject=rel, domain=relTo.qname, linkrole=rel.linkrole, dimension=relFrom.qname)
                if not rel.targetRole and relTo.substitutionGroupQname.localName == u"domainItem":
                    val.modelXbrl.error(u"SBR.NL.2.3.6.03",
                        _(u"Dimension %(dimension)s in DRS role %(linkrole)s, missing targetrole to consecutive domain relationship"),
        ...
        for linkrole, domains in domainsInLinkrole.items():
            if linkrole and len(domains) > 1:
                val.modelXbrl.error(u"SBR.NL.2.3.7.04",
                    _(u"Linkrole %(linkrole)s, has multiple domains %(domains)s"),
                    modelObject=val.modelXbrl, linkrole=linkrole, domains=u", ".join([unicode(dom.qname) for dom in domains]))
        ...
        for dimDomMems, linkroles in linkrolesByDimDomMems.items():
            if len(linkroles) > 1:
                val.modelXbrl.error(u"SBR.NL.2.3.6.02",
                    _(u"Dimension %(dimension)s  usable members same in linkroles %(linkroles)s"),
                    modelObject=val.modelXbrl, dimension=dimDomMems[0].qname, linkroles=u', '.join(l for l in linkroles))
        ...
        for rel in val.modelXbrl.relationshipSet(XbrlConst.domainMember).modelRelationships:
            if val.modelXbrl.relationshipSet(XbrlConst.domainMember, rel.targetRole).fromModelObject(rel.toModelObject):
                val.modelXbrl.error(u"SBR.NL.2.3.7.03",
                    _(u"Domain member %(member)s in DRS role %(linkrole)s, has nested members"),
        ...
        for rel in val.modelXbrl.relationshipSet(XbrlConst.domainMember).modelRelationships:
            ...
            if isinstance(relTo, ModelConcept):
                ...
                if relFrom.substitutionGroupQname.localName == u"domainItem":
                    if relTo.substitutionGroupQname.localName != u"domainMemberItem":
                        val.modelXbrl.error(u"SBR.NL.2.2.2.19",
                            _(u"Domain member item %(member)s in DRS role %(linkrole)s is not a domainMemberItem"),
                            modelObject=rel, member=relTo.qname, linkrole=rel.linkrole)
                else:
                    if relTo.substitutionGroupQname.localName == u"domainMemberItem":
                        val.modelXbrl.error(u"SBR.NL.2.2.2.19",
                            _(u"Domain item %(domain)s in DRS role %(linkrole)s is not a domainItem"),
                            modelObject=rel, domain=relFrom.qname, linkrole=rel.linkrole)
                        break # don't repeat parent's error on rest of child members
                    elif relFrom.isAbstract and relFrom.substitutionGroupQname.localName != u"primaryDomainItem":
                        val.modelXbrl.error(u"SBR.NL.2.2.2.19",
                            _(u"Abstract domain item %(domain)s in DRS role %(linkrole)s is not a primaryDomainItem"),
                            modelObject=rel, domain=relFrom.qname, linkrole=rel.linkrole)
                        break # don't repeat parent's error on rest of child members
        ...
        for hc, DRSdims in hypercubeDRSDimensions.items():
            ...
            for ELR, mutableDims in DRSdims.items():
                ...
                if dims not in hcELRdimSets:
                    ...
                    val.modelXbrl.error(u"SBR.NL.2.3.5.02",
                        _(u"Hypercube %(hypercube)s has same dimensions in ELR roles %(linkrole)s and %(linkrole2)s: %(dimensions)s"),
                        modelObject=hc, hypercube=hc.qname, linkrole=ELR, linkrole2=hcELRdimSets[dims],
                        dimensions=u", ".join([unicode(dim.qname) for dim in dims]))
...
def cyclePath(source, cycles):
    ...
    for rel in reversed(cycles):
        if isinstance(rel,bool):
            ...
            path.append(u"{0}:{1} {2}".format(rel.modelDocument.basename, 
                                             rel.sourceline, 
                                             rel.toModelObject.qname if isForward else rel.fromModelObject.qname))
    return unicode(source.qname) + u" " + u" - ".join(path)            
        ...
        if isinstance(relFrom, ModelConcept) and isinstance(relTo, ModelConcept):
            ...
            if isDomMbr or not relTo.isAbstract:
                if relTo in members:
                    ...
                    if isDomMbr:
                        ...
                        val.modelXbrl.error(u"SBR.NL.2.3.5.01",
                            _(u"Primary items for hypercube %(hypercube)s ELR %(ELR)s, have non-unique (inheritance) member %(concept)s in DRS role %(linkrole)s"),
            ...
            if not isDomMbr: # pri item relationships
                if (relTo.isAbstract and not relFrom.isAbstract or 
                    relFrom.substitutionGroupQname.localName != u"primaryDomainItem"):
                    val.modelXbrl.error(u"SBR.NL.2.3.7.01",
                        _(u"Primary item %(concept)s in DRS role %(linkrole)s for hypercube %(hypercube)s, has parent %(concept2)s which is not a primaryDomainItem"),
