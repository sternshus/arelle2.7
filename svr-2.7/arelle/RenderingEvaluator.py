...
def init(modelXbrl):
    ...
    if modelXbrl.modelDocument.type == ModelDocument.Type.INSTANCE:
        ...
        instance = ModelDocument.create(modelXbrl, ModelDocument.Type.INSTANCE, 
                                        u"dummy.xml",  # fake URI and fake schemaRef 
                                        (u"http://www.xbrl.org/2003/xbrl-instance-2003-12-31.xsd",))
    ...
    if hasXbrlTables:
        ...
        ValidateFormula.validate(modelXbrl, xpathContext=modelXbrl.rendrCntx, parametersOnly=True, statusMsg=_(u"compiling rendering tables"))
        ...
        for modelTable in modelXbrl.modelRenderingTables:
            ...
            if modelTable.aspectModel not in (u"non-dimensional", u"dimensional") and hasNsWithAspectModel:
                modelXbrl.error(u"xbrlte:unknownAspectModel",
                ...
                oppositeAspectModel = (_DICT_SET(set([u'dimensional',u'non-dimensional'])) - _DICT_SET(set([modelTable.aspectModel]))).pop()
                ...
                if Aspect.CONCEPT not in aspectsCovered and not hasNsWithAspectModel:
                    modelXbrl.error(u"xbrlte:tableMissingConceptAspect",
                        _(u"Table %(xlinkLabel)s does not include the concept aspect as one of its participating aspects"),
                ...
                for tblParamRel in modelXbrl.relationshipSet((XbrlConst.tableParameter, XbrlConst.tableParameterMMDD)).fromModelObject(modelTable):
                    ...
                    if parameterName in parameterNames:
                        modelXbrl.error(u"xbrlte:tableParameterNameClash ",
                            _(u"Table %(xlinkLabel)s has parameter name clash for variable %(name)s"),
        ...
        modelXbrl.profileStat(_(u"compileTables"))
...
def checkBreakdownDefinitionNode(modelXbrl, modelTable, tblAxisRel, tblAxisDisposition, uncoverableAspects, aspectsCovered):
    ...
    if isinstance(definitionNode, (ModelDefinitionNode, ModelEuAxisCoord)):
        for aspect in definitionNode.aspectsCovered():
            ...
            if (aspect in uncoverableAspects or
                (isinstance(aspect, QName) and modelTable.aspectModel == u'non-dimensional')):
                modelXbrl.error(u"xbrlte:axisAspectModelMismatch",
                    _(u"%(definitionNode)s %(xlinkLabel)s, aspect model %(aspectModel)s, aspect %(aspect)s not allowed"),
            ...
            if aspect in modelTable.priorAspectAxisDisposition:
                ...
                if tblAxisDisposition != otherAxisDisposition and aspect != Aspect.DIMENSIONS:
                    modelXbrl.error(u"xbrlte:aspectClashBetweenBreakdowns",
                        _(u"%(definitionNode)s %(xlinkLabel)s, aspect %(aspect)s defined on axes of disposition %(axisDisposition)s and %(axisDisposition2)s"),
        ...
        ruleSetChildren = XmlUtil.children(definitionNode, definitionNode.namespaceURI, u"ruleSet")
        if definitionNode.isMerged:
            if ruleSetChildren:
                modelXbrl.error(u"xbrlte:mergedRuleNodeWithTaggedRuleSet",
                    _(u"Merged %(definitionNode)s %(xlinkLabel)s has tagged rule set(s)"),
            ...
            if labelRels:
                modelXbrl.error(u"xbrlte:mergedRuleNodeWithLabel",
                    _(u"Merged %(definitionNode)s %(xlinkLabel)s has label(s)"),
                    modelObject=[modelTable, definitionNode] + [r.toModelObject for r in labelRels], 
                    definitionNode=definitionNode.localName, xlinkLabel=definitionNode.xlinkLabel)
            if not definitionNode.isAbstract:
                modelXbrl.error(u"xbrlte:nonAbstractMergedRuleNode",
                    _(u"Merged %(definitionNode)s %(xlinkLabel)s is not abstract"),
    ...
    if isinstance(definitionNode, ModelRuleDefinitionNode):
        ...
        for ruleSet in XmlUtil.children(definitionNode, definitionNode.namespaceURI, u"ruleSet"):
            tag = ruleSet.tagName
            ...
            if tag in tagConstraintSets:
                modelXbrl.error(u"xbrlte:duplicateTag",
                    _(u"%(definitionNode)s %(xlinkLabel)s duplicate rule set tags %(tag)s"),
        ...
        for tag, constraintSet in definitionNode.constraintSets.items():
            if otherConstraintSet is None:
                ...
                modelXbrl.error(u"xbrlte:constraintSetAspectMismatch",
                    _(u"%(definitionNode)s %(xlinkLabel)s constraint set mismatches between %(tag1)s and %(tag2)s in constraints %(aspects)s"),
                    ...
                    aspects=u", ".join(aspectStr(aspect) 
    ...
    if isinstance(definitionNode, ModelDimensionRelationshipDefinitionNode):
        hasCoveredAspect = True
        if modelTable.aspectModel == u'non-dimensional':
            modelXbrl.error(u"xbrlte:axisAspectModelMismatch",
                _(u"DimensionRelationship axis %(xlinkLabel)s can't be used in non-dimensional aspect model"),
        ...
        for aspect in definitionNode.aspectsCovered():
            if isinstance(aspect, QName): # dimension aspect
                ...
                if concept is None or not concept.isDimensionItem:
                    modelXbrl.error(u"xbrlte:invalidDimensionQNameOnAspectNode",
                        _(u"Aspect node %(xlinkLabel)s dimensional aspect %(dimension)s is not a dimension"),
                        modelObject=(modelTable,definitionNode), xlinkLabel=definitionNode.xlinkLabel, dimension=aspect)
    
    if not definitionNodeHasChild:
        if (definitionNode.namespaceURI in (u"http://www.eurofiling.info/2010/rendering", u"http://xbrl.org/2011/table") 
            and not hasCoveredAspect):
            modelXbrl.error(u"xbrlte:aspectValueNotDefinedByOrdinate",
                _(u"%(definitionNode)s %(xlinkLabel)s does not define an aspect"),
        ...
        if (isinstance(definitionNode, ModelClosedDefinitionNode) and
            ...
            modelXbrl.error(u"xbrlte:abstractRuleNodeNoChildren",
                _(u"Abstract %(definitionNode)s %(xlinkLabel)s has no children"),
                modelObject=(modelTable,definitionNode), xlinkLabel=definitionNode.xlinkLabel, definitionNode=definitionNode.localName)
...
def checkBreakdownLeafNodeAspects(modelXbrl, modelTable, tblAxisRel, parentAspectsCovered, breakdownAspects):
    ...
    if isinstance(definitionNode, (ModelDefinitionNode, ModelEuAxisCoord)):
        ...
        if not definitionNode.isAbstract and not isinstance(definitionNode, ModelBreakdown): # this is a leaf node
            ...
            if (missingAspects):
                modelXbrl.error(u"xbrlte:missingAspectValue",
                    _(u"%(definitionNode)s %(xlinkLabel)s does not define an aspect for %(aspect)s"),
                    modelObject=(modelTable,definitionNode), xlinkLabel=definitionNode.xlinkLabel, definitionNode=definitionNode.localName,
                    aspect=u', '.join(aspectStr(aspect) for aspect in missingAspects))
