...
class FactPrototype():      # behaves like a fact for dimensional validity testing
    def __init__(self, v, aspectValues=None):
        ...
        if aspectValues is None:
            aspectValues = {}
        self.aspectEntryObjectId = aspectValues.get(u"aspectEntryObjectId", None)
    ...
    def objectId(self):
        return u"_factPrototype_" + unicode(self.qname)
    ...
    def propertyView(self):
        ...
        return ((u"concept", unicode(self.qname) if self.concept is not None else u"not specified"),
                (u"dimensions", u"({0})".format(len(dims)),
                  tuple(dimVal.propertyView if dimVal is not None else (unicode(dim.qname),u"None")
                        for dim,dimVal in sorted(dims.items(), key=lambda i:i[0])
                        if hasattr(dimVal,u'propertyView')))
                  if dims else (),
                )
...
class ContextPrototype():  # behaves like a context
    def __init__(self, v, aspectValues):
        ...
        for aspect, aspectValue in aspectValues.items():
            if aspect == Aspect.PERIOD_TYPE:
                if aspectValue == u"forever":
                    self.isForeverPeriod = True
                elif aspectValue == u"instant":
                    self.isInstantPeriod = True
                elif aspectValue == u"duration":
                ...
                if v.modelXbrl.qnameDimensionDefaults.get(aspect) != aspectValue: # not a default
                    try:
                        ...
                        if contextElement != u"scenario": # could be segment, ambiguous, or no information
                ...
                if aspect == Aspect.PERIOD: # period xml object
                    ...
                    for contextPeriodAttribute in (u"isForeverPeriod", u"isStartEndPeriod", u"isInstantPeriod",
                                                   u"startDatetime", u"endDatetime", u"instantDatetime",
                                                   u"periodHash"):
                    ...
                    for entityIdentAttribute in (u"entityIdentifier", u"entityIdentifierHash"):
    ...
    def dimValues(self, contextElement, oppositeContextElement=False):
        if not oppositeContextElement:
            return self.segDimVals if contextElement == u"segment" else self.scenDimVals
        else:
            return self.scenDimVals if contextElement == u"segment" else self.segDimVals
...
class DimValuePrototype():
    ...
    def propertyView(self):
        if self.isExplicit:
            ...
            return (unicode(self.dimensionQname), 
                    XmlUtil.xmlstring( self.typedMember, stripXmlns=True, prettyPrint=True )
                    if isinstance(self.typedMember, ModelObject) else u"None" )
    ...
    def __init__(self, v, aspectValues):
        ...
        for aspect, aspectValue in aspectValues.items():
            if aspect == Aspect.UNIT: # entitytIdentifier xml object
                for unitAttribute in (u"measures", u"hash", u"isSingleMeasure", u"isDivide"):
